...
Here, **instrument your product** with OpenTelemetry. When the customer configures an endpoint (e.g. via a startup flag or config file), your application exports telemetry from the process they’re running.
...
Here, you add a **platform feature,** such as “Telemetry Drains” or “Observability Destinations”, that lets customers configure *where* to send telemetry. Your platform collects telemetry from their workload (and from your own services, like routers) and forwards it to the customer’s OTLP endpoint.<br />
...
Whether customers run Kuma’s control and data planes on their own Kubernetes clusters or VMs, it comes pre-configured to emit traces, metrics, and logs to an OTel backend. Users configure the export, which runs from the users’ Kuma deployments, through the mesh policies:
...
This is where the industry has landed.<br />
...
This provides strong logical isolation guarantees. Slowdowns or misconfigurations in one customer’s pipeline does not affect other customers.
...
To give your users a frictionless observability story:

- **Default to push:** Use OTLP to export every signal you generate. The same push-based architecture works for logs, traces, and metrics.
- **Stay vendor netural:** By sticking to one standard protocol per signal, your product instantly supports any OTel-compatible backend.
- **Document the contract:** Clearly document your endpoint formats, authentication headers, supported signals, and attribute schemas. Give your users the confidence that they can rely on your data once it lands in their backends.

That’s the pattern used by Cloudflare (except for metrics), Heroku, Kuma, and Keycloak. Designing for all three signals using open standards from the start removes friction, reduces your engineering overhead, and empowers customers to best utilize their data on their own terms.
...
- [ ]  Decide which **signals** to support out of **logs**, **traces**, and **metrics** (ideally all three).
- [ ]  Give users a way to set their **destination endpoint** and any optional auth headers. Consider offering **per-signal** endpoints
- [ ]  Optionally allow users to **enable/disable signals** (e.g. `-signals traces,metrics,logs`) to control volume and cost.
- [ ]  Emit telemetry via **OpenTelemetry SDK** or an **OTel Collector** that exports in the OTLP format for each signal.
- [ ]  **Document** all details such as the endpoint format (gRPC/HTTP), required headers, and **attribute/schema semantics per signal** (logs, traces, metrics).
- [ ]  Decide on collector topology: **one collector per tenant** vs **one collector with per-tenant pipelines**.
- [ ]  Provide a small **example config** or snippet (e.g. collector config or env vars) so users can plug it in quickly.