...
- **SigNoz**: OpenTelemetry-native APM with a Datadog-style UI, a Datadog dashboard translator, and Datadog Receiver support for staged migrations off `dd-trace`.
- **Datadog (`dd-trace`)**: Per-host APM charges and a proprietary wire protocol that ties your tracing code to the Datadog Agent, so moving backends later means rewriting the instrumentation.
