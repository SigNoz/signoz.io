...
<InlineCTA message="No user-based pricing, no vendor lock-in. Start with SigNoz Cloud in minutes — self-host anytime with the same open-source codebase." />
...
- **OpenTelemetry-Native Architecture:** SigNoz is built on [OpenTelemetry](https://signoz.io/opentelemetry/) from the ground up, so you instrument once with open standards and keep your data portable. It supports any language or framework that OpenTelemetry supports. New Relic also offers OpenTelemetry support, but its platform is still designed around its own proprietary agents.
...
<Figure src="/img/blog/common/signoz-llamaindex-detailed-trace-view.webp" alt="LLM Observability in SigNoz" caption="Track token usage, model calls, latency, and errors across your LLM applications" />
