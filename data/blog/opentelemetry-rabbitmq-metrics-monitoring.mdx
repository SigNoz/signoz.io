...
Collectors provide many metrics that you can use to monitor how your RabbitMQ server is performing. Currently, [all metrics](https://signoz.io/guides/how-to-get-all-the-metrics-of-an-instance-with-prometheus-api/) types are ‘Sum’ with value type ‘Integer,’ which essentially means it comes in the cumulative sum instead of the raw data of each message separately.
