...
meta_title: "kubectl logs: View Kubernetes Pod Logs"
published_date: 2024-05-08
updated_date: 2026-07-08
tags: [kubernetes, logging]
authors: [daniel]
description: "Master the kubectl logs command with examples: view pod logs, handle multi-container pods, filter by time, and troubleshoot common issues."
...
Before we move ahead, feel free to check out our comprehensive guide on [Kubernetes logging](https://signoz.io/blog/kubernetes-logging/) that covers the above concepts in much more detail.
...
SigNoz offers comprehensive [Kubernetes monitoring](https://signoz.io/kubernetes-monitoring/) built on OpenTelemetry, providing full-stack observability designed specifically for Kubernetes environments.
...
SigNoz follows [OpenTelemetry](https://signoz.io/opentelemetry/) semantic conventions like `host.name`, `k8s.node.name`, and `k8s.pod.name` to ensure consistent naming across telemetry signals.
