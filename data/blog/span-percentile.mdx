...
description: When you're debugging a trace and staring at a span that took 2 seconds, you need to know if that's actually slow or just normal. Span Percentile shows you the percentile inline in the trace detail view.
image: /img/blog/2026/01/span-percentile.webp
keywords: [SigNoz, Span Percentile, Distributed Tracing, OpenTelemetry, Observability, Trace Detail, Latency, APM, Developer Tools]
related_articles:
  - '/comparisons/opentelemetry-trace-id-vs-span-id'
  - '/guides/how-to-get-the-95th-percentile-of-an-average-in-prometheus'
  - '/blog/distributed-tracing-span'
  - '/blog/opentelemetry-spans'
---
...
When you open a span in the trace detail view, you'll see a percentile inline-flex rounded-full px-2 py-0.5 text-xs font-medium right next to the span name, something like "p78". This tells you immediately that this span's duration was slower than 78% of similar spans over the last hour.
...
Click on the inline-flex rounded-full px-2 py-0.5 text-xs font-medium, and an expandable panel shows you the full picture.
...
**During incident triage -** You're paged for high latency. You pull up a slow trace and find a span that took 800ms. The percentile inline-flex rounded-full px-2 py-0.5 text-xs font-medium shows "p94". That's useful because this span is slower than usual, but it's not a dramatic outlier. You keep digging. Another span shows "p99.5". There's your culprit.
...
To use it, open any trace, click on a span in the waterfall, and look for the percentile inline-flex rounded-full px-2 py-0.5 text-xs font-medium next to the span name. Click to expand the full breakdown. Use the "+" button to add filters if you need a narrower comparison.
