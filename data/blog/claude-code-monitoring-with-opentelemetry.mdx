...
<KeyPointCallout title="💡 Take It Further: Query this data directly from your terminal" defaultCollapsed={true}>
You've successfully instrumented Claude Code and can visualize the metrics in your dashboard. The natural next step is bringing that observability directly into your coding session. Using [Agent Native Observability](https://signoz.io/agent-native-observability/), you can connect the [SigNoz MCP server](https://signoz.io/docs/ai/signoz-mcp-server/) to Claude Code. Instead of switching contexts to a browser, you can query your traces, logs, and metrics in natural language to debug agent loops directly from your IDE.
</KeyPointCallout>
