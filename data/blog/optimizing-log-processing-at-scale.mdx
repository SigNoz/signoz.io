...
When customers start sending millions of logs per minute, you quickly discover whether your processing pipeline can actually scale with vertical scaling. At SigNoz, we discovered that our log collector wasn't working well when we added more CPU or memory as performance didn't improve proportionally.
...
The problem became severe at **1 million logs per minute**. At this threshold, we would see significant lag that could persist for hours, making logs unqueryable during critical periods.
...
- **Processing Rate:** 66k logs/second
- **Consumer Lag:** 2,650 messages (reduced)

### 30% Performance Gain

The processing rate increased from 35.7k to 66k logs/second, representing a **30% improvement** in throughput with the same infrastructure.
...
The testing was conducted on a 4 core machine. We achieved better performance with the same infrastructure allocation. The optimization ensures that allocated resources deliver their full potential, maximizing the value customers get from their infrastructure investment.
...
- **30% throughput improvement** (35.7k to 66k logs/second)
