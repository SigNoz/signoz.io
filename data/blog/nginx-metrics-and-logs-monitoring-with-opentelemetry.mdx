...
If this is a fresh installation of NGINX, you can add this block to your `default.conf` file. Once added, run the command `sudo nginx -t` to validate the change. If the test is successful, you can restart the NGINX service. Now, hit the URL `http://your-domain/status` (use `http://localhost` for `your domain` if you are testing this locally). You will see an output similar to what is shown below.
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/blog/2023/12/nginx_create_alerts.webp" alt="Create alerts on NGINX metrics in SigNoz"/>
