...
1. `cd` to the server directory and then run the go grpc server
   ```jsx
   SERVICE_NAME=goAppServer OTEL_EXPORTER_OTLP_HEADERS=signoz-ingestion-key=YOUR_SIGNOZ_ACCESS_TOKEN OTEL_EXPORTER_OTLP_ENDPOINT=YOUR_COLLECTOR_ENDPOINT go run .
   ```

2. `cd` to the client directory and then run the go grpc client
   ```jsx
   SERVICE_NAME=goAppClient OTEL_EXPORTER_OTLP_HEADERS=signoz-ingestion-key=YOUR_SIGNOZ_ACCESS_TOKEN OTEL_EXPORTER_OTLP_ENDPOINT=YOUR_COLLECTOR_ENDPOINT  go run .
   ```
