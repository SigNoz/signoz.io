...
authors: [mahak_kaur]
description: Kathleen Xue at Shaped used a coding agent with SigNoz MCP to fix a Redis latency bug that had lived in production for almost a year. A closer look at what agent-native observability actually takes.
...
<a href="https://www.linkedin.com/in/kathleen-xue-b2004381" target="_blank" rel="noopener noreferrer nofollow">Kathleen Xue</a> is a Senior Software Engineer at <a href="https://www.shaped.ai/" target="_blank" rel="noopener noreferrer nofollow">Shaped</a>, the Y Combinator-backed company whose APIs let teams build production-ready ranking and recommendation models. Keeping the system stable is most of the job. "80% of my job is just checking SigNoz," she says. And for months, one thing refused to stay fixed: a long tail of latency on Shaped's calls to their Redis client. The team tried fix after fix. None of them held. The slow requests kept surfacing, every so often, unpredictably.
...
Kathleen was living in SigNoz long before MCP arrived: dashboards, logs, and traces, every day. Traces most of all. She's added spans "everywhere in the code base" because they're easy to dive into, and they replaced a slower habit. The team used to lean on Python profilers to find bottlenecks; with traces she can do "the same thing as Python profiling, but at scale across many, many requests," instead of digging into one at a time.

Even with good traces, though, debugging latency meant repetition: open one slow trace, find the bottleneck, fix it, then do it again for the next one. MCP changed the unit of work. Now she can hand the agent "around 10 trace IDs" at once and ask it to "find the trend across these requests and identify the bottleneck," instead of working through them one by one.
