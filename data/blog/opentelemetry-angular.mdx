...
@NgModule({
  ...
  imports: [
    ...
    OpenTelemetryInterceptorModule.forRoot({
      ...
      otelcolConfig: {
        url: 'https://ingest.{REGION}.signoz.cloud:443/v1/traces', // URL of opentelemetry collector
        headers: {
          // SigNoz access token
...
Make sure to update then URL of OpenTelemetry Collector under `otelcolConfig`. 
In our case since we’re using SigNoz Cloud, the URL is `https://ingest.{REGION}.signoz.cloud:443/v1/traces`.

If you are running SigNoz in local, the URL will be `http://127.0.0.1:4318/v1/traces`.
...
To start your backend API, you can do so by using the below command:

```bash
OTEL_RESOURCE_ATTRIBUTES=service.name=NAME_OF_SERVICE \
...
- `NAME_OF_SERVICE` is the name of the service you want
...
- replace `REGION` with your region, (in, us, etc)
