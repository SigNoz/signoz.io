...
A freshness note: both tools now sit downstream of OpenTelemetry, which has become the standard way to instrument applications and emit traces. Jaeger in particular has leaned in, with Jaeger v2 built on the [OpenTelemetry Collector](https://signoz.io/blog/opentelemetry-collector-complete-guide/) and its legacy client libraries deprecated in favor of OpenTelemetry SDKs. In practice you instrument with OpenTelemetry and choose Jaeger or Zipkin (or another backend) to store and visualize the traces.
