...
meta_title: "OpenTelemetry Collector Loki Receiver for Logs"
published_date: 2023-12-09
updated_date: 2026-07-08
tags: [OpenTelemetry]
authors: [daniel]
description: "Configure Promtail to send logs through the OpenTelemetry Collector Loki receiver to SigNoz, with a step-by-step code tutorial."
image: /img/blog/2023/12/otel-col-loki-cover.jpeg
hide_table_of_contents: false
keywords: [opentelemetry,signoz,observability,metrics,logs,loki]
related_articles:
  - '/blog/nginx-metrics-and-logs-monitoring-with-opentelemetry'
  - '/blog/parsing-logs-with-the-opentelemetry-collector'
  - '/blog/opentelemetry-apm'
  - '/opentelemetry/setting-up-signoz'
...
While this article focuses specifically on ingesting logs via the Loki receiver, you can check out our guide on the [OpenTelemetry Collector log-parsing workflow](https://signoz.io/blog/parsing-logs-with-the-opentelemetry-collector/) to learn how to transform and filter your log data once it arrives.
...
The OpenTelemetry Collector offers various deployment options to suit different environments and preferences. It can be deployed using Docker, Kubernetes, Nomad, or directly on Linux systems. You can find all the <a href = "https://opentelemetry.io/docs/collector/installation" rel="noopener noreferrer nofollow" target="_blank" >installation options</a>. For this article, the OpenTelemetry Collector will be installed manually.
...
It receives logs from Promtail, transforms them from the Loki format to the OTLP format, and then passes them to the OpenTelemetry Collector for further processing and exporting to SigNoz. You can find more information on the <a href = "https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/lokireceiver" rel="noopener noreferrer nofollow" target="_blank" >OpenTelemetry Loki receiver</a>.
...
To see Promtail running, visit `http://localhost:9080/targets` on your browser.

<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/blog/2023/12/loki_promtail.webp" alt="Promtail targets page showing the local system and Flask log targets being scraped"/>
...
In the context of logs and data processing pipelines, a "[processor](https://signoz.io/docs/logs-pipelines/introduction/#processors)" refers to a component that performs specific operations on the logs as they flow through the pipeline. Processors are used to manipulate, transform, or enrich the log data according to predefined rules or configurations.
...
Regular expressions, often abbreviated as “regex,” are powerful and flexible sequences of characters that define a search pattern. It is a tool used for pattern matching within strings, making it useful for tasks such as string validation, search and replace operations, and data extraction. You can learn more about Regex from this <a href = "https://www.sitepoint.com/learn-regex/" rel="noopener noreferrer nofollow" target="_blank" >regex tutorial</a>.
