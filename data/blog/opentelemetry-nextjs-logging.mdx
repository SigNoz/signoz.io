...
```bash
npm install -E @opentelemetry/api@1.30.0 \
...
> **Important:** Keep the OpenTelemetry stable (`1.x`) packages and experimental logs (`0.207.x`) packages on matching versions. Mixing lines causes TypeScript errors like `Property 'addLogRecordProcessor' does not exist on type 'LoggerProvider'` and Resource constructor typing issues.

**Dependency Breakdown:**

- **`@opentelemetry/api`**, **`@opentelemetry/resources`**, **`@opentelemetry/semantic-conventions`**: Stable core APIs and metadata helpers
- **`@opentelemetry/api-logs`**, **`@opentelemetry/sdk-logs`**, **`@opentelemetry/exporter-logs-otlp-http`**: Experimental logging API + SDK + OTLP/HTTP exporter (all `0.207.0`)
...
Pinning the package versions in Step 1 ensures `loggerProvider.addLogRecordProcessor(...)` exists, and switching to `resourceFromAttributes(...)` keeps the `Resource` typings happy in modern OpenTelemetry releases.
...
### Which Logger Should You Import?

| Context                                              | Import                                              | Why                                                                                                                                                      |
| ---------------------------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Server components, route handlers, server actions    | `import { logger } from "@/lib/logger"`             | Primary server logger that mirrors console output and forwards every entry to OpenTelemetry via `exportLogEntry`.                                        |
| Client components (including `'use client'` helpers) | `import { browserLog } from "@/lib/browser-logger"` | Buffers in the browser, sends to `/api/logs`, and the API route forwards to the server logger.                                                           |
| Optional structured console logging on the server    | `import { serverLogger } from "@/lib/pino-logger"`  | Use when you want Pino output in addition to the default logger. Call `exportLogEntry(...)` inside custom helpers if you also want these logs in SigNoz. |

This split keeps the API surface clear: reach for `logger` on the server, `browserLog` in the browser, and treat the Pino-backed `serverLogger` as an optional add-on for richer console formatting.
...
Create a page that lets you trigger both server and browser logs and confirm they reach SigNoz end-to-end:
...
export default function LogsDemoPage() {
  ...
  return (
    <div style={{ padding: 16 }}>
      <h1>Logs Demo</h1>
      <p>Trigger a server action and client-side logs, then verify them in SigNoz.</p>
      <form action={serverLogAction}>
        <button type="submit">Emit server logs (server action)</button>
      </form>
...
This sample makes the flow explicit: `logger` runs on the server (including server actions) and exports directly to OpenTelemetry, while `browserLog` runs in the client and relies on `/api/logs` to forward entries to the same pipeline.

**Quick verification checklist**

1. Start the OTel Collector/SigNoz stack and `yarn dev` for your Next.js app.
2. Visit `/logs-demo`, submit the server-action form, and click both client-side buttons.
3. In SigNoz Logs, filter by `service.name="nextjs-observability-demo"` and check for entries with `source="server"` and `source="client"`.
4. Open any entry to confirm `traceId`/`spanId` values are present (if tracing is enabled) and that client logs show metadata such as `userAgent`.
