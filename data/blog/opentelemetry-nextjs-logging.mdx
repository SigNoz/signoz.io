...
meta_title: "Structured Logging in Next.js with OpenTelemetry"
published_date: 2025-06-25
updated_date: 2026-08-10
tags: [OpenTelemetry, JavaScript, Logging]
authors: [yuvraj]
description: "Implement structured logging in your Next.js app with OpenTelemetry, including the OTLP log exporter and full trace correlation."
...
related_articles:
  ...
  - '/blog/opentelemetry-nextjs-production'
...
If you haven’t instrumented tracing yet, please follow the first guide in this series, “[How to Instrument Next.js Applications with OpenTelemetry](/blog/opentelemetry-nextjs/),” and then come back.

From here on, we’ll assume you already have the `nextjs-observability-demo` project running with `@vercel/otel` traces and the OpenTelemetry collector stack — the same `create-next-app@latest --example with-supabase` scaffold and `.env.local` OTEL variables set up in [Part 1](/blog/opentelemetry-nextjs/).

## Step 1 – Add an OTEL_SERVICE_NAME

One addition on top of your existing `.env.local`: a stable service name for the logs pipeline. This tells our app **what** to call itself when it emits logs (`OTEL_SERVICE_NAME`), separate from `OTEL_EXPORTER_OTLP_ENDPOINT`, which already tells it **where** to send data.
...
This file is the heart of our server-side logging. We'll create `lib/logs-exporter.ts` to configure and launch the `LoggerProvider`, which is responsible for batching and sending logs to our collector. The exporter emits [OpenTelemetry logs](https://signoz.io/blog/opentelemetry-logs/) in the standard [OTLP format](https://signoz.io/blog/what-is-otlp/), ready for any compatible backend.
...
<Figure
  src="/img/blog/2025/06/opentelemetry-nextjs-logging-image 1.webp"
...
<Figure
  src="/img/blog/2025/06/opentelemetry-nextjs-logging-image 2.webp"
