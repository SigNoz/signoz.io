...
## Structured Logging in Next.js with OpenTelemetry
...
```
[Browser] ─▶ [browser-logger.ts] ─▶ /api/logs ─▶ logger.ts ─▶ logs-exporter.ts ─▶ OTEL Collector ─▶ SigNoz
```

---

## 1. Server-Side Logging with Trace Context
...
## 2. Client-Side Logging with Auto-Export
...
// instrumentation.tsexport async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    // Initialize tracing    await import("@vercel/otel");    // Initialize logs export    const { initializeLogsExporter } = await import("./lib/logs-exporter");    initializeLogsExporter();  }
}
```
...
- Filter by source: `source="browser"` or `source="server"`
    ...
    <Figure src="/img/blog/2025/06/opentelemetry-nextjs-logging-image.webp" alt="" caption="" />- Click any log to view trace metadata
...
- Correlate with full request trace via `traceId`
    ...
    <Figure src="/img/blog/2025/06/opentelemetry-nextjs-logging-image%202.webp" alt="" caption="" />The real power of working with logs and traces in SigNoz comes from correlation - click “Inspect in Trace” or the trace ID to jump directly to the corresponding trace when checking a log or “Go to related logs” from any trace details page.
