...
meta_title: Node.js Performance Monitoring Explained Practically
...
related_articles:
  ...
  - '/opentelemetry/custom-metrics-nodejs'
...
On instrumentation, the current best practice is OpenTelemetry's zero-code auto-instrumentation: install `@opentelemetry/auto-instrumentations-node` and load it with the `--require` flag. It captures HTTP, database, and framework spans without code changes. Manual SDK calls are now mainly for adding custom spans on top of that.
...
```powershell
npm install @opentelemetry/api @opentelemetry/sdk-node @opentelemetry/sdk-metrics @opentelemetry/exporter-metrics-otlp-http @opentelemetry/host-metrics
```
...
async function main() {
  try {
    ...
    // Start host metrics AFTER sdk.start() so they use the registered MeterProvider
    const hostMetricsCollector = new HostMetrics();
...
```js
import './otel.js'; // starts the NodeSDK and registers the global MeterProvider
import { metrics } from '@opentelemetry/api';

// Get a meter from the SDK's registered global MeterProvider
const meter = metrics.getMeter('custom-node-meter');
...
// Bucket boundaries are configured via a View on the NodeSDK (see otel.js), not here
...
After you’ve implemented the above [OpenTelemetry](https://signoz.io/opentelemetry/) setup in your Node.js application, your app will start sending custom metrics — like `custom_node_metric` — to SigNoz every 5 seconds.
...
<Figure src="/img/blog/2025/06/nodejs-performance-monitoring-image 1.webp" alt="Signoz Custom Metrics Dashboard" caption="Signoz Custom Metrics Dashboard" />
...
> 👉 For a detailed guide on Auto Instrumentation with OpenTelemetry, check out: SigNoz | [**OpenTelemetry Node.js - Getting Started with Tracing, Logs, and Metrics**](https://signoz.io/opentelemetry/nodejs/)
> 
...
<Figure src="/img/blog/2025/06/nodejs-performance-monitoring-image 2.webp" alt="Signoz Metrics: Explorer tab" caption="Signoz Metrics: Explorer tab" />
...
<Figure src="/img/blog/2025/06/nodejs-performance-monitoring-image 3.webp" alt="Query feature on Explorer tab" caption="Query feature on Explorer tab" />
...
<Figure src="/img/blog/2025/06/nodejs-performance-monitoring-image 4.webp" alt="Custom Metrics Alert Panel" caption="Custom Metrics Alert Panel" />
...
<Figure src="/img/blog/2025/06/nodejs-performance-monitoring-image 5.webp" alt="Monitor Host and Kubernetes Metrics" caption="Monitor Host and Kubernetes Metrics" />
...
4. **PM2**: A process manager with basic monitoring and log handling; easy to use but limited [APM](https://signoz.io/blog/open-source-apm-tools/) capabilities.
...
- [Java application monitoring](https://signoz.io/guides/monitoring-java-applications/)
