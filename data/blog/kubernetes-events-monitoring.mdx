...
Each [OpenTelemetry](https://signoz.io/opentelemetry/) receiver collects its subset of metrics, logs and/or traces. In this case, Kubernetes Events Receiver only collects logs. Here are the logs collected by Kubernetes Events Receiver that are split into 2 groups: **events** and **objects**.
...
Learn more about [Kubernetes & OpenTelemetry Collectors on SigNoz](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/).
...
In this tutorial, you installed an OpenTelemetry Collector to collect Kubernetes events and sent the collected data to SigNoz for monitoring and alerts. See [Kubernetes monitoring with SigNoz](https://signoz.io/kubernetes-monitoring/) for the broader cluster observability workflow.
