...
title: OpenTelemetry for Rust Developers - The Complete Implementation Guide
...
<Figure src="/img/blog/2026/03/opentelemetry-rust-80552ce3-40fc-434a-92df-e7bb32a0187b.webp" alt="uv, which is now the go-to Python package manager, is essentially entirely written in Rust" caption="uv, which is now the go-to Python package manager, is essentially entirely written in Rust" />

(source: <a href="https://github.com/astral-sh/uv" rel="noopener noreferrer nofollow" target="_blank">https://github.com/astral-sh/uv</a>)
...
This section gives a brief overview of OpenTelemetry. Feel free to skip ahead to the next section that dives into technical decisions.
...
<Figure src="/img/blog/2026/03/opentelemetry-rust-image.webp" alt="The OpenTelemetry Rust application emits log events via the tracing crate." caption="The OpenTelemetry Rust application emits log events via the tracing crate." />
...
As discussed above, the OTel Rust project focuses on *bridging* existing logging implementations. The `opentelemetry::global` module therefore lacks a helper like `set_tracer_provider` or `set_meter_provider` for logs.
...
<Figure src="/img/blog/2026/03/opentelemetry-rust-image%201.webp" alt="SigNoz trace detail view showcasing an error trace for sending an invalid payload." caption="SigNoz trace detail view showcasing an error trace for sending an invalid payload." />
...
<Figure src="/img/blog/2026/03/opentelemetry-rust-image%202.webp" alt="Access all the logs for a trace via telemetry correlation in SigNoz." caption="Access all the logs for a trace via telemetry correlation in SigNoz." />
...
<Figure src="/img/blog/2026/03/opentelemetry-rust-image%203.webp" alt="A distributed trace in SigNoz. Notice how each external call creates a child span under the caller service’s span." caption="A distributed trace in SigNoz. Notice how each external call creates a child span under the caller service’s span." />
...
- **Active Requests:** Since this metric records the number of in-flight requests, it will primarily capture data points for the (relatively) long-running `/fibonacci` and the `//`  fallback endpoints.
    ...
    <Figure src="/img/blog/2026/03/opentelemetry-rust-5054a75e-0523-4740-a9fe-184ed35a9d29.webp" alt="Viewing the Active Requests data per-url, we can see predictable traffic patterns." caption="Viewing the Active Requests data per-url, we can see predictable traffic patterns." />
...
- **Request Duration**: This metric is an OpenTelemetry [histogram](https://signoz.io/blog/opentelemetry-histogram/) that records the duration of each request and increments a counter corresponding to a specific duration *bucket* for that endpoint. These buckets enable the observability backend to calculate percentiles and infer the performance of that endpoint over time.
    ...
    <Figure src="/img/blog/2026/03/opentelemetry-rust-ade2692e-80ee-4830-b843-8b42fcb921ab.webp" alt="Request duration histograms track system health by monitoring p99 (and other) latencies." caption="Request duration histograms track system health by monitoring p99 (and other) latencies." />
