---
title: "OpenTelemetry with Serilog in .NET: A Practical Guide"
meta_title: "OpenTelemetry Serilog Logging in .NET"
published_date: 2026-01-14
updated_date: 2026-06-16
tags: [OpenTelemetry, DotNet]
authors: [dhruv_ahuja]
description: Integrate Serilog with OpenTelemetry in .NET to correlate logs with traces, troubleshoot faster, and build a production-ready observability pipeline.
image: /img/blog/2026/01/opentelemetry-serilog-impact.webp
keywords: [serilog opentelemetry, serilog otlp sink, dotnet structured logging, opentelemetry logging, dotnet monitoring, serilog, opentelemetry serilog]
related_articles:
  - '/blog/opentelemetry-dotnet-logs'
...
This section describes Serilog and its key features. Skip to the [next section](#opentelemetry-connecting-logs-and-traces) if you are already familiar with Serilog.
...
## OpenTelemetry: Connecting Logs and Traces
...
<KeyPointCallout title="Beyond Serilog">
Serilog is just one way to ship .NET logs. For a fuller look at [sending .NET logs with OpenTelemetry](https://signoz.io/blog/opentelemetry-dotnet-logs/)—including other libraries and approaches—see our dedicated guide.
</KeyPointCallout>
...
If you are new to OpenTelemetry in .NET, you can also check out our [OpenTelemetry .NET guide](https://signoz.io/blog/opentelemetry-dotnet/).

Before we begin with the implementation overview and hands-on steps, ensure you have:
...
Log-level overrides minimize the “noise” generated by system libraries during application startup. The log context and machine name enrichers are standard Serilog enrichers. Log context allows us to define attributes to add to all log entries within a context (e.g., an HTTP endpoint method). Next, we specify the Console and SQLite output sinks, tweaking data organization within the database.

For shipping .NET logs specifically without relying on Serilog, see how to collect [.NET application logs with OpenTelemetry](https://signoz.io/blog/opentelemetry-dotnet-logs/).
