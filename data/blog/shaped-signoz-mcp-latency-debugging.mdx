...
authors: [mahak_kaur]
description: Kathleen Xue at Shaped used a coding agent with SigNoz Cloud and the SigNoz MCP server to fix a Redis latency bug that had persisted in production for almost a year.
...
<a href="https://www.linkedin.com/in/kathleen-xue-b2004381" target="_blank" rel="noopener noreferrer nofollow">Kathleen Xue</a> is a Senior Software Engineer at <a href="https://www.shaped.ai/" target="_blank" rel="noopener noreferrer nofollow">Shaped</a>, the Y Combinator-backed company whose APIs let teams build production-ready ranking and recommendation models. Her work centers on reliability and latency, so keeping the system stable is the priority, and that means SigNoz Cloud is almost always open. "80% of my infra debugging work is checking SigNoz," she says. And for months, one thing refused to stay fixed. Shaped's calls to their Redis client carried a long tail of latency. The team tried fix after fix. None of them held. The slow requests kept surfacing, every so often, unpredictably.

One day, out of frustration, she stopped guessing at fixes. She pulled a batch of traces from SigNoz Cloud that all showed the same timeout from the same client, handed them to a coding agent through the [SigNoz MCP server](https://signoz.io/docs/ai/signoz-mcp-server/), and asked it to just tell her what was going on.

Because the agent could see both the traces in SigNoz Cloud and Shaped's codebase, it connected the two quickly. The problem was in how they were configuring their async worker environment.
...
SigNoz Cloud provided the traces and logs; the codebase provided the implementation details. By combining both, Kathleen could verify whether the agent's explanation matched the actual system behavior before trusting the fix. That's the division that works today. The agent gathers and connects evidence, and the engineer brings the context (what the business cares about, and what a real trend looks like versus a coincidence).

So her advice for teams trying this is about the data first, not the agent. Get your logs, your Kubernetes events, and whatever custom metrics you track into SigNoz Cloud, she says, so that when you use the MCP "it's going to have all that rich information to go off of."
...
After this conversation, Shaped announced that it had been acquired by Whatnot. [Read the full announcement here](https://www.shaped.ai/blog/shaped-is-joining-whatnot).
