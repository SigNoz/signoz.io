...
MSI brought SigNoz into that environment before a major ERP migration. The team needed a better way to store logs, inspect traces, and find the exact request behind a bug. After the migration, that evidence became part of daily debugging. One save in the UI could touch around ten endpoints, pass through older business logic and stored procedures, and still depend on the ERP and database underneath it.
...
So Taylor connected Claude to SigNoz through SigNoz MCP, before SigNoz shipped its in-product AI teammate, [Noz](https://signoz.io/blog/introducing-noz-your-ai-teammate-in-signoz/). Now, instead of opening one trace at a time, he could ask the agent to compare traces and hand back the pattern. "It was pulling in more context than I was able to pull in just from looking through the one span and going through each trace," he says.
...
Taylor still decides which pattern is real, which fix is worth making, and how to explain the issue to the team that owns it.
