...
description: Learn how the SigNoz MCP server lets you investigate logs and traces from plain English — whether you're starting from a vague symptom or a failing endpoint.
...
SigNoz's native OTel integration means traces, logs, and metrics are already correlated. The MCP builds on SigNoz’s native OTel integration by removing the remaining friction of querying, filtering, and navigation that still require manual effort even when the data is in one place.
...
- a failing endpoint with no trace ID at all
...
That transition is important because investigations rarely start with perfect info. You don't always know which service to look at or waht the logs look like. Traditional observability tooling handles the analysis well once you have all that context, but gathering that context takes time.

The SigNoz MCP removes that step. Whether you're starting from a symptom or a failing endpoint, the entry point is always the same. Describe what you're looking for, and let the MCP find it. The correlation between traces, logs, and metrics that SigNoz provides natively through OpenTelemetry is still doing the heavy lifting underneath, but the MCP makes accessing it conversational rather than manual.
