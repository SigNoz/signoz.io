...
New Relic is a cloud-based observability platform that covers application monitoring, infrastructure monitoring, log management, and alerting in a single platform. Unlike Splunk, which requires [separate products and add-ons](https://signoz.io/comparisons/newrelic-vs-splunk/) for each of these, each with its own pricing, New Relic brings everything together with pricing based on data ingest (per GB) plus user licensing (and optionally compute-based pricing for advanced features).
...
If you want to widen the search beyond this list, our roundups of [open source log management tools](https://signoz.io/blog/best-open-source-log-management-tools/) and [log analysis tools](https://signoz.io/comparisons/log-analysis-tools/) cover the same ground from different angles. And if Elastic is on your shortlist, the [Elasticsearch vs Splunk](https://signoz.io/blog/elasticsearch-vs-splunk/) comparison weighs search depth against cost in more detail.
