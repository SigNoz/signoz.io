...
image: /img/blog/2025/10/auth-model-opentelemetry-cover.webp
...
Gemini defines authentication as the process of verifying that a user, device, or system is who or what it claims to be, typically by using credentials like a username and password. When I was first learning about authenticating systems, I related it to the term authenticity, which is closely related to trustworthiness, that is, can the source of incoming data or request be trusted enough to accept it?  You can stick with a definition or build an idea based on what works best for you. :)
...
That’s why this week, I want to talk about something crucial but often overlooked: authentication for your OpenTelemetry Collectors. These collectors are the busy data hubs of your observability pipeline, handling huge amounts of information every moment. Securing them is non-negotiable, and also a perfect use case for strong authentication.



## Authentication in OpenTelemetry
...
For more complex authentication, you can follow the same sequence of steps as we did for receivers above, but we register the extension under exporters. Here’s the entire code sample.


```yaml
extensions:
  # --- Extension 1: AWS SigV4 Authentication ---
  # Used by exporters that need AWS-style request signing (e.g., OpenSearch, Managed Service).
  sigv4auth:
    region: “us-east-1”
    service: “aoss”  # Example: Amazon OpenSearch Service

exporters:
  # --- Exporter 1: Simple API Key Authentication (Most Common) ---
  # Sends telemetry to SigNoz Cloud using a static ingestion key.
  otlp:
    endpoint: “ingest.us.signoz.cloud:443”
    headers:
      # The secret key is loaded from an environment variable for security.
      signoz-ingestion-key: “${SIGNOZ_API_KEY}”

  # --- Exporter 2: Advanced Authentication using an Extension ---
  # Sends data to an AWS service using SigV4 signing.
  otlphttp/aws:
    endpoint: “https://my-opensearch-domain.us-east1.aoss.amazonaws.com”
    auth:
      # This tells the exporter to use the ‘sigv4auth’ extension defined above.
 ...
 service:
   extensions: [sigv4auth]  # Enable the SigV4 extension (if used)
   pipelines:
     traces:
       receivers: [otlp]
       processors: [batch]
       exporters: [otlp]  # or [otlphttp/aws]
```
