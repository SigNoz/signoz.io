...
- **SigNoz**: OpenTelemetry-native, priced on data volume rather than node or container count, with a Datadog dashboard translator and Datadog Receiver support for staged migrations.
...
Most teams running Datadog on Kubernetes report that the K8s side of the bill grows faster than the cluster itself. This article walks through how Datadog actually monitors Kubernetes, where the costs and operational pain points compound, and how teams replace the Datadog stack with OpenTelemetry and SigNoz.
...
Rotating high-cardinality tags increases custom-metric usage because [billing keys off unique metric-name and tag-value combinations](https://signoz.io/blog/datadog-custom-metrics-pricing/). A team that emits one cleanly-named application metric can end up paying for tens of thousands of unique series once high-cardinality Kubernetes tags are attached.
...
| **Wire protocol** | Datadog-native protocol; OTLP ingest also supported on newer Agent paths | OTLP (open standard) |
