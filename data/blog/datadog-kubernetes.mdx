...
Both products will record `bookings_total{user_id="..."}` but the difference is on the invoice. Datadog counts every unique combination of metric name and tag values as a billable custom metric, while SigNoz Cloud prices by sample volume regardless of tag count.

A metric tagged with `user_id` on a cluster with 100,000 distinct users produces 100,000 billable series on Datadog. On SigNoz Cloud, the same data costs the same as the metric without the tag.

### A cluster that autoscales

On Datadog, scaling from 20 nodes to 200 multiplies the per-host APM and Infrastructure Monitoring charges by ten, even when telemetry volume does not change. On SigNoz Cloud, the bill follows data volume, so horizontal scaling stays cheap.
...
SigNoz Cloud's primitives map closely to Datadog's: service maps, trace waterfalls, the log explorer, and [Kubernetes resource views](https://signoz.io/kubernetes-monitoring/). Engineers who already know Datadog can usually find their way around SigNoz in hours, not weeks.
