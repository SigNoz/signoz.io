...
authors: [mahak_kaur]
description: A case study of how Alien Intelligence built an AI SRE workflow on SigNoz Cloud to triage alerts, reduce noise, and keep humans in the loop.
image: /img/blog/2026/06/alien-blog-cover.webp
keywords: [ai sre workflow, signoz mcp, alert triage]
related_articles:
  - '/blog/introducing-agent-native-observability'
  - '/blog/automating-oncall-lifecycle-signoz-mcp'
...
We recently launched the [hosted SigNoz MCP server](https://signoz.io/docs/ai/signoz-mcp-server/) for SigNoz Cloud, and our users are building interesting workflows to make their lives easier.

We got on a call with one such user. [Leo Blondel](https://www.linkedin.com/in/leo-blondel-963435202/), CTO at [Alien Intelligence](https://www.alien.club/), was kind to take the time to discuss their AI SRE workflow, which they built with SigNoz Cloud. If you’re currently building such workflows, this blog will give you a clear look at how they built it, what worked, and what other teams can borrow from it.
...
Using the hosted SigNoz MCP server in SigNoz Cloud, he built an AI SRE workflow that takes over the first pass of alert triage: checking whether an alert is real, gathering context, and sending him a Slack message only when it needs attention. It also gave the team a pattern they can reuse for future workflows where humans and agents work together.

Here’s an overview of how the AI SRE workflow looks.

<Figure
  src="/img/blog/2026/06/ai-sre-workflow-alien-overview-image.webp"
  alt="AI SRE workflow that Alien Intelligence built with SigNoz Cloud"
  caption="AI SRE workflow that Alien Intelligence built with SigNoz Cloud"
/>

The workflow starts with alerts in SigNoz Cloud. When something fires, SigNoz Cloud POSTs a webhook to a small channel server that forwards the event into a long-running Claude agent. The agent uses the hosted SigNoz MCP server to inspect traces, logs, and metrics. It also has *read-only* access to Kubernetes and GitLab, a Slack channel for two-way conversation with the on-call human, and its own runbook with per-alert playbooks.
...
## Why SigNoz Cloud Worked for This Workflow
...
He had been leaning toward SigNoz before building any of this. OpenTelemetry-native, open source, and a pricing model that worked for his team. After Datadog didn't work out, he configured everything in SigNoz Cloud and found the setup faster than expected. The documentation being structured for AI consumption also cut down the prompt engineering needed to get the agent querying correctly.

Two things made SigNoz Cloud fit this workflow specifically: direct MCP access to telemetry, and an open-source foundation the agent could reason about.
...
That nuance matters because MCP alone was not the full differentiator. He said Datadog also has an MCP server, and that it is *"not bad."* What made SigNoz Cloud easier for this workflow was the combination of MCP access, OpenTelemetry-native data, open source code and docs, and a pricing model that worked for his team.
...
SigNoz Cloud gave the agent structured access to telemetry without scraping, translation layers, or black-box guesswork.
...
SigNoz Cloud gives this workflow the pieces it needs: alerts, logs, metrics, traces, dashboards, and MCP access in one OpenTelemetry-native platform.
