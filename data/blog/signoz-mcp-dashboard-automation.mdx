...
slug: signoz-mcp-dashboard-automation
date: 2026-04-29
tags: [SigNoz, MCP, Observability, Dashboards]
...
Dashboards are one of the fundamental parts of observability. They give you visibility into how your services are performing, what's healthy, and what isn't. But building them is a different story. Setting one up properly takes time, requires knowing the right metric names, and involves lots of manual configuration that you put off until you really need it.

That delay has real consequences. A new service goes to production without proper monitoring because nobody had time to build the dashboard before the release. Or an incident starts and the on-call engineer doesn't have the exact panels they're looking for. Dashboards exist to give you visibility, but there's oftentimes a lot of friction to build them.
...
When a new service gets deployed, there are some things you immediately need to know: is it healthy, what's the error rate, how is latency looking? Getting answers to those questions without a dashboard means manually filtering through metrics and traces every time you want to check.  But as we've established, building that dashboard takes time you often don't have right at the moment of a release.
...
```text
Check if the payments-service is sending telemetry to SigNoz.
...
<figure data-zoomable align='center'>
    <img
        ...
        alt="Dashboard for payments-service showing error rate, p99 latency, and throughput panels"
    />
</figure>
...
```text
The payments-service is throwing errors and latency has spiked. Create an incident dashboard
...
<figure data-zoomable align='center'>
    <img
        ...
        alt="Incident dashboard scoped to payments-service"
    />
</figure>

If a service is throwing errors, the MCP identifies other services that it depends on and builds dependency health panels for all of them without you needing to list them out. Now you're able to create detailed, incident-specific dashboards in the middle of an active incident in seconds, without any manual effort.
...
The SigNoz MCP allows you to change the way you approach dashboards. Dashboards stops being something you plan, build, and maintain in advance, and becomes something you generate on demand for exactly what you need at any given moment.
