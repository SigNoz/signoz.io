...
Running the collector as a Deployment gives you a single, cluster-wide instance that is well suited to gathering cluster-level signals such as node and pod metrics, resource quotas, and Kubernetes events. The following setup shows how to collect and visualize these signals end to end with OpenTelemetry.
