...
The OTel API is the interface for instrumentation. It provides standardised classes and methods to create spans, record metrics, and [log events](https://signoz.io/comparisons/opentelemetry-events-vs-logs/) in your code. Using the OTel API means your instrumentation will be compatible with any OpenTelemetry-compliant backend, avoiding vendor lock-in in your code.
...
3/ The data reaches the [OTel](https://signoz.io/blog/opentelemetry-vs-datadog/) Collector, which acts as the processing hub. Here, telemetry can be sampled, filtered to reduce noise, or enriched using metadata from other systems. This step adds valuable context to raw signals.
