...
slug: kubernetes-monitoring-opentelemetry
date: 2024-06-02
tags: [Tech Tutorial]
...
Working in the observability and monitoring space for the last few years, we have had multiple users complain about lack of detailed monitoring for messaging queues and Kafka in particular. Especially with the coming of standards like OpenTelemetry, 
...
Some challenges that can be solved involves:
...
**Fig. 2:** Kafka setup with 2 Topics, multiple producers and 2 consumer group
...
[Reference](https://github.com/SigNoz/kafka-opentelemetry-instrumentation/tree/master/client-spans)  
...
You check out our detailed video tutorial here on how to setup complete client side monitoring of Kafka [here](https://www.youtube.com/watch?v=ov1pyCBwlJs) 
...
2. Consumers are slow and are not processing messages fast enough. This can be because of 2 reasons
    a. Consumers getting slow (either they call some other services like DBs internally and they are having issues or some other reasons)
    b. Consumers having high errors
3. Producers are sending high volume of data 
...
2. **Consumer Group issues** - The other source of issues can be consumers getting slow. These will be reflected in RED metrics of the consumer services and users can dive deeper into traces of any service which they find having high latency to understand the exact traces which are causing the issue. This could surface issues like if the consumer is calling a DB internally and it is getting blocked. Or if a third party API being called by the consumer is getting slow. This could also happen if a consumer is stuck in a retry or got blocked in processing due to a weird/heavy message.
...
**Approach** : Possible [reasons](https://docs.confluent.io/platform/current/monitor/monitor-consumer-lag.html#causes-of-consumer-lag) for increase in consumer lag are:
...
Another issue we have observed users facing is around dropping of messages from producer to consumer services. More explicitly - the messages were not consumed within an accepted time interval by the consumer. A typical use case for this scenario could be an email delivery system - where emails are sent to a queue and from there forwarded to multiple inboxes. If they are not delivered within a specified time (say 5 minutes), they could be considered as having been dropped.


With the above solution approaches, you can find if latency has increased between a producer and consumer services. But some times, the messages have not reached consumers and in that case the end-to-end latency is `undefined` and it is not included in the p99 latency calculations. 

In such a case, the producer metrics will be fine, consumers metrics will be fine. Consumer lag will be going up but you need to know which messages could not make to consumer with a given SLO and might want to look into those traces and drill down by customers affected by doing a group by on `customer_id` and even decide to act on the SLO breach by escalation
...
In the above sections, we have explained the limitations of currently available solutions for monitoring messaging systems (more specifically Kafka). We have proposed solutions for this can be improved deeply leveraging tracing powered by OpenTelemetry.
...
Please comments below or write to us at <a href="mailto:product@signoz.io">product@signoz.io</a> 
