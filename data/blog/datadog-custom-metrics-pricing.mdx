...
This blog walks through how Datadog actually bills custom metrics, what counts as one, how cardinality drives the total, what each plan includes, and how Metrics without Limits and Metric Name pricing change the math. Every figure here comes from Datadog's official billing documentation as of July 2026.
...
Above mentioned clean multiplication holds only when your tags vary independently. If tags are correlated, adding one may not raise the count, Datadog notes your total usually scales with the most granular tag rather than the product of every tag.
</KeyPointCallout>
...
This is where the `ingested` vs. `indexed` split from earlier comes back. By default, every custom metric you send is both ingested (received by Datadog) and indexed (kept queryable), and you're billed on the indexed count. [Metrics without Limits](https://docs.datadoghq.com/metrics/metrics-without-limits/) lets you keep sending everything while choosing which tags stay queryable, which shrinks your indexed volume and, with it, your bill. Take the `request.latency` example again. Say you drop the high-cardinality `user_id` tag from indexing but keep `region` and `status`. Your indexed count falls from 5,000 back down to 4 combinations, while all 5,000 are still ingested so the raw data isn't lost. 
...
[SigNoz](https://signoz.io/) on other hand takes a different approach, and here's how the two compare on the things that actually move a custom metrics bill:
