...
Once you've instrumented your Next.js app with OpenTelemetry, the next step is getting it into production; then once it's live, staying on top of what actually happens in the real world. Whether you're shipping via Vercel or running your own infra, the setup has some key differences worth noting.

And once traces, metrics, and logs are flowing, this guide also covers the monitoring patterns you'll reach for first, such as 404s, external APIs, and exceptions, plus alerting, sampling, and data sanitization for when you're running at scale. (Tracking frontend [Web Vitals and widget performance](https://signoz.io/blog/opentelemetry-nextjs-web-vitals/) is covered earlier in this series.)
...
404s aren't just noise, for example, a spike after a deploy usually means a broken redirect or a route that moved without a corresponding link update.
...
Useful for debugging navigation flow, understanding aspects like where users get stuck, which pages act as dead ends, and whether errors correlate with a particular entry point.
...
For DB queries, background jobs, and similar app-level gaps, wrap them manually the same way as the [Common Monitoring Patterns](#common-monitoring-patterns) above (start a span, tag it, end it).
