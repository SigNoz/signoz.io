...
I am joined today by Pranay, co-founder and CEO of SigNoz. We will be discussing some basic concepts of the [OpenTelemetry](https://signoz.io/comparisons/opentelemetry-vs-splunk/) collector. We have drawn a few questions from the CNCF Slack channel, and those specific questions have already been answered. Therefore, we will not be addressing those questions directly.
...
The other only disadvantage or one of the disadvantages of this is that because you are running an application pod with each, like an [OTel](https://signoz.io/blog/opentelemetry-vs-datadog/) collector with each of the application pods, you are just running so many of them and that is higher resource requirements.
