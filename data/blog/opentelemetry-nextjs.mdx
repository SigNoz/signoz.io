...
Once done, you can recognize the next steps forward to ensure you have an observability system that scales as your business needs -- and inevitably, the application complexity -- do. For the purpose of this write-up, we'll showcase how NextJS integrates with OpenTelemetry.
...
So, when you lack instrumentation for your Next.js applications, you are essentially flying blind, and are prone to errors that would be tough to contextualize, and correlate otherwise. For example, redirection from  an application's home page to the `Choose Project` section, based on the user authorization status, could cause a duplicate API call due to state mismatch. Without observability into your application's web interactions, you might not be able to correlate the duplicate API call with the state mismatch easily. Whereas, instrumentation would showcase all operations that the application makes, under a [single trace](https://signoz.io/blog/distributed-tracing/).
