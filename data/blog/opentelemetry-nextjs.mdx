...
description: "Learn how to instrument your Next.js application using OpenTelemetry and collect traces, metrics, and logs a for full-stack observability setup."
...
related_articles:
  ...
  - '/blog/opentelemetry-browser-instrumentation'
  - '/blog/opentelemetry-express'
...
<InlineCTA message="Go beyond Vercel's built-in observability. Send your Next.js traces, metrics, and logs to SigNoz for end-to-end visibility across your entire stack." />
...
<Figure src="/img/blog/2025/06/opentelemetry-nextjs-image 1.webp" alt="SigNoz Trace Details Page with Support for over a million span." caption="SigNoz Trace Details Page with Support for over a million span." />

## Alternatively: Direct Export to SigNoz

If you're prototyping quickly or running in an environment where adding an OTel Collector isn't practical, you can send traces directly from the app to SigNoz,  skipping the collector entirely, using the `OTLPHttpJsonTraceExporter` from `@vercel/otel`.<br />
See [Collector vs Direct Exporter](https://signoz.io/blog/opentelemetry-nextjs-production/#collector-vs-direct-exporter) later in this series for the full setup and the tradeoffs involved.
...
In the next parts of this series, we'll go deeper into what you can actually *do* with this data:

- Track frontend Web Vitals and third-party widget performance
- Implement structured logging with full trace correlation, across both server and browser
- Trace 404s, slow external API calls, and unhandled exceptions
- Deploy to production, set up alerting, and tune sampling for scale
