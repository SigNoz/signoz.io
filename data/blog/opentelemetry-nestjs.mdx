...
date: 2025-07-24
tags: [OpenTelemetry, NestJS, Distributed Tracing, Observability]
...
- TypeScript 5.0.4+
- NestJS 4.0+ (8.0+ recommended for better compatibility)
- An observability backend (we'll use [SigNoz](https://signoz.io/teams/) for this guide)

> If you're using NestJS 11+, ensure you're running Node.js 20+, as it's a requirement for compatibility.
...
When you implement the above distributed tracing pattern across your microservices, you can visualize the complete request flow in SigNoz's trace view. The platform automatically correlates all spans within a trace, showing you exactly how requests flow between services, where bottlenecks occur, and which operations fail.

For example, when an order creation request fails during payment processing, you can immediately see the error context, timing information, and the exact service where the failure occurred:

<Figure src="/img/blog/2025/07/opentelemetry-nestjs-signoz-trace-view.webp" alt="SigNoz trace details page showing a trace with error" caption="SigNoz trace details page showing a trace with error" />
...
You can find a complete working example of all the concepts covered in this guide in our [NestJS OpenTelemetry demo repository](https://github.com/yuvraajsj18/nestjs-otel-demo).
...
<Figure src="/img/blog/2025/07/opentelemetry-nestjs-apm-view.webp" alt="SigNoz's APM view displaying your `nestjs-otel-demo` service with key metrics" caption="SigNoz's APM view displaying your `nestjs-otel-demo` service with key metrics" />
...
OpenTelemetry’s Node.js SDK requires Node.js 18.19.0+ or 20.6.0+. Additionally, if you are using NestJS 11+, you must use Node.js 20 or higher.
