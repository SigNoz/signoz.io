...
Prefer to instrument in code instead of using the CLI? Call `FirecrawlInstrumentor().instrument()` once at startup, before you create the `Firecrawl` client. The client binds its methods at construction time, so clients created before instrumentation are not traced. See the <a href="https://pypi.org/project/firecrawl-otel/" target="_blank" rel="noopener noreferrer nofollow">package documentation</a> for the in-code setup and for passing explicit tracer and meter providers.
</Admonition>
