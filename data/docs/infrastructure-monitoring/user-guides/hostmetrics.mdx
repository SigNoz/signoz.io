...
<Admonition type="info">
  The `resourcedetection` processor automatically attaches resource attributes — including `host.name`, `host.id`, and OS/cloud metadata — to every metric emitted by the receiver. Without it, hosts appear in SigNoz without a hostname and aren't clickable on the Hosts list.
</Admonition>
