---
date: 2026-05-22
title: Kubernetes Metrics - Monitor K8s Clusters
description: Setup Kubernetes monitoring with OpenTelemetry kubeletstats and k8scluster receivers. Track node, pod, container, and volume metrics in SigNoz dashboards.
doc_type: howto
id: k8s-metrics
---
...
**`allocatable_types_to_report`** controls which allocatable resource types are reported as metrics. The `k8s.node.allocatable_cpu` and `k8s.node.allocatable_memory` metrics are only available when `cpu` and `memory` are included in this list. Add `storage` to also report allocatable storage for volumes.

#### 2. Enable Kubernetes Metadata

Configure the k8sattributes processor to add Kubernetes context. Include `k8s.persistentvolumeclaim.name` if you are monitoring PersistentVolumeClaims in the Volumes view:
