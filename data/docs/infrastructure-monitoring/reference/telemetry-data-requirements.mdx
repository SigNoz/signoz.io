...
- `Volume Used` is a computed value (`capacity − available`), not a separate metric. Inode columns are an upcoming addition and will rely on `k8s.volume.inodes`, `k8s.volume.inodes.used`, `k8s.volume.inodes.free`.
- **Setup**:
