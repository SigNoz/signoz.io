...
- In multi-cluster setups, use this to scope troubleshooting (scheduling failures, version skew, control-plane incidents) to a single cluster. ([K8s cluster architecture](https://kubernetes.io/docs/concepts/architecture/))
...
- Compare against **CPU Alloc (cores)** to gauge saturation; when the gap closes, the node is a candidate for workload rebalancing, pod rightsizing, or cluster scale-out. ([Managing resources for containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#requests-and-limits))
...
- When **CPU Usage (cores)** approaches this value, new pods may stay `Pending` for lack of fit; revisit reservations, pod requests, or add capacity. ([Reserve compute resources for system daemons](https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/))
...
- A node trending toward its **Memory Allocatable** is at imminent eviction risk; the kubelet evicts in QoS order — BestEffort → Burstable → Guaranteed. ([Node-pressure eviction signals](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#eviction-signals), [Pod QoS](https://kubernetes.io/docs/concepts/workloads/pods/pod-qos/))
...
- When **Memory Usage (WSS)** closes in on this value and `memory.available` drops below the kubelet's hard threshold (default `100Mi`), MemoryPressure is set with `0s` grace and eviction begins. ([Hard eviction thresholds](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#hard-eviction-thresholds))
...
- The kubelet periodically posts node status to the API server; `Ready = Unknown` is what the node controller records when no kubelet update has arrived within the grace period.
- A node going NotReady taints with `node.kubernetes.io/not-ready:NoExecute`; pods are evicted after the default 5-minute toleration window (`tolerationSeconds=300`) — investigate the kubelet, the kubelet→API-server network path, and node-level resource pressure. ([Node conditions](https://kubernetes.io/docs/concepts/architecture/nodes/#condition), [Taint-based evictions](https://kubernetes.io/docs/reference/labels-annotations-taints/#node-kubernetes-io-not-ready))
...
- A `Pending` spike on a single node typically means the scheduler can't place pods there (check **CPU Alloc** / **Memory Allocatable** headroom and node taints); `Failed` / `Unknown` spikes warrant drilling into those pods' events and logs from the node detail page. ([Pod phase](https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#pod-phase))
...
- When the Usage band approaches Allocatable, the node is CPU-saturated and pods may start throttling; when Requests far exceeds Usage, scheduling reservations are sitting idle and pod `requests.cpu` can be tightened to pack the node better. ([Managing resources for containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#requests-and-limits))
...
- Plots seven series: usage **Avg / Max / Min**, **Working Set**, **RSS**, **Allocatable**, and total container **Requests**. `working_set` (anonymous + active file pages, excluding reclaimable inactive file pages) is the value the kubelet uses for OOM and eviction decisions; `usage` includes reclaimable page cache and so reads higher than working set.
- Working Set approaching Allocatable signals imminent MemoryPressure-driven eviction; the gap between Usage and Working Set is the reclaimable page cache the kernel can free first, before the kubelet starts evicting pods. ([Node-pressure eviction signals](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#eviction-signals))
...
- Allocatable % approaching 100% means the node is running out of schedulable CPU; Requests % consistently above 100% means workloads are bursting above their reservations (allowed, but a signal requests are under-provisioned and pods may contend for CPU under load). ([Managing resources for containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#requests-and-limits))
...
- When Allocatable % approaches 100%, drill into **Memory Usage (bytes)** to check whether **Working Set** is also at the threshold; sustained pressure triggers MemoryPressure and pod eviction in QoS order. ([Hard eviction thresholds](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#hard-eviction-thresholds))
...
- Use this to identify noisy-neighbor pods consuming a disproportionate share of node CPU — candidates for tighter `limits.cpu`, isolation onto a dedicated node pool via taints/tolerations, or a QoS-class adjustment so they're chosen first under pressure. ([Pod QoS](https://kubernetes.io/docs/concepts/workloads/pods/pod-qos/), [Taints and tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/))
...
- Within a QoS class, the kubelet evicts the pod whose memory usage most exceeds its `requests.memory` first — use this chart to spot those pods before MemoryPressure hits and either right-size their requests or move them off the node. ([Pod selection for kubelet eviction](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#pod-selection-for-kubelet-eviction))
...
- Sustained non-zero values warrant attention — common causes are misconfigured CNI plugins, faulty NICs, MTU mismatches, or a noisy neighbor saturating the host network; K8s does not auto-evict on network errors, so follow up via the CNI plugin's troubleshooting docs. ([Cluster networking](https://kubernetes.io/docs/concepts/cluster-administration/networking/))
...
- Use this to identify which interface (and indirectly which workload) is driving network load on the node; K8s has no eviction signal for network bandwidth, so saturation is addressed via NetworkPolicy, moving heavy workloads to dedicated nodes, or scaling out. ([Cluster networking](https://kubernetes.io/docs/concepts/cluster-administration/networking/))
...
- When Available drops below 10% of Capacity, the kubelet sets DiskPressure, garbage-collects dead pods/containers, and starts evicting pods; if `imagefs` is on a separate device the threshold is 15% of its capacity. ([Hard eviction thresholds](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#hard-eviction-thresholds), [Garbage collection](https://kubernetes.io/docs/concepts/architecture/garbage-collection/))
...
- When this approaches 90% (i.e., Available below 10% of Capacity), the kubelet triggers DiskPressure and pod eviction begins; bound disk consumption with per-pod `ephemeral-storage` limits, tune image GC, or expand the underlying volume. ([Hard eviction thresholds](https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#hard-eviction-thresholds), [Ephemeral storage](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#local-ephemeral-storage))
