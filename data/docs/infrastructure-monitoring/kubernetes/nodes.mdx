...
- A NotReady node taints with `node.kubernetes.io/not-ready:NoExecute`; pods are evicted after the default 5-minute toleration window — investigate the kubelet and the kubelet→API-server network path. ([Node conditions](https://kubernetes.io/docs/concepts/architecture/nodes/#condition), [Taint-based evictions](https://kubernetes.io/docs/reference/labels-annotations-taints/#node-kubernetes-io-not-ready))
