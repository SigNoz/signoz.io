...
- Use this to track effective cluster capacity over time — sudden drops are the most reliable signal that a node went down or was cordoned; persistent dips below your expected node count indicate ongoing node-health issues.
...
- Any non-zero value means a slice of the cluster's capacity is unusable; after the default 5-minute toleration window, pods on those nodes are evicted — so a sustained spike here is also a leading indicator for pod churn elsewhere in the cluster.
