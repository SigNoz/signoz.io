...
- This chart relies on the OTel metric `k8s.node.condition_ready` from the k8s_cluster receiver (encoded `1` = Ready, `0` = NotReady, `-1` = Unknown), taking the latest value per `k8s.node.name` in each step interval and keeping only the nodes reporting Ready (`HAVING max(condition_ready) = 1`).
- One line per Ready node, flat at `1` for as long as that node keeps accepting pods. Because the chart is filtered to `1`, a node going NotReady or Unknown leaves a gap in its line instead of dipping, so a line that stops is the signal to chase.
- Use this to confirm cluster-wide node health at a glance: compare the number of lines against your expected node count, and check **NotReady Nodes** for the nodes that went down, were cordoned, or lost contact with the API server.

#### NotReady Nodes

- This chart relies on the same OTel metric `k8s.node.condition_ready` from the k8s_cluster receiver, taking the latest value per `k8s.node.name` in each step interval and keeping only the nodes reporting NotReady (`HAVING max(condition_ready) = 0`).
- One line per node whose kubelet is reporting unhealthy. An empty chart means no node is at `0`, which is the healthy case. Nodes at `-1` (Unknown: no kubelet update within the grace period) appear in neither this chart nor **Ready Nodes**, so cross-check the node count between the two when it doesn't add up.
- Any node plotted here means a slice of the cluster's capacity is unusable; after the default 5-minute toleration window, pods on those nodes are evicted: the line's `k8s.node.name` label tells you which nodes are about to lose their pods.
