...
- When **CPU Usage (cores)** consistently approaches this, the cluster's scheduling capacity is exhausted and new pods will stay `Pending` until either nodes are added or existing pod requests are tightened.
...
- This chart relies on the OTel metric `k8s.node.condition_ready` from the k8s_cluster receiver, grouped by `k8s.node.name` with a `having max(condition_ready) = 1` filter, rendered as a table with one row per Ready node.
- Each row identifies a node currently in `Ready` state (latest `condition_ready` = 1) — the row count is the total Ready-node count, and the `k8s.node.name` column tells you exactly which nodes are healthy.
- Use this to confirm cluster-wide node health at a glance — when row count drops below your expected node total, the missing node names are the ones that went down or were cordoned.

#### NotReady Nodes

- This chart relies on the OTel metric `k8s.node.condition_ready` from the k8s_cluster receiver, grouped by `k8s.node.name` with a `having max(condition_ready) = 0` filter, rendered as a table with one row per NotReady node.
- Each row identifies a node currently in `NotReady` state — the table is empty when the cluster is healthy and rows appear as nodes transition to NotReady.
- Any rows mean a slice of the cluster's capacity is unusable; after the default 5-minute toleration window, pods on those nodes are evicted — the `k8s.node.name` column tells you which nodes are about to lose their pods.
