...
description: Production readiness checklist for SigNoz covering cluster setup, scaling, TLS, sampling, and operational best practices for running SigNoz reliably.
---
...
2. If you are running in k8s, use [k8s-infra](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/) to collect telemetry (logs, metrics, traces).
...
6. Secure signoz and otel-collector using TLS ingress. See [Secure SigNoz in Kubernetes using Ingress-NGINX and Cert-Manager | SigNoz](https://signoz.io/docs/tutorial/setting-up-tls-for-signoz/). To secure collector receivers and exporters at the component level, see [Enable TLS for OpenTelemetry Collector](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/tls-configuration/).
