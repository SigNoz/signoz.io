...
title: Hermes Agent Monitoring and Obsevability with OpenTelemetry
description: Monitoring and observability for Hermes agent usage and performance with OpenTelemetry instrumentation to send traces lgos and metrics to SigNoz
doc_type: howto
---

## Overview

This guide walks you through setting up monitoring and observability for Hermes agent using <a href="https://opentelemetry.io/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry</a> and exporting traces, metrics, and logs to SigNoz. With this integration, you can observe and track your Hermes AI agent sessions, LLM calls, tool invocations, and more.

Monitoring Hermes agent with telemetry ensures full observability across your AI coding workflows. By leveraging SigNoz, you can analyze correlated traces, logs, and metrics in unified dashboards, configure alerts, and gain actionable insights on your Hermes usage.
...
You should see `hermes_otel` listed as enabled.
...
OTEL_SIGNOZ_INGESTION_KEY=replace_with_your_signoz_ingestion_key
OTEL_PROJECT_NAME=hermes-agent
```

- Keep the ingestion key in `.env`; do **not** put the key directly in YAML.
- Change `OTEL_SIGNOZ_ENDPOINT` if your [SigNoz region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint) or self-hosted endpoint is different. It should end with `/v1/traces`.
...
This should produce a agent root span, LLM/API spans, tool spans if tools are used, Hermes metrics, and Python log records.
...
Your Hermes usage should now automatically emit traces, metrics, and logs. In SigNoz, look for a service named `hermes-agent`.

You should be able to view Hermes traces in SigNoz under the traces tab:
...
You should be able to view logs in Signoz Cloud under the logs tab. You can also view logs by clicking on the “Logs" section in the trace view to see correlated logs:
...
You should be able to see Hermes related metrics in Signoz Cloud under the metrics tab:
...
You can also check out our custom [Hermes Agent dashboard](https://signoz.io/docs/dashboards/dashboard-templates/hermes-dashboard/) which provides specialized visualizations for monitoring your Dify usage. The dashboard includes pre-built charts specifically tailored for LLM usage, along with import instructions to get started quickly.
...
With `capture_previews: true`, spans may include previews of prompts, tool arguments, tool results, and responses. For stricter privacy, use:

```yaml
capture_previews: false
```

Logs may still include application log messages. Keep `log_level: INFO` or stricter unless you intentionally need debug logs.
