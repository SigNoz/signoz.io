...
**Cause:** you are deploying with `flavor: compose` but no Compose tooling was found.
...
## `docker swarm is not active (state: inactive)`

**Cause:** you are deploying with `flavor: swarm` but Swarm mode is off.
...
## `current node is a swarm worker, not a manager`

**Cause:** `docker stack deploy` only runs from a Swarm **manager**.
...
## `compose file does not exist at path: ...`

**Cause:** you ran `cast` before generating the deployment. `cast` deploys what `forge` writes under `pours/`.
