  ...
  tail_sampling:
    ...
    policies:
      ...
      - name: keep-all-errors
        ...
        status_code:
          status_codes: [ERROR]
      # Policy 2: AND requires all three sub-policies to match
...
2. Check `decision_wait`. If it is too short, spans may not have arrived before evaluation. Start with `10s` and increase if traces are long-running.
...
3. Review your policies. A `drop` policy overrides any `sample` decision regardless of policy order. If a `drop` policy matches, the trace is dropped even if another policy would have sampled it.

</details>

## Next Steps

- [Cost Control](https://signoz.io/docs/cost-control/optimize-traces/): find which span operations drive your volume, then verify the reduction
