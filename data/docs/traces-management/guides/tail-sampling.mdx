...
<Admonition type="tip" title="Recommended policy order">
  Place `drop` policies first. You reduce the spans buffered in memory by discarding noisy, low-value traces (health checks, probes) before the keep policies run:
...
Monitor `otelcol_processor_tail_sampling_sampling_trace_dropped_too_early` for early drops. If this metric rises, increase `num_traces` or reduce `decision_wait`. Both changes increase memory usage.
...
3. Set `maximum_trace_size_bytes` to drop abnormally large traces before they consume memory.
