...
The `string_attribute` policy matches span and resource attributes, not the span name itself. To match on span name, use the `ottl_condition` policy with the `name` field.
    ...
    drop_sub_policy:
      - name: match-span-name-regex
        ...
        ottl_condition:
          error_mode: ignore
          span:
            - 'IsMatch(name, "^(health|ready|live).*")'
```
