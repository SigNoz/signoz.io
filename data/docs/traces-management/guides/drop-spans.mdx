...
1. **Probabilistic Sampling Processor**: keeps a configured percentage of traces at ingestion time, dropping the rest. Set `sampling_percentage` to the fraction you want to keep (for example, `10` keeps 10% and drops 90%).
   ...
   See the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/probabilisticsamplerprocessor" target="_blank" rel="noopener noreferrer nofollow">probabilistic sampler documentation</a> for all options.

2. **Tail Sampling Processor**: buffers all spans for a trace, then evaluates the complete trace against policies: keep errors, drop health probes, sample by latency, and more. See the [Tail Sampling](https://signoz.io/docs/traces-management/guides/tail-sampling/) guide for configuration and examples.
...
See the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/filterprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">OTel filter processor documentation</a> for the full OTTL condition reference. For a broader introduction to OTTL syntax, contexts, and functions across all signals, see [OTTL Reference](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/ottl/).
...
- [APM Metrics](https://signoz.io/docs/userguide/custom-apm-dashboards-alerts/): monitor request rate, error rate, and latency after controlling trace volume
