...
When a slow request hits, you want the logs that fired during that span. When an error surfaces in logs, you want the trace that caused it. Neither works without `trace_id` and `span_id` in your logs.
