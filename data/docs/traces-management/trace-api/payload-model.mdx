...
## Sample Payload for builderQuery with table panelType
...
```json
{
    ...
    "compositeQuery": {
        ...
        "chQueries": {
            "A": {
                "query": "SELECT stringTagMap['service.name'] AS `service.name`, toFloat64(avg(durationNano)) AS value FROM signoz_traces.distributed_signoz_index_v2 WHERE timestamp BETWEEN {{.start_datetime}} AND {{.end_datetime}} GROUP BY (`service.name`) order by (`service.name`) ASC;"
            }
        }
    }
}
