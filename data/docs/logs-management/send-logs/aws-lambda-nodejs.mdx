...
Below is an `index.js` file of a [sample app](https://github.com/SigNoz/nodejs-lambda). Here we are creating a span, emitting a log line and then flushing the LoggerProvider. 
Note that it is important to flush the LoggerProvider to not miss any log lines.
