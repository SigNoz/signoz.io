...
```java:src/main/java/com/example/Application.java defaultCollapsed minimap
package com.example;
...
- `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: The specific endpoint to send logs to (e.g., `https://ingest.<region>.signoz.cloud:443/v1/logs`). Overrides `OTEL_EXPORTER_OTLP_ENDPOINT`.
- `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL`: The transport protocol. Options include `http/protobuf` (default in Java agent 2.x), `grpc`, and `http/json`.
