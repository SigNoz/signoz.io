...
title: Send logs from Convex to SigNoz using log streaming
description: Send Convex logs to SigNoz using Convex log streaming webhooks and a Webhook Bridge.
doc_type: howto
---

This guide shows you how to send logs from a Convex backend to SigNoz using **Convex log streaming** and a **Webhook Bridge**.

Convex log streaming sends logs as JSON or JSONL webhooks with HMAC-SHA256 signatures. SigNoz ingests logs via its JSON logs endpoint. You'll need deploy a small Express service that:
...
Most steps are identical. To adapt this guide, update the endpoint and remove the ingestion key header as shown in <a href="https://signoz.io/docs/ingestion/cloud-vs-self-hosted/#cloud-to-self-hosted" target="_blank" rel="noopener noreferrer nofollow">Cloud → Self-Hosted</a>.
</KeyPointCallout>
...
async function sendToSigNoz(logs) {
  ...
  if (!region || !ingestionKey) {
    console.warn("Skipping SigNoz export: Missing SIGNOZ_REGION or SIGNOZ_INGESTION_KEY");
    return;
  }
  ...
  const transformedLogs = logs.map((log) => {
    ...
    // Set resource attributes (service.name for SigNoz grouping)
    newLog.resources = {
...
|----------|-------------|
| `WEBHOOK_SECRET` | The HMAC secret from your Convex dashboard |
| `SIGNOZ_REGION` | Your SigNoz Cloud [region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint) (e.g., `us`, `eu`, `in`) |
| `SIGNOZ_INGESTION_KEY` | Your SigNoz [ingestion key](https://signoz.io/docs/ingestion/signoz-cloud/keys/) |
| `SERVICE_NAME` | (Optional) Service name for logs in SigNoz, defaults to `convex-logs` |
| `PORT` | (Optional) Server port, defaults to `3080` |

## Step 3: Run Locally

Start the bridge with environment variables:

```bash
WEBHOOK_SECRET=<your-convex-secret> \
...
## Step 4: Configure Convex Log Streaming
...
- `SIGNOZ_REGION` or `SIGNOZ_INGESTION_KEY` is incorrect
...
- Explore the [Logs Explorer](https://signoz.io/docs/logs-management/) to query and filter Convex logs
- Create [alerts](https://signoz.io/docs/alerts/) on Convex error logs
- Build a [dashboard](https://signoz.io/docs/dashboards/) to monitor Convex function performance