...
After saving the destination, verify that logs are being sent to SigNoz:

1. **Open SigNoz Logs**: Navigate to the `Logs` tab in your SigNoz dashboard.
2. **Generate Supabase activity**: Trigger activity in your Supabase project, such as API requests, authentication flows, Postgres queries, Storage access, Realtime usage, or Edge Function executions.
3. **View logs**: Within a few minutes, new Supabase logs should appear in SigNoz.
...
- **Query logs**: Use the [Logs Explorer](https://signoz.io/docs/userguide/logs/) to search and filter Supabase logs
- **Create alerts**: Set up alerts for repeated errors or unusual log patterns
