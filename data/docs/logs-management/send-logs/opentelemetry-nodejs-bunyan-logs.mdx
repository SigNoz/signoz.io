...
```bash
export OTEL_EXPORTER_OTLP_ENDPOINT="https://ingest.<region>.signoz.cloud:443/v1/logs"
...
env:
  - name: OTEL_EXPORTER_OTLP_ENDPOINT
    value: 'http://<otel-collector-service>.<namespace>.svc.cluster.local:4318/v1/logs'
...
```dockerfile
ENV OTEL_EXPORTER_OTLP_ENDPOINT="https://ingest.<region>.signoz.cloud:443/v1/logs"
...
```bash
docker run \
  -e OTEL_EXPORTER_OTLP_ENDPOINT="https://ingest.<region>.signoz.cloud:443/v1/logs" \
  -e OTEL_EXPORTER_OTLP_HEADERS="signoz-ingestion-key=<SIGNOZ_INGESTION_KEY>" \
  -e OTEL_SERVICE_NAME="nodejs-bunyan-logger" \
  ...
```
