...
Before any user-defined pipeline runs, SigNoz applies a **default Pipeline** to every incoming log entry. This step ensures the body is always a JSON and that `message` field is always String, if exists. Your Logs Pipelines always receive an already-processed entry.
...
|-----------|--------------|
| **String** | Attempt to parse as a JSON object (`{…}`). If valid, the parsed map is used. else, the string is wrapped: `{"message": "<original string>"}` |
...
**Synonym promotion** — if `message` is absent or `null`, SigNoz looks for certain fields that are eligible to be promoted as the log message. The first eligible field found is moved to `message`; the original field is deleted.
...
## Filtering on JSON body keys

Multi-type values for the same key are supported. A key that holds a string in one log and a number in another can be filtered with either type.
