...
In order to scrub the relevant data, it is important to examine the existing log attributes. Below is a sample log with different attributes.
...
OpenTelemetry Collector offers several processors to filter and modify sensitive data before it reaches SigNoz:

1. `transform`: Change values within spans
2. `attributes`: Modify individual attributes
3. `redaction`: Mask or block attribute values
...
To demonstrate what each processor does, we will use the sample log shown below
