...
- **[Search Logs](/docs/logs-management/logs-api/search-logs)** — filter logs by attributes, body content, severity, or trace ID with pagination support.
- **[Aggregate Logs](/docs/logs-management/logs-api/aggregate-logs)** — compute counts, averages, and group-by operations over filtered log data.
- **[Logs URL for Explorer](/docs/logs-management/logs-api/logs-url-for-explorer-page)** — generate a direct URL to the Logs Explorer with predefined filters and time range.
- **[Payload Model](/docs/logs-management/logs-api/payload-model)** — request and response schema reference.
