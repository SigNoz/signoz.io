...
Set environment variables to point to your SigNoz Cloud or self-hosted endpoint.
...
For Kubernetes deployments, environment variables are configured via ConfigMaps and Secrets. See Step 6 for the complete Kubernetes deployment manifests.

You can set these variables in your deployment YAML:
```yaml
env:
...
For Docker deployments, environment variables are configured in your `docker-compose.yml` or `Dockerfile`. See Step 6 for the complete Docker setup.
...
### Step 5: Instrument the Temporal Worker
...
**Start the worker:**
Make sure you exported the environment variables from Step 1 in your terminal.
```bash
npm run start
```

**Start the client (in another terminal):**
Make sure to export environment variables in this new terminal too. You may want to set `OTEL_SERVICE_NAME` to `temporal-client` so it shows up distinctly in SigNoz.
```bash
npm run workflow
```
