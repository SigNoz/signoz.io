...
meta_title: OpenTelemetry for Temporal (TypeScript)
...
```json:package.json defaultCollapsed
{
  "dependencies": {
...
```typescript:src/instrumentation.ts minimap defaultCollapsed
...
```typescript:src/worker.ts {1,12-24} minimap defaultCollapsed
...
```typescript:src/client.ts {1} defaultCollapsed minimap
...
```yaml:temporal-worker-deployment.yaml defaultCollapsed minimap
apiVersion: apps/v1
kind: Deployment
metadata:
...
```yaml:temporal-client-job.yaml defaultCollapsed minimap
apiVersion: batch/v1
kind: Job
metadata:
...
```typescript:src/logger.ts defaultCollapsed
...
- **Explore deeper concepts**: For a conceptual look beyond this setup, see our writeup on [observability for Temporal workflows](https://signoz.io/blog/deep-temporal-observability/).
