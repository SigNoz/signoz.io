...
### Step 7: Configure Environment and Run

**Verify these values:**
- `<region>`: Your SigNoz Cloud [region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint).
- `<your-ingestion-key>`: Your [ingestion key](https://signoz.io/docs/ingestion/signoz-cloud/keys/).

Choose your deployment environment and follow the corresponding instructions:
...
Set environment variables and start your worker and client. You must `export` the environment variables before running the application.
...
kubectl create secret generic signoz-secrets \
  --from-literal=signoz-ingestion-key=<your-ingestion-key>
```
...
```yaml:temporal-client-deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
kubectl apply -f temporal-client-deployment.yaml
```
...
On Windows, set environment variables using PowerShell or Command Prompt.
...
**In a separate PowerShell window, start the client:**
...
```cmd
:: Start the worker
set OTEL_SERVICE_NAME=temporal-worker
