...
import { setupOtelSdk } from './instrumentation';
...
**Cause**: Temporal generates metrics with many unique label combinations.

**Fix**: Adjust the log level filter in `Runtime.install()`:
