...
The `rabbitmq` receiver reads the monitoring password from a `RABBITMQ_PASSWORD` environment variable, so it never lives in the config file. Every environment below exports to SigNoz Cloud with the same `otlp_http` exporter, so add it once if you do not have one:
...
### Step 2: Configure the file_log receiver
...
```yaml:config.yaml
receivers:
  # On Collector releases before v0.151.0 use the filelog alias
  file_log/rabbitmq:
...
service:
  pipelines:
    logs/rabbitmq:
      receivers: [file_log/rabbitmq]
      processors: [batch]
      exporters: [otlp_http]
```

Set `<rabbitmq-log-path>` to the path from Step 1. If you do not have an `otlp_http` exporter yet, add the one shown in the **Metrics** tab.
...
<Admonition type="info" title="RabbitMQ logs only to journald?" defaultCollapsed="true">
  If your node logs only to the systemd journal instead of a file, skip the `file_log/rabbitmq` receiver. Use the [collect systemd logs guide](https://signoz.io/docs/logs-management/send-logs/collect-systemd-logs/) and filter the receiver to the `rabbitmq-server.service` unit.
</Admonition>
...
<Admonition type="warning">
  Use one log path per RabbitMQ instance. If you collect container stdout, do not also add the `file_log/rabbitmq` receiver, or logs arrive twice.
</Admonition>

</TabItem>
<TabItem value="k8s" label="Kubernetes">

The SigNoz `k8s-infra` chart [collects pod logs](https://signoz.io/docs/userguide/collect_kubernetes_pod_logs/) by default, so RabbitMQ pod logs reach SigNoz with no extra receiver. Filter them in SigNoz by the RabbitMQ pod or namespace. Do not add the VM `file_log/rabbitmq` receiver when pod log collection is already active.
...
Use one log collection path per RabbitMQ instance. Disable `file_log/rabbitmq` if Docker or Kubernetes log collection already reads the container's stdout.
