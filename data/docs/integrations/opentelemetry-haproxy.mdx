...
```yaml:haproxy-metrics-collection-config.yaml defaultCollapsed minimap
receivers:
  haproxy:
...
```yaml:docker-compose.yml defaultCollapsed
services:
  haproxy:
    image: haproxy:3.0
    volumes:
...
```yaml:haproxy-deployment.yaml defaultCollapsed
spec:
  containers:
    - name: haproxy
      image: haproxy:3.0
...
```yaml:haproxy-logs-collection-config.yaml minimap defaultCollapsed
receivers:
  syslog:
    udp:
...
<Admonition type="info" title="Using log stdout instead?" defaultCollapsed="true">
  HAProxy can write logs to stdout with `log stdout format raw daemon`. If you run the [SigNoz k8s-infra Helm chart](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/), it already collects pod stdout, so those lines reach SigNoz without a syslog receiver. Parse them with a [Logs Pipeline](https://signoz.io/docs/logs-pipelines/introduction/) using the same regex as Step 2.
</Admonition>
...
```yaml:haproxy-collection-config.yaml minimap defaultCollapsed
receivers:
  haproxy:
    endpoint: "${env:HAPROXY_STATS_ENDPOINT}"
    collection_interval: 30s
