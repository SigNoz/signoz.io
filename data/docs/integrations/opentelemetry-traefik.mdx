...
Traefik writes two kinds of logs:

- **Access logs:** one record per HTTP request, with the router, backend service, status code, and timings. This guide collects these.
- **Application logs:** Traefik's own startup and configuration events, controlled by the separate `log` option.
...
Traefik writes the access log itself. The Collector tails that file, parses each JSON line into fields, and forwards the result to SigNoz.
...
Traefik writes to stdout when you omit `filepath`. Restart Traefik to apply the change.
...
service:
  pipelines:
    logs/traefik:
      receivers: [file_log/traefik-access]
      processors: [resource/traefik, batch]
      exporters: [otlp_http/traefik]
```
...
Traefik writes access logs to stdout when you omit `filepath`. If you run the [SigNoz k8s-infra Helm chart](https://signoz.io/docs/install/kubernetes/), it already collects pod stdout, so those lines reach SigNoz without a file log receiver.
