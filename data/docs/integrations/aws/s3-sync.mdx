...
description: Sync logs from S3 buckets to SigNoz using One-Click AWS Integration.
doc_type: howto
---
...
your Amazon S3 buckets into SigNoz. 
...
services whose logs are stored in S3 buckets.

## Prerequisites
Before setting up the AWS S3 Sync integration, ensure the following:
- **AWS Account Access**: Administrative access to your AWS account with permissions to create IAM roles, Lambda functions, and other required resources.
...
- For more details on the general AWS integration setup, see [One-Click AWS Integrations](https://signoz.io/docs/integrations/aws/one-click-aws-integrations/).
...
To verify that your logs are being synced correctly:

1.  Navigate to **Logs** → **Logs Explorer** in the SigNoz UI.
2.  Search for logs using the relevant S3 bucket attributes.
3.  Confirm that new logs appear as files are added to your S3 bucket.

## Next Steps

- [Create Alerts](https://signoz.io/docs/alerts/) for your logs to monitor for specific error patterns or system events.
- Build [Dashboards](https://signoz.io/docs/userguide/manage-dashboards/) to visualize log trends and patterns.
- Explore [Log Pipelines](https://signoz.io/docs/logs-pipelines/introduction/) to process and enrich your log data.