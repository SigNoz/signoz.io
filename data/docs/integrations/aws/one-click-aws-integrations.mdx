...
- **Persistent ECS Task**: The integration deploys an always-running ECS task in your AWS account. This task must remain running to auto-discover new resources and maintain integration state.
...
The integration deploys an **AWS Integration Agent** as a continuously running ECS task in your chosen region. This agent must remain running to support auto-discovery and maintain integration state. It:
- Scans for selected services across enabled regions.
- Automatically discovers new resources as they are provisioned.
