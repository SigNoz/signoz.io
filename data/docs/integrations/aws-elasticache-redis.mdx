...
description: Collect AWS ElastiCache Redis metrics and logs using OpenTelemetry Collector and send them to SigNoz for monitoring and analysis.
doc_type: howto
---
## Overview

This integration helps you monitor key AWS Elasticache (Redis) metrics and logs, view them with an out-of-the-box dashboard, and parse Redis logs for better querying and aggregation.

<Admonition type="tip">
**SigNoz Cloud users**: You can also use the [One-Click AWS Integration](https://signoz.io/docs/integrations/aws/elasticache/) for automated setup with pre-built dashboards.
</Admonition>
...
3. An OpenTelemetry (OTEL) Collector (v0.88.0+):
   - [Install the OTEL Collector](https://signoz.io/docs/opentelemetry-collection-agents/vm/install/) (v0.88.0+) if not already done
...
Verify these values:
- `<redis-server-endpoint>`: The hostname and port of your ElastiCache Redis instance (e.g., `my-redis.xxxx.cache.amazonaws.com:6379`)
- `<PASSWORD>`: Your Redis AUTH password (if configured)
...
Use the <a href="https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/Log_Delivery.html" target="_blank" rel="noopener noreferrer nofollow">log delivery</a> instructions to send Redis logs to CloudWatch Logs.

The log collection of ElastiCache instance requires specifying the list of log group names. From the AWS CloudWatch console, please find the log group(s) relevant to the integration.
...
<figure data-zoomable align='center'>
    <img src="/img/docs/integrations/aws-elasticache-redis/aws-elasticache-redis-integration-listening.webp" alt="Listening for data from AWS Elasticache Redis"/>
...
<Figure 
  src="/img/docs/integrations/aws-elasticache-redis/elasticache-overview-dashboard.webp" 
  alt="Dashboard for monitoring AWS Elasticache (redis) Metrics" 
  caption="Dashboards for monitoring AWS Elasticache (redis) Metrics"
