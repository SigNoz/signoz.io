...
description: Collect AWS RDS MySQL metrics and logs using OpenTelemetry Collector and send them to SigNoz for monitoring and analysis.
doc_type: howto
---
## Overview
...
<Admonition type="tip">
**SigNoz Cloud users**: You can also use the [One-Click AWS Integration](https://signoz.io/docs/integrations/aws/rds/) for automated setup with pre-built dashboards.
</Admonition>

<Admonition type="info" title="Running MySQL yourself instead of on RDS?" defaultCollapsed="true">
See [Monitor MySQL logs](https://signoz.io/docs/integrations/opentelemetry-mysql/) and [MySQL metrics](https://signoz.io/docs/metrics-management/mysql-metrics/).
</Admonition>

<Admonition type="info" title="Running Aurora MySQL instead of RDS MySQL?" defaultCollapsed="true">
Aurora reports metrics for the cluster as well as for each instance. The configuration here reads instance metrics only, and `FreeStorageSpace` and `ReplicaLag` return no data on Aurora. Follow [Monitor Amazon Aurora](https://signoz.io/docs/integrations/aws-rds-aurora/) instead.
</Admonition>
...
Save the following config for collecting AWS RDS MySQL metrics in a file named `aws-rds-mysql-metrics.yaml` and update the region key with relevant value.
...
Verify these values:
- `<mysql-server-endpoint>`: The hostname and port of your MySQL RDS instance (e.g., `mydb.xxxx.us-east-1.rds.amazonaws.com:3306`)
...
<figure data-zoomable align='center'>
    <img src="/img/docs/integrations/aws-rds-mysql/aws-rds-mysql-integration-listening.webp" alt="Listening for data from AWS RDS MySQL"/>
