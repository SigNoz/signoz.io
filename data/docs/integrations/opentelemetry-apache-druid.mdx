...
3. Search for `druid.broker.query.time` or `druid.broker.jvm.mem.used.heap` to confirm metrics flow.
...
- Set up [alerts](https://signoz.io/docs/userguide/alerts-management/) on `druid.<node>.ingest.kafka.lag` or coordinator segment queue metrics.
