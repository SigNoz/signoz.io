...
```yaml:druid-metrics-collection-config.yaml defaultCollapsed
receivers:
  statsd:
...
If you deploy via the [SigNoz k8s-infra Helm chart](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/), apply a values override to enable StatsD ingestion:
