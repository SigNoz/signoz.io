...
Kong pushes all metrics over OTLP on the `push_interval` you configure (default: 10 seconds). The `enable_bandwidth_metrics`, `enable_latency_metrics`, `enable_request_metrics`, and `enable_upstream_health_metrics` flags in the plugin config control which metric groups Kong collects.
...
1. Go to **Services** section in SigNoz to see your Kong service listed.
2. Navigate to **Traces** section to browse distributed traces matching your defined routes. 
3. Use **Metrics Explorer** and query `kong.latency.total` or `kong.nginx.connection.count`.
4. Use **Logs** and filter by `service.name` to read access logs.
