...
```yaml:apache-metrics-collection-config.yaml defaultCollapsed
receivers:
  apache:
...
If you run the [SigNoz k8s-infra Helm chart](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/), add the `apache` receiver through a values override instead of the standalone config file from Step 2. Point the endpoint at the Apache Service DNS:
...
```yaml:apache-logs-collection-config.yaml minimap defaultCollapsed
receivers:
...
The official Apache image writes access logs to stdout and error logs to stderr. If you run the [SigNoz k8s-infra Helm chart](https://signoz.io/docs/opentelemetry-collection-agents/k8s/k8s-infra/install-k8s-infra/), it already collects pod stdout and stderr, so Apache logs reach SigNoz without a file log receiver.
