...
3. Select **API Key** for authentication and set the custom header name to `signoz-ingestion-key`, and enter your SigNoz [ingestion key](https://signoz.io/docs/ingestion/signoz-cloud/keys/) from your SigNoz Cloud account. You can find this under **Settings > Ingestion Settings** in SigNoz.
...
- To check for metrics, go to the **Dashboards** > **Metrics Explorer** and look for metrics prefixed with `libpq` or metrics carrying your configured `service.name` related to Postgres.
