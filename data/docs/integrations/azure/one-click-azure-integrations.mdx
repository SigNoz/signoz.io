...
description: Monitor Azure compute, storage, networking, and database services with SigNoz One-Click Azure Integration. Connect your subscription and start monitoring.
doc_type: howto
tags: [SigNoz Cloud]
---
...
In SigNoz, connecting, updating, or disconnecting a cloud account requires the **Admin** role. Editors and Viewers can view connected accounts and their services. See the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/#cloud-integration) for the exact scopes.
</Admonition>

## Supported integrations

<Listicle name="azure-one-click" />
...
The SigNoz agent container app does not update automatically. To update it to the latest version:

1. Check each service's documentation page for its minimum required agent version (listed at the top).
2. In the Azure Portal, go to **Subscriptions** and select your subscription.
3. Navigate to **Resource Groups** and open **signoz-integration-rg**.
4. Select the **signoz-integration-agent** container app.
5. In the left menu, under **Application**, choose **Containers**.
6. In the **Properties** section, click **Edit** on the container.
7. Update only the version tag in the **Image and tag** field to the latest version shown in the SigNoz Azure Integration control panel, then save.
8. Verify the container app restarts with the new version by checking the **Revisions** view for a new active revision.
...
- **Supported Azure services**: Azure CDN FrontDoor, Azure Blob Storage, Azure Virtual Machines, Azure Container Apps, Azure App Services, Azure Kubernetes Service (AKS), and the database services — Azure SQL Database, Azure SQL Managed Instance, Azure Database for MySQL Flexible Server, Azure Database for PostgreSQL Flexible Server, Azure MongoDB (vCore), Azure Cosmos DB, Azure Managed Instance for Apache Cassandra, and Azure Cache for Redis. More services will be added in future releases.
