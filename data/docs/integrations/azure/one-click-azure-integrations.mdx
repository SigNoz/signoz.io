...
- One of the following Azure roles on the subscription:
...
az stack sub create \
  ...
  --parameters signozApiUrl=<ingestion-url> signozIngestionKey=<api-key> resourceGroups=<resource-groups> \
...
New-AzSubscriptionDeploymentStack `
  ...
  -TemplateParameterObject @{
    signozApiUrl = "<ingestion-url>"
    signozIngestionKey = "<api-key>"
...
- `<azure-region>`: Azure region to deploy agent resources (e.g., `eastus`)
- `<template-url>`: template URL from the generated command
- `<ingestion-url>`: your SigNoz ingestion endpoint
- `<api-key>`: your SigNoz ingestion key
- `<resource-groups>`: comma-separated resource group names to monitor
...
3. Confirm both stacks show a **Succeeded** status:
   ...
   - `signoz-integration-telemetry`: deploys telemetry resources (Event Hubs and diagnostic settings)
