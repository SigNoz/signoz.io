...
<Admonition type="info" title="Using self-hosted SigNoz?" defaultCollapsed={true}>
  Self-hosted setup differs only in the OTLP endpoint and the removed ingestion-key header. See [Cloud → Self-Hosted](https://signoz.io/docs/ingestion/cloud-vs-self-hosted/#cloud-to-self-hosted).
</Admonition>
...
- For logs: an [OpenTelemetry Collector](https://signoz.io/docs/opentelemetry-collection-agents/get-started/) (v0.88.0 or newer) that can read the Cassandra log files
...
Setting `target_system` to `cassandra,jvm` collects both Cassandra request and compaction metrics and the underlying JVM metrics (heap, threads, GC). Set `<cassandra-host>` to the address or service name the scraper uses to reach Cassandra's JMX port
...
Run Cassandra with `-Djava.rmi.server.hostname` set to the pod IP (via the downward API) so the scraper can follow the RMI stub to the Cassandra pod. Apply the manifests:
...
Cassandra writes its operational logs to `system.log`. The OpenTelemetry Collector filelog receiver tails this file and forwards the entries to SigNoz.
...
- **Java**: the OpenTelemetry Java agent auto-instruments the DataStax and Cassandra drivers. See [OpenTelemetry Java](https://signoz.io/docs/instrumentation/opentelemetry-java/).
...
- **Node.js**: instrument the `cassandra-driver` with [OpenTelemetry JavaScript](https://signoz.io/docs/instrumentation/opentelemetry-javascript/).
- **Go**: instrument `gocql` with [OpenTelemetry Go](https://signoz.io/docs/instrumentation/opentelemetry-go/).
...
2. **Metrics:** Open **Metrics** in SigNoz and search for metrics starting with `cassandra_` (for example `cassandra_client_request_read_latency_99p`) and `jvm_`.
...
**JMX scraper logs `Connection refused`, or no `cassandra_*` metrics appear**
