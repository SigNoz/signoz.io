...
```yaml:cassandra-scraper.yaml minimap defaultCollapsed
apiVersion: v1
kind: ConfigMap
metadata:
  name: scraper-config
...
```yaml:cassandra-metrics-collection-config.yaml defaultCollapsed
receivers:
  jmx/cassandra:
...
```yaml:cassandra-logs-collection-config.yaml minimap defaultCollapsed
receivers:
  filelog/cassandra:
...
service:
  pipelines:
    logs/cassandra:
      ...
      exporters: [otlphttp/cassandra-logs]
```
