...
meta_title: AWS VPC Flow Logs Monitoring - Network Traffic
description: Learn to forward VPC Flow Logs from S3 to SigNoz using Lambda for deep network traffic visibility, security analysis, and troubleshooting.
doc_type: howto
---

## Overview

This documentation provides a detailed walkthrough on how to set up an AWS Lambda function to collect Virtual Private Cloud (VPC) Flow Logs stored in an AWS S3 bucket and forward them to SigNoz. By following this guide, you can automatically send your VPC logs to SigNoz, enabling you to visualize and monitor your network traffic patterns and security events.
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-create_bucket_2.webp" alt="S3 bucket creation form with bucket name and region fields"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image"  src="/img/docs/elb/elb-logs-lambda_1.webp" alt="AWS Lambda Console showing Create function button"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/create_function.webp" alt="Lambda function creation form with Author from scratch option selected"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_3.webp" alt="Lambda runtime and permissions configuration options"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/lambda_function.webp" alt="Newly created Lambda function overview page"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/execution_role.webp" alt="Lambda function Configuration tab showing Permissions section"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/add_policy.webp" alt="IAM page showing Add permissions dropdown with Attach policies option"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_7.webp" alt="IAM Attach policies page with AmazonS3FullAccess policy selected"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-policies-used.webp" alt="List of IAM policies required for Lambda function execution"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/add_trigger.webp" alt="Lambda function console showing Add trigger button"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/vpc-docs/add_trigger_2.webp" alt="S3 trigger configuration showing event type selection dropdown"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-layer_3.webp" alt="AWS Lambda Layers page with Create Layer button"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-layer_4.webp" alt="Layer configuration page with upload zip file option"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-layer_5.webp" alt="Lambda function Layers section with Add a layer button"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-layer_6.webp" alt="Add layer dialog showing Custom layers option and layer selection dropdown"/>
...
```python:lambda_function.py minimap defaultCollapsed
import json
import gzip
import boto3 
import requests
...
- `<name_of_your_bucket>`: The name of your S3 bucket containing VPC Flow Logs.
...
```python:local_test.py defaultCollapsed minimap
import os
import gzip
import json
import requests
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-timeout_increase.webp" alt="Lambda function General Configuration showing timeout setting"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_test_event.webp" alt="Lambda test event configuration dialog with S3 PUT event template"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_10.webp" alt="Lambda function code editor showing the VPC log forwarding code"/>
...
<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_output.webp" alt="Lambda function execution output showing successful log transmission"/>
...
To verify that your VPC Flow Logs are being sent correctly to SigNoz:

1. Navigate to **Logs** > **Logs Explorer** in SigNoz.
2. You should see your VPC logs appearing in the logs list.
3. Click on any log line to view its detailed information and verify the parsed fields like `srcaddr`, `dstaddr`, `action`, etc.

<figure data-zoomable align='center'>
    <img className="box-shadowed-image" src="/img/docs/elb/elb-logs-lambda_12.webp" alt="SigNoz Logs Explorer showing VPC Flow Logs with parsed network traffic fields"/>
    <figcaption><i>A sample VPC Flow Log line in SigNoz Logs Explorer</i></figcaption>
</figure>

## Next Steps

- [Create alerts](https://signoz.io/docs/alerts/) based on your VPC logs to monitor for security events or unusual traffic patterns
- [Build dashboards](https://signoz.io/docs/userguide/manage-dashboards/) to visualize your network traffic flows and security metrics
- Explore [Log Pipelines](https://signoz.io/docs/logs-pipelines/introduction/) to further process and enrich your VPC Flow Logs
