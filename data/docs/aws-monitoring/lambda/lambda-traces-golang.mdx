...
```go:tracing.go minimap defaultCollapsed
package main
...
```go:main.go minimap defaultCollapsed
package main
...
```yaml:collector.yaml defaultCollapsed
receivers:
  otlp:
    protocols:
      http:
...
- [Monitor AWS Lambda MicroVMs](https://signoz.io/docs/aws-monitoring/lambda-microvms/) if you run Firecracker sandboxes rather than functions.
