...
Set these two environment variables on the function:
...
Then remove `OTEL_EXPORTER_OTLP_HEADERS`, because the collector now holds the ingestion key. Keep `OTEL_SERVICE_NAME` and `OTEL_RESOURCE_ATTRIBUTES` on the function.
