...
meta_title: AWS Lambda Logs via S3 - Forwarder Setup Guide
description: Learn to forward Lambda logs to SigNoz using S3 and a Lambda function for scalable serverless log collection and analysis.
doc_type: howto
---
...
See the <a href="https://aws.amazon.com/blogs/compute/using-aws-lambda-extensions-to-send-logs-to-custom-destinations/" target="_blank" rel="noopener noreferrer nofollow">AWS Lambda extensions blog post</a> to know how to automatically collect Lambda logs in S3.
</Admonition>
...
<Admonition type="info">
  For the scope of this documentation, we assume that all the data in S3 bucket is in the same
  format. For example, if one file is in `.csv` format (say), then all files within the bucket will
  be in `.csv` format. For files in different formats, you will have to use different parsing
  functions for each format or update the existing function accordingly.
</Admonition>
...
<Figure
  src="/img/docs/lambda-docs/create_function.webp"
  alt="Choose the Author from scratch and fill up other details"
...
<Figure
  ...
  alt="Choose Create a new role here"
...
<Figure
  src="/img/docs/lambda-docs/lambda_logs_page.webp"
  alt="Your barebones Lambda function is created now"
...
<Figure
  src="/img/docs/lambda-docs/execution_role.webp"
  alt="Choosing execution role from Configurations tab"
...
<Figure
  src="/img/docs/lambda-docs/lamba_roles.webp"
  alt="Attach policies to your Lambda function"
...
<Figure
  ...
  alt="Policies you'll need to run your Lambda function"
...
<Figure
  ...
  alt="Extra policies you might require to execute your Lambda function"
...
<Figure
  src="/img/docs/lambda-docs/add_trigger.webp"
  alt="Add a trigger to the function."
...
<Figure
  src="/img/docs/lambda-docs/add_trigger_2.webp"
  alt="Choose event types from the drop down menu"
...
<Figure
  ...
  alt="Uploading the requirements.zip file to later add it as a layer "
...
<Figure
  ...
  alt="Add a layer to your Lambda function"
...
```python defaultCollapsed minimap
import boto3
import requests
...
Replace `<region>` in the URL with your [SigNoz Cloud region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint). You can find ingestion details (REGION and SIGNOZ_INGESTION_KEY) in the SigNoz dashboard.
...
<Figure
  ...
  alt="Configuring execution timeout of Lambda function"
...
<Figure
  ...
  alt="Visual representation of code output of Lambda function (SAMPLE ONLY)"
  caption="Visual representation of code output of Lambda function (SAMPLE ONLY)"
  className="box-shadowed-image"
/>
...
<Figure
  ...
  alt="A sample log line of the logs sent from AWS Lambda"
  caption="A sample log line of the logs sent from AWS Lambda (SAMPLE ONLY)"
  className="box-shadowed-image"
/>

## Related Resources

- [Collect AWS Lambda Logs via OTel Collector Extension Layer](https://signoz.io/docs/aws-monitoring/lambda/lambda-logs-otel-extension/)
- [One-Click AWS Integration for Lambda](https://signoz.io/docs/integrations/aws/lambda/)
- [Collect Lambda Metrics](https://signoz.io/docs/aws-monitoring/lambda/lambda-metrics/)
