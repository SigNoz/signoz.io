...
meta_title: Monitor Celery - Auto & Manual Instrumentation Setup
description: Learn how to monitor Celery tasks and workers in SigNoz using OpenTelemetry. Set up auto-instrumentation with environment variables and manual instrumentation for custom metrics.
...
<a href="https://docs.celeryq.dev/en/stable/index.html" rel="noopener noreferrer nofollow">Celery</a> is a distributed task queue, written in Python, that allows 
...
Install and configure OpenTelemetry for scraping the metrics from Celery. Follow [OpenTelemetry Binary Usage in Virtual Machine](https://signoz.io/docs/opentelemetry-collection-agents/vm/install/) guide for detailed instructions.
...
```python:instrumentation.py defaultCollapsed minimap
...
We are using Redis Queue in this example. However, you can choose any of the <a href="https://docs.celeryq.dev/en/main/getting-started/backends-and-brokers/index.html#broker-overview" rel="noopener noreferrer nofollow">supported brokers </a> for the Celery setup.
</Admonition>
...
For more information on instrumenting Celery worker applications, you can refer [this](https://signoz.io/docs/instrumentation/opentelemetry-python/) page.
</Admonition>

### Step 5: Enabling flower metrics collection (Optional)

While using celery, you can optionally enable flower metrics collection. You can read about flower in the <a href="https://flower.readthedocs.io/en/latest/" rel="noopener noreferrer nofollow">official docs </a>.
...
<Figure
  ...
  caption="Celery Dashboard"
/>

<Figure
  src="/img/docs/messaging-queues/celery/metrics-dashboard-2.webp"
  alt="Celery Dashboard (continued)"
  caption="Celery Dashboard (continued)"
/>

<Figure
  src="/img/docs/messaging-queues/celery/celery-traces-panel.webp"
  alt="Celery Dashboard (continued)"
  caption="Celery Traces Panel"
/>

<Figure
  src="/img/docs/messaging-queues/celery/celery-traces-detailed.webp"
  alt="Celery Dashboard (continued)"
  caption="Detailed Celery Traces"
/>

## Troubleshooting

### Worker Online Status Shows 0

If your Celery dashboard shows "Worker Online: 0" even when workers are running, this is likely because task events are disabled by default. The "Worker Online" status relies on the `flower_worker_online` metric from Flower, which requires the `worker_send_task_events` setting to be enabled.

To fix this, start your Celery worker with the `-E` flag to enable task events:

```bash
celery -A your_app worker --loglevel=info -E
```
...
Once enabled, the dashboard will properly track and display worker online status.

For more information, refer to the <a href="https://docs.celeryq.dev/en/latest/userguide/configuration.html#worker-send-task-events" rel="noopener noreferrer nofollow"> Celery configuration documentation </a> and <a href="https://docs.celeryq.dev/en/stable/userguide/monitoring.html#worker-online" rel="noopener noreferrer nofollow"> monitoring guide </a>.
