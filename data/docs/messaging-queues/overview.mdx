...
description: Use managed SigNoz Cloud to correlate Kafka and Celery metrics with OpenTelemetry traces. Self-Hosted SigNoz remains user-operated.
---

SigNoz Cloud is the managed path for using **Messaging Queues**. SigNoz operates platform storage, scaling, upgrades, security, and availability. With Self-Hosted SigNoz, your team operates those parts of the platform in its own environment.

The **Messaging Queues feature** provides visibility into distributed systems built on **Kafka** or **Celery**. It correlates native broker or queue metrics with traces through <a href="https://opentelemetry.io/docs/specs/semconv/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry semantic conventions</a>.
