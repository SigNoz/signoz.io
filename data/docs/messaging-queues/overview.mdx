...
description: Correlate Kafka and Celery metrics with OpenTelemetry traces using SigNoz Cloud, or run the platform on your infrastructure with Self-Hosted SigNoz.
---

Monitor messaging queues with SigNoz Cloud, or deploy [Self-Hosted SigNoz](https://signoz.io/docs/install/self-host/) on your infrastructure.

The **Messaging Queues feature** in SigNoz provides visibility into distributed systems built on **Kafka** or **Celery**. While other tools focus only on native broker or queue metrics, SigNoz correlates those metrics with traces in real time through <a href="https://opentelemetry.io/docs/specs/semconv/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry semantic conventions</a>.
