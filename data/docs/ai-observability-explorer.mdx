...
The Explorer is part of the AI Observability section, which is available to every user and labeled **Beta** in the side navigation. See the [AI Observability overview](https://signoz.io/docs/ai-observability/#prerequisites) for prerequisites.
</Admonition>
...
The Explorer ships with seven pre-configured quick filter fields, provisioned automatically for every organization:
...
The Trace view groups spans into traces and shows one row per root span. Two header controls shape the table:

- **Order by**: sort the traces by any AI trace field. The default is **Last Activity Time** (descending). You can switch to fields such as token counts, estimated cost, or call counts.
- **Options**: open the column picker to show, hide, and reorder columns. Your selection is saved in the browser and persists across sessions.

The **Trace ID** column is always visible and cannot be hidden or moved, since it links each row to its trace. Every other column is optional.

When you have not customized the picker, the table shows: Service Name, Root Span Name, Trace Duration, Span Count, Trace ID, LLM Call Count, Total Tokens, and Estimated Total Cost.

The column picker exposes the AI-specific fields SigNoz fetches for your trace data, so the exact list depends on what your account has instrumented. Commonly available columns include:

- **Service Name**
- **Root Span Name**: name of the trace's root span.
- **Trace Duration**: end-to-end duration of the full trace.
- **Span Count**
- **Start Time** and **End Time**
- **Error Count**
- **Input** and **Output**: the root span's input and output payloads.
- **LLM Call Count**: number of LLM calls in the trace.
- **Tool Call Count** and **Distinct Tool Count**
- **Input Tokens**, **Output Tokens**, and **Total Tokens**
- **Estimated Total Cost**
- **Max LLM Duration**: duration of the slowest LLM call in the trace.
