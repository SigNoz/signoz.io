...
```yaml:config.yaml {12} defaultCollapsed
receivers:
  otlp:
    protocols:
      grpc:
...
Use the [hostmetrics](https://signoz.io/docs/infrastructure-monitoring/user-guides/hostmetrics/) receiver to scrape CPU, memory, disk, filesystem, load, and network metrics from the VM.

```yaml:config.yaml {20} defaultCollapsed
receivers:
  hostmetrics:
...
If the config is valid, the command exits without errors. Then apply the changes and restart the collector. Confirm data in [Traces](https://signoz.io/docs/traces-management/guides/overview/), [Metrics](https://signoz.io/docs/metrics-management/overview/), or [Logs](https://signoz.io/docs/logs-management/overview/) (and [Infrastructure](https://signoz.io/docs/infrastructure-monitoring/overview/) for host metrics) depending on what you enabled.
