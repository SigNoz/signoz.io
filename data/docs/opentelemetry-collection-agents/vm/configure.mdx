...
You can attach custom resource attributes (for example, `environment`, `team`, or any custom key) to **all telemetry** sent by the collector by setting the [`OTEL_RESOURCE_ATTRIBUTES` environment variable](https://signoz.io/docs/logs-management/guides/set-resource-attributes-for-logs/#using-environment-variables) in the collector’s environment.
...
- **Config and logs:** Use the validation steps for your [installation method](https://signoz.io/docs/opentelemetry-collection-agents/vm/install/) to confirm the config is valid and the collector is running without errors.
- **In SigNoz:** Confirm data in [Traces](https://signoz.io/docs/traces/), [Metrics](https://signoz.io/docs/metrics/), or [Logs](https://signoz.io/docs/logs/) (and [Infrastructure](https://signoz.io/docs/infrastructure-monitoring/) for host metrics) depending on what you enabled.
