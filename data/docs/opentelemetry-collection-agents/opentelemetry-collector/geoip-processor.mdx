...
The GeoIP processor reads an IP address from a configured attribute, looks it up in a local MaxMind database, and writes geographic data back as `geo.*` attributes on the same record. It supports traces, logs, and metrics.

For `8.8.8.8`, the processor adds `geo.city_name`, `geo.country.iso_code`, `geo.location.lat`, `geo.location.lon`, and more. Use these attributes for geographic filtering, dashboards, and alerts in SigNoz.
...
By default, the processor checks `client.address` then `source.address`, and writes attributes to the resource. To override the defaults:
