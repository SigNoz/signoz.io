...
**Drop data points by attribute value**
...
processors:
  transform:
    ...
    log_statements:
      - context: log
        statements:
          - set(resource.attributes["deployment.region"], "us-east-1")
```
...
- **error_mode.** The default is `ignore` (via the `processor.*.defaultErrorModeIgnore` feature gate, enabled by default in recent collector releases). Set `error_mode: propagate` to surface type mismatch failures as pipeline errors instead of skipping them silently.
