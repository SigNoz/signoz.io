...
The OpenTelemetry (OTel) Collector is a vendor-agnostic telemetry data pipeline that receives, processes, and exports traces, metrics, and logs. It acts as a central hub for telemetry data with three key capabilities:

- **Receive:** Telemetry data in multiple formats (OTLP, Jaeger, Prometheus, Zipkin, etc.)
- **Process:** Data through transformation and filtering
- **Export:** To one or more observability backends

This guide covers each configuration component in detail and includes best practices for production deployments. For background on why you might use a Collector, see [Why use the OpenTelemetry Collector?](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/why-to-use-collector/). For a broader conceptual walkthrough before configuring Collector, see our [OpenTelemetry Collector complete guide](https://signoz.io/blog/opentelemetry-collector-complete-guide/).
...
```yaml:otel-collector-config.yaml defaultCollapsed minimap
receivers:
  otlp:
    protocols:
      grpc:
...
The native OpenTelemetry Protocol (OTLP) receiver, which supports both gRPC and HTTP.
...
<Admonition>
  For the full list of available receivers, see the
  <a href="https://github.com/open-telemetry/opentelemetry-collector/tree/main/receiver" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector receivers</a>
  and
  <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector-contrib receivers</a>.
</Admonition>

#### Multiple Instances of the Same Receiver

You can run multiple instances of the same receiver type by appending a forward slash and a name: `type/name`. For example, to scrape two different endpoint groups at different intervals:
...
Each named instance is configured independently and must be listed separately in the pipeline's `receivers` array. The name after the slash is arbitrary but must be unique.

<Admonition type="warning">
The forward slash separator is required. Writing `httpcheckcritical` (without the slash) causes a configuration error.
</Admonition>
...
The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/resourceprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">resource processor</a> adds, updates, or removes resource attributes on telemetry data.
...
For more examples, see [Setting resource attributes for logs using the resource processor](https://signoz.io/docs/logs-management/guides/set-resource-attributes-for-logs/#using-resource-processor).

#### Attributes Processor

Attributes provide additional metadata, or context within telemetry data, such as HTTP status code, HTTP URL, log level.

The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/attributesprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">attributes processor</a> modifies span, log, or metric attributes in telemetry data.
...
For more examples, see the guides on dropping [logs](https://signoz.io/docs/logs-management/guides/drop-logs/), [metrics](https://signoz.io/docs/userguide/drop-metrics/), and [traces](https://signoz.io/docs/traces-management/guides/drop-spans/#filter-processor).

#### Transform Processor

The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/transformprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">transform processor</a> modifies telemetry data using the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/pkg/ottl" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Transformation Language</a> (OTTL). This is especially useful for scenarios such as:
...
For more examples, see the PII scrubbing guides for [logs](https://signoz.io/docs/logs-management/guides/pii-scrubbing/) and [traces](https://signoz.io/docs/traces-management/guides/pii-scrubbing/).

#### GeoIP Processor

The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/geoipprocessor" target="_blank" rel="noopener noreferrer nofollow">GeoIP processor</a> reads an IP address from a configured attribute, looks it up in a local MaxMind database, and writes geographic attributes (`geo.city_name`, `geo.country.iso_code`, `geo.location.lat`, `geo.location.lon`, and more) back onto the same record. It works across traces, logs, and metrics.
...
See the [GeoIP processor guide](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/geoip-processor/) for setup, configuration options, and troubleshooting.

<Admonition>
  For the full list of available processors, see the
  <a href="https://github.com/open-telemetry/opentelemetry-collector/tree/main/processor" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector processors</a>
  and
  <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector-contrib processors</a>.
</Admonition>

#### Resource Detection Processor

The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/resourcedetectionprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">resource detection processor</a> collects resource information about the host environment. It can automatically detect attributes like hostname, OS information, cloud provider details (AWS, GCP, Azure, etc.), and Kubernetes metadata:

```yaml
processors:
  resourcedetection:
    # Drop "docker" if this host does not run Docker. The detector then only logs warnings.
...
This example adds resource attributes from the environment, host system, and Docker containers to traces, logs, and metrics. On Kubernetes, add the `k8snode` detector, which needs the `K8S_NODE_NAME` environment variable and RBAC read access to nodes. On Collector v0.154.0 and newer, use `k8s_api` to avoid a deprecation warning. See the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/resourcedetectionprocessor/README.md#supported-detectors" target="_blank" rel="noopener noreferrer nofollow">complete list of supported detectors</a>.

The `timeout` defines how long the processor waits for detection to complete. See [Setting resource attributes for logs using the resource detection processor](https://signoz.io/docs/logs-management/guides/set-resource-attributes-for-logs/#using-resource-detection-processor) for more details.
...
<Admonition>
  For the full list of available exporters, see the
  <a href="https://github.com/open-telemetry/opentelemetry-collector/tree/main/exporter" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector exporters</a>
  and
  <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector-contrib exporters</a>.
</Admonition>
...
The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/extension/healthcheckextension/README.md" target="_blank" rel="noopener noreferrer nofollow">health_check extension</a> provides an HTTP endpoint that can be probed to see the status of the collector. This extension can be used as a liveness and/or readiness probe on Kubernetes.
...
The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/extension/pprofextension/README.md" target="_blank" rel="noopener noreferrer nofollow">pprof extension</a> enables performance profiling of the OTel Collector using Go's built-in <a href="https://pkg.go.dev/net/http/pprof" target="_blank" rel="noopener noreferrer nofollow">pprof</a> tools. This is useful for debugging performance issues such as high CPU or memory usage and goroutine leaks.
...
The <a href="https://github.com/open-telemetry/opentelemetry-collector/blob/main/extension/zpagesextension/README.md" target="_blank" rel="noopener noreferrer nofollow">zPages extension</a> provides live debugging pages with real-time information about the collector's internal state, including traces and pipeline activity. This is useful for:
...
<Admonition>
  For the full list of available extensions, see the
  <a href="https://github.com/open-telemetry/opentelemetry-collector/tree/main/extension" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector extensions</a>
  and
  <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/extension" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector-contrib extensions</a>.
</Admonition>
...
<Figure
  ...
  caption="OpenTelemetry Collector: Receivers → Processors → Exporters → SigNoz Cloud"
/>
...
The `type/name` naming convention (for example, `httpcheck/critical`) applies to all component types: receivers, processors, and exporters. When using named instances, reference the full `type/name` in the pipeline:
...
See [Enable TLS for OpenTelemetry Collector](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/tls-configuration/) for step-by-step configuration of receiver TLS, exporter TLS, and mutual TLS (mTLS).

See the <a href="https://opentelemetry.io/docs/security/config-best-practices/#use-encryption-and-authentication" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Security Best Practices on encryption and authentication</a> for more details.
...
See the <a href="https://opentelemetry.io/docs/security/config-best-practices/#protect-against-denial-of-service-attacks" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Security Best Practices on DoS protection</a> for more information on minimizing the exposure of OTel Collector endpoints.
...
This configuration prevents the collector from blocking, ensures it retries failed requests, and helps avoid excessive memory usage under heavy loads. See the <a href="https://github.com/open-telemetry/opentelemetry-collector/blob/main/exporter/exporterhelper/README.md#configuration" target="_blank" rel="noopener noreferrer nofollow">exporter helper configuration reference</a> for more options.
...
See the <a href="https://github.com/open-telemetry/opentelemetry-collector/blob/main/config/configgrpc/README.md#compression-comparison" target="_blank" rel="noopener noreferrer nofollow">gRPC compression comparison</a> for a detailed comparison of available compression techniques.
...
Before deploying the collector, validate the configuration with the `--dry-run` flag. This allows you to catch misconfigurations or issues with the config without starting the collector.
...
- [Why use the OpenTelemetry Collector?](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/why-to-use-collector/): understand when and why to use a Collector in your telemetry pipeline.
- [Switch from direct export to Collector](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/switch-to-collector/): migrate from direct OTLP export to a Collector-based setup.
- [SigNoz Cloud ingestion endpoints](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint): find the correct region endpoint for SigNoz Cloud.
