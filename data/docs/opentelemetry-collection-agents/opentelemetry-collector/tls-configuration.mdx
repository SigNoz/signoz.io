...
title: Enable TLS for OpenTelemetry Collector
...
| `client_ca_file_reload` | Receivers | Set `true` to reload `client_ca_file` when modified on disk |
...
| `reload_interval` | Receivers, Exporters | Interval to hot-reload `cert_file` and `key_file`, e.g. `"1h"` |
