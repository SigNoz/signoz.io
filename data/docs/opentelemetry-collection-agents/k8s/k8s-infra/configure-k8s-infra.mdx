...
Helm replaces arrays entirely rather than merging them. If you override `podAssociation`, you must include **all** desired rules — not just the ones you want to add. Omitting the default `connection` rule, for example, prevents the k8sattributes processor from associating telemetry to pods by source IP, which causes `extractLabels` and `extractAnnotations` to stop working.
</Admonition>
...
Defining a processor under `otelAgent.config.processors` (or `otelDeployment.config.processors`) adds it to the collector configuration but does **not** automatically include it in any pipeline. You must also reference the processor in the appropriate pipeline's `processors` list under `otelAgent.config.service.pipelines` for it to take effect.

<Admonition type="warning">
Helm replaces arrays entirely rather than merging them. When you override a pipeline's `processors` list, include all processors you need in that pipeline — both your custom processor and the existing defaults.
</Admonition>
