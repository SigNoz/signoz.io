...
To install OpenTelemetry Collector in your EKS Fargate Kubernetes cluster, please check out the [EKS Fargate Installation guide](/docs/opentelemetry-collection-agents/k8s/serverless/install/).
...
```yaml:fluentbit-config-map.yaml defaultCollapsed minimap
kind: ConfigMap
apiVersion: v1
metadata:
  name: aws-logging
