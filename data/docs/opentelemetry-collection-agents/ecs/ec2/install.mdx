...
receivers:
  ...
  # Collects ECS task and container metrics from the ECS Task Metadata Endpoint.
  # As a daemon, this only covers the collector's own task. For per-task
  # container metrics of your applications, use the ECS sidecar setup.
  ...
  filelog:
    ...
    operators:
      - id: container-parser
        type: container
        # Docker log paths carry no Kubernetes metadata; skip path parsing.
        add_metadata_from_filepath: false

processors:
  # Detects host-level resource attributes (EC2 instance, region, account).
  # The `ecs` detector is left out on purpose: it reads the collector's own task
  # metadata, so in a daemon it would label every application's telemetry with
  # the collector's task. Set ECS task attributes from the application instead.
  resourcedetection:
    detectors: [env, ec2]
    override: false
...
exporters:
  ...
  # Uses environment variables for portability, which you'll set in your ECS task definition.
  otlphttp:
    endpoint: 'http://<SIGNOZ_SELF_HOST_ENDPOINT>:4318'
...
If you are someone who understands things better through video implementation, here are videos to help you. Check them out:

**Part 1 — AWS ECS OpenTelemetry Tutorial: Daemon Service Setup, SSM Config & CloudFormation Deploy**

<YouTube id="adodyKwlh84" />

**Part 2 — AWS ECS OpenTelemetry Tutorial: Deploy App Containers, Route OTLP & Validate Signals in SigNoz**

<YouTube id="NxW0XhWhN-s" />
...
- [Configure the Daemon Service Collection Agent](/docs/opentelemetry-collection-agents/ecs/ec2/configure/)
...
- [Learn more about Collection from EC2](/docs/opentelemetry-collection-agents/ecs/ec2/overview/)
