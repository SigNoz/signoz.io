...
Then add `resource/ownership` to the `processors` list of each pipeline that needs it. Edit the list your file already has. Do not replace it, because a pipeline often already runs `memory_limiter`, `resourcedetection`, a filter, or `batch`, and removing any of those changes what the Collector sends:

```yaml:otel-collector-config.yaml
service:
  pipelines:
    traces:
      # Keep every entry this list already has. Add resource/ownership before batch.
