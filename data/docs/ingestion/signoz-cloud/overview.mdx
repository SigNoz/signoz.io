...
* Using the `signoz-ingestion-key` header with the value set to your Ingestion key.
* Using the `signoz-ingestion-key` header with the value set `Bearer <key>` to your Ingestion key. 
* Using the `signoz-ingestion-key` header for basic authentication. 
