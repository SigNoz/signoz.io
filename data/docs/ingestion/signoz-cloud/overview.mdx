...
Configure the OpenTelemetry [exporter](https://opentelemetry.io/docs/specs/otel/protocol/exporter/) in your collector configuration file to point to the appropriate SigNoz endpoint. Use either the `otlp` (gRPC) or `otlphttp` exporter — note that the two expect different endpoint formats.
