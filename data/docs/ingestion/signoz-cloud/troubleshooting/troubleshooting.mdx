...
<Admonition type="tip">
To quickly confirm that data is actually landing in SigNoz, call the [Stats API](/docs/telemetry-stats-api/): `GET /api/v1/stats`. Non-zero `telemetry.traces.count`, `telemetry.logs.count`, or `telemetry.metrics.count` values and recent `last_observed` timestamps confirm that the corresponding signal is being received.
</Admonition>
