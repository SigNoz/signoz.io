...
This page covers ingestion status codes and errors once your telemetry is being exported. If you're not sure your application is generating or exporting telemetry at all, start with [Debug missing traces, logs, and metrics in SigNoz](https://signoz.io/docs/ingestion/signoz-cloud/troubleshooting/data-not-appearing/) instead.
</Admonition>
...
<Admonition type="tip">
To quickly confirm that data is actually landing in SigNoz, call the [Stats API](https://signoz.io/docs/telemetry-stats-api/): `GET /api/v1/stats`. Non-zero `telemetry.traces.count`, `telemetry.logs.count`, or `telemetry.metrics.count` values and recent `last_observed` timestamps confirm that the corresponding signal is being received. The metric keys cover scalar samples only, so exponential histograms do not show up there.
</Admonition>
...
### Why am I seeing "connection is draining" logs when pushing metrics to SigNoz?

These "connection is draining" logs are expected and occur occasionally as the SigNoz ingestor system automatically scales up and down. These logs indicate temporary connection issues, but they don't result in data loss.

**Example log:**

```json
{"level":"info","ts":1757814915.0786107,"caller":"exporterhelper/retry_sender.go:118","msg":"Exporting failed. Will retry the request after interval.","kind":"exporter","data_type":"metrics","name":"otlp","error":"rpc error: code = Unavailable desc = the connection is draining","interval":"7.07140113s"}
```
...
Seeing these info logs 2-3 times per day is within normal operating parameters and doesn't require any action on your part. The retry mechanism built into the OTLP exporter ensures reliable delivery of your observability data despite these temporary connection issues.
