...
Prefer OTLP/gRPC (port 4317) for server-side telemetry. It offers better performance and backpressure. Use OTLP/HTTP (port 4318) for browser-based telemetry where gRPC is not supported.
...
Migrating from an existing OpenTelemetry setup? See: <a href="https://signoz.io/docs/migration/migrate-from-opentelemetry/self-hosted/" target="_blank" rel="noopener noreferrer">Migrate from existing OpenTelemetry to Self-Hosted SigNoz</a>.
