...
- Details about the scope at which SigNoz is operating:
    ...
    - Note: **SigNoz does not collect the actual queries or user requests**
...
<Admonition type="info">
If possible, we kindly ask you to keep the usage reporting feature enabled, as it helps us better understand how the open source community operates SigNoz.

In case you want to opt-out, we request you to disable the `identites` which will disable collecting any trais or identities and will sends inforamtion about only the Environment, Scale and Version related information.
You can set the following configuration for this:
```yaml
statsreporter:
  enabled: true
  collect:
