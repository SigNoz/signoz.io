...
<Admonition type="info" title="Why this guide uses hooks rather than the built-in exporter">
Muse Code ships an OpenTelemetry exporter, but its `telemetry.auth` setting accepts only `bearer` or `none` and offers no field for supplying your own credential, so it cannot send the `signoz-ingestion-key` header SigNoz Cloud requires. This guide uses Muse Code's hook system instead, which runs your own script and can therefore set any header it needs.
</Admonition>
...
Muse Code runs a hook command at each point in its lifecycle and passes the event to that command as JSON on stdin. The script below turns those events into OpenTelemetry spans and posts them to SigNoz over OTLP/HTTP. No SDK is required, because SigNoz accepts OTLP/HTTP JSON directly.
...
<Admonition type="warning" title="Configuration must live in a file, not in environment variables">
