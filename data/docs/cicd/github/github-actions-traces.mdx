...
meta_title: GitHub Actions Traces - Setup OTel Receiver
description: Learn how to collect GitHub Actions pipeline traces in SigNoz using the GitHub Receiver. Set up webhook, OTel Collector config for VM and Kubernetes.
...
Create `github-otel-collector-config.yaml` file and add the following contents.
    ...
    ```yaml:/deploy/docker/github-otel-collector-config.yaml defaultCollapsed
    ...
    receivers:
      github:
        webhook:
