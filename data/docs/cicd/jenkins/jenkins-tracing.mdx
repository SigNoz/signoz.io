---
date: 2025-04-28
id: jenkins-tracing
title: Jenkins Traces
description: Instrument Jenkins with OpenTelemetry and send traces to SigNoz
---
   
## Overview 
This integration helps you to monitor and observe Jenkins with OpenTelemetry by visualizing jenkins jobs and pipelines executions as distributed traces in SigNoz
To know more about tracing refer [here](https://signoz.io/docs/instrumentation/overview/)

## Prerequisites
- Jenkins [>2.479.3]

### Setup

SigNoz runs its own OTel collector in both the Self-Hosted and Cloud version

### Install and Configure Jenkins OpenTelemetry plugin
* For installation refer [here](https://plugins.jenkins.io/opentelemetry/)

<Tabs>

<TabItem value="signoz-cloud" label="SigNoz Cloud" default>

### Step 1: Set the OTLP Endpoint
Navigate to Manage Jenkins -> System -> OpenTelemetry section

Depending on the choice of your region for the SigNoz cloud, the ingest endpoint will vary according to this table.
  | Region | Endpoint |
  | --- | --- |
  | US |  ingest.us.signoz.cloud:443 |
  | IN |  ingest.in.signoz.cloud:443 |
  | EU | ingest.eu.signoz.cloud:443 |

### Step 2: Set the Authentication headers
In the same section click on Authentication and select **Header Authentication**

Add Header key as **signoz-ingestion-key** and value as SigNoz_Ingestion_Key

<figure data-zoomable align="center">
    <img
        src="/img/docs/cicd/jenkins/jenkins-otel-cloud-config.webp"
        alt="Jenkins OTel plugin configuration for SigNoz cloud"
    />
    <figcaption>
        <i>
        Jenkins OTel plugin configuration for SigNoz cloud
        </i>
    </figcaption>
</figure>

Note: Replace \<signoz-ingestion-key\> with your ingestion key

</TabItem>

<TabItem value='self-host' label='Self-Host' default>

Navigate to Manage Jenkins -> System -> OpenTelemetry section 
<figure data-zoomable align="center">
    <img
        src="/img/docs/cicd/jenkins/jenkins-otel-config.webp"
        alt="Jenkins OTel plugin configuration for Self-Hosted SigNoz"
    />
    <figcaption>
        <i>
        Jenkins OTel plugin configuration for Self-Hosted SigNoz
        </i>
    </figcaption>
</figure>

Enter the SigNoz self-hosted endpoint in the textbox and set **No Authentication**

</TabItem>
</Tabs>

## Visualize the traces in SigNoz Traces section

<figure data-zoomable align="center">
    <img
        src="/img/docs/cicd/jenkins/jenkins-traces.webp"
        alt="Jenkins pipelines traces on SigNoz UI"
    />
    <figcaption>
        <i>
        Jenkins pipelines traces on SigNoz UI
        </i>
    </figcaption>
</figure>

## Jenkinsfile custom spans

With the OTel plugin installed, you can also configure custom spans in your Jenkins pipelines

```groovy
  pipeline {
    agent any
    stages {
      stage('Add Custom Span Attribute') {
          steps {
            script {
              withSpanAttribute(
                  key: 'my-error-prone-span',
                  value: 'new-traces',
                  type: 'STRING',
                  target: 'CURRENT_SPAN'
              )
              echo 'Custom span attribute added'
            }
          }
        }
    }
  }
```

For more details about spans refer [here](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/trace/api.md#span)

## Conclusion

Jenkins Otel plugin can be used to visualize Jenkins traces effectively on SigNoz