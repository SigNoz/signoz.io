...
- **Understand Agent Behavior**: See how tool calls are distributed across tools and where execution time goes, and measure steps per thread — items started vs completed, tool executions, and model calls per trace — to understand how much work each agent run performs.
...
- **Tool Call Distribution**: Shows how tool calls are spread across different tools (grouped by `openclaw.toolName`), helping you see which tools agents rely on most.
- **Tool Calls Time Distribution**: Breaks down total execution time spent in each tool, surfacing the tools that consume the most time and are most likely to slow down or destabilize runs.
...
- **Items Started vs Completed vs Active**: Plots the average number of harness items started, completed, and active per agent run over time, showing how much work runs take on and whether items are finishing or piling up.
- **Steps per Thread (Current Avg Completed)**: Single value showing the average number of completed items per harness run in the selected range, a quick gauge of how many steps a typical thread takes.
- **Tool Executions per Trace**: Table of `openclaw.tool.execution` span counts grouped by trace, identifying the threads that performed the most tool calls.
- **Model Calls (Turns) per Trace**: Table of `openclaw.model.call` span counts grouped by trace, showing how many model turns each thread consumed and highlighting unusually long conversations.
