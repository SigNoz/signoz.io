...
- **execute_tool error rate % by tool**: Per-tool error rate %, computed as `execute_tool` spans with `has_error=true` over total, grouped by tool (`gen_ai.tool.name`). This pinpoints which specific tools are failing rather than just the aggregate rate.
