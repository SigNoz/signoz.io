...
The cert-manager dashboard tracks certificate readiness, expiry, controller activity, ACME operations, and resource usage across all Kubernetes namespaces.
...
Only populated when cert-manager uses an ACME issuer such as Let's Encrypt. Self-signed and CA issuers do not generate these metrics.
