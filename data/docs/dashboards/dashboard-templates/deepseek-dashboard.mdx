...
To use this dashboard, you need to set up the data source and send telemetry to SigNoz. Follow the [DeepSeek Monitoring](https://signoz.io/docs/deepseek-monitoring/) guide to get started.
</Admonition>
...
This dashboard tracks critical performance metrics for your DeepSeek API usage using OpenTelemetry to help you:
...
- **Analyze Model Adoption**: Understand which DeepSeek model variants are being used most often to track preferences and measure adoption of newer releases.
...
- **Understand Service Distribution**: See which services and programming languages are leveraging the DeepSeek API across your stack.
...
- **Total Error Rate**: Tracks the percentage of DeepSeek API calls that return errors, providing a quick way to identify reliability issues.
...
- **HTTP Request Duration**: Monitors the duration of outbound HTTP requests made during LLM calls, helping identify network bottlenecks and API response time patterns that impact overall DeepSeek API performance.

### Usage Analysis
- **Model Distribution**: Shows which DeepSeek model variants are being called most often, helping track preferences and measure adoption across different models.
- **Requests Over Time**: Captures the volume of requests sent to DeepSeek API over time, revealing demand patterns and high-traffic windows.
- **Services and Languages Using DeepSeek**: Breakdown showing where the API is being adopted across different services and programming languages in your stack.
- **DeepSeek API Request Logs**: Comprehensive list of all generated logs for DeepSeek API requests associated with the given service name, providing detailed visibility into API call patterns.
