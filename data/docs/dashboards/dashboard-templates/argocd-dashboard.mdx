...
- **Application Details**: Comprehensive view of individual applications and their current state
- **Orphaned Resources per App**: Identifies stray resources not properly tracked or cleaned up by ArgoCD
...
- **Cluster Connection Status**: Real-time status of Kubernetes cluster connectivity
  - Essential for identifying communication issues between ArgoCD and destination clusters
...
- **Pending Repo Requests**: Indicates backlogs in Git repository operations

  - Useful for spotting synchronization bottlenecks

- **Git Request Duration (P99)**: The 99th percentile response time for Git interactions
  - Tracks performance and latency when interacting with Git providers
