...
The time series `chartAppearance.spanGaps` field is validated. When `fillOnlyBelow` is `true`, `fillLessThan` is required and must be a positive duration. `fillLessThan` accepts second (`s`), minute (`m`), hour (`h`), day (`d`), and week (`w`) units (for example `5m`, `1d`, `1w`). Any dashboard payload with `fillOnlyBelow: true` and a missing, empty, or non-positive `fillLessThan` fails validation on save or load. Update those panels or API/Terraform payloads with a valid duration. In the UI these two fields are the **Disconnect values** control and its **Threshold value** input; see [Disconnect values](https://signoz.io/docs/dashboards/panel-types/timeseries/#disconnect-values).
</Admonition>
