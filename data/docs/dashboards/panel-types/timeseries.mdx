...
Hover over a point to see its value in the tooltip. The tooltip header shows that point's timestamp. For a point on today's date, the header shows the time only (for example, `14:32:05`). For a point on any earlier day, it shows the full date and time.
...
- **Search series** — When a legend has more series than fit in the space reserved for it, a search box and a **Showing N of M series** readout appear above the list. Type part of a series name to filter the list in real time; the count updates to match the visible results. This applies to both `Right` and `Bottom` legends.

<Admonition type="info" title="Default series colors updated">
The default series color palette no longer assigns red hues to arbitrary series, so red stays reserved for threshold and alert indicators. Explicit color overrides are unchanged, but any series that took a red tone from the default palette now renders in a replacement hue (olive, green, teal, blue, indigo, or purple). This affects existing dashboards across all series-based panel types. If you relied on a default color to identify a series, set an explicit color from **Legend Colors**.
</Admonition>
