...
<Figure
  ...
  alt="A dashboard using three Text panels: a transparent section title with its header hidden, a coloured runbook, and an incident checklist with two boxes ticked"
  caption="Three Text panels on one dashboard: a transparent section title, a coloured runbook, and an incident checklist."
/>
...
Fenced code blocks are highlighted for `bash` (`sh`, `shell`), `css`, `diff`, `docker` (`dockerfile`), `go`, `java`, `javascript` (`js`), `json`, `markup` (`html`, `xml`), `python` (`py`), `rust`, `sql`, `typescript` (`ts`), and `yaml` (`yml`). A fence with no language, or one the panel does not recognise, renders as plain monospace rather than an error.
...
- **Background**: a row of swatches.
  ...
  - Eight presets: Robin, Purple, Sakura, Cherry, Amber, Forest, Sienna, and Slate. Each preset pairs a surface colour with a text colour, defined separately for light and dark mode, and every pair clears a 4.5:1 contrast ratio. A panel saved in dark mode shows the same preset in its light colours in light mode, with no re-save.
  - **Custom** takes any hex colour (`#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa`). The text colour is chosen automatically for contrast against whatever you pick.
...
| `presentation.background` | string, optional | A hex colour. `#00000000` is transparent, and omitting the field uses the default panel surface. The server validates that the value is a hex colour. |
