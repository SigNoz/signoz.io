...
Anomaly-based alerts are currently only available through the **old alerts experience**. When creating a new alert, do not select "Try the new experience" — use the classic alert creation flow to access the Anomaly Detection Alert option.
</Admonition>
