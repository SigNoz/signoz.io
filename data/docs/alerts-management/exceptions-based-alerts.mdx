...
title: How to Create Exceptions-based Alerts
...
An exceptions-based alert fires when a ClickHouse query over exception data crosses a threshold. Use this alert type to detect specific exception types, track error frequency, or monitor exception patterns across services.

## Overview

Create an exceptions-based alert in three steps: write the ClickHouse query, set the alert conditions, and configure severity and notifications.

<Admonition type="info">

**Prerequisites**
- At least one [notification channel](https://signoz.io/docs/setup-alerts-notification/) configured in SigNoz
- Traces being ingested into SigNoz (exceptions are derived from trace data)
