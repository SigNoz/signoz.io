...
Time aggregation determines how multiple data points within a collection interval are combined before evaluation.
...
Aggregation: max (not count distinct)
Evaluation: at least once
Reason: You want the maximum restart count, not the number of unique restart values
```
