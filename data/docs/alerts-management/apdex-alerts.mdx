...
This PromQL-based approach only works with histogram metrics that use **cumulative temporality** (such as `http.server.duration.bucket` exported with cumulative aggregation). SigNoz's auto-generated span metrics (`signoz_latency.bucket`, `signoz_latency.count`) use **delta temporality** by default, and PromQL does not read delta series. If you use `signoz_latency` metrics in these queries, you will get no data.
</Admonition>
...
<Figure
  src="/img/docs/alerts/apdex-alert-1.webp"
  alt="Alert condition"
...
<Figure
  src="/img/docs/alerts/apdex-alert-2.webp"
