...
A Metric-based alert in SigNoz allows you to define conditions based on metric data and trigger alerts when these conditions are met. You can define your metric query using **Query Builder**, **ClickHouse queries**, or **PromQL**. For sample PromQL expressions, see [PromQL Alert Examples](https://signoz.io/docs/alerts-management/user-guides/promql-alert-examples/).
...
<Figure
  src="/img/docs/alerts/alert-name-labels.webp"
  alt="Alert name and labels at the top of the alert creation page"
  caption="Set alert name and labels"
/>

## Step 1: Define the Metric
In this step, you use the [Metrics Query Builder](https://signoz.io/docs/userguide/query-builder-v5/)
...
- **[Legend Format](https://signoz.io/docs/userguide/query-builder-v5/#legend-format)**: Customize the legend's format in the visual representation of the alert.
...
<Figure
  ...
  alt="Using Query Builder to the metric to monitor"
  caption="Using Query Builder to define the metric to monitor"
/>


To know more about the functionalities of the Query Builder, checkout the [documentation](https://signoz.io/docs/userguide/query-builder-v5/).
...
<Figure
  src="/img/docs/alerts/evaluation-window-rolling.webp"
  alt="Evaluation window options showing Rolling and Cumulative modes"
  caption="Evaluation window with Rolling and Cumulative options"
/>
...
### Threshold Unit
   ...
   This dropdown stays disabled until you set a unit on the query in Step 1, and it then offers the units in that same category.
...
<Figure
  src="/img/docs/alerts/alerts-metrics-based-2.webp"
  alt="Set alert conditions with threshold, routing policies, and advanced options"
...
You can incorporate template variables in the alert message to make notifications more informative:

| Variable | Description |
| --- | --- |
| `{{$value}}` | The current aggregated value that triggered the alert, formatted with the unit set on the query in Step 1 |
| `{{$threshold}}` | The threshold value that was breached, formatted with the [Threshold Unit](#threshold-unit) from Step 2 |
| `$<attribute-name>` | Any attribute used in the Group By clause (e.g., `$service.name`). The `{{$service.name}}` form works too |

The two variables read their units from different places. `{{$value}}` uses the unit set on the query in Step 1, and `{{$threshold}}` uses the Threshold Unit in Step 2. The Threshold Unit list is filtered to the query unit's category but still mixes decimal and binary units, so choosing `Gigabytes (GB)` for the query and `Bytes (B) (IEC)` for the threshold renders GB in one half of your message and GiB in the other. Pick both from the same family to keep them consistent.

**Example**: If you have a query grouped by `service.name` with a threshold of 500 MB, you could write:
`Memory usage for $service.name is {{$value}} (threshold: {{$threshold}})`
...
<Figure
  src="/img/docs/alerts/alerts-metrics-based-3.webp"
...
<Figure
  src="/img/docs/product-features/alerts/metrics/mem-usage-alert-builder.webp"
  alt="metrics builder query for memory usage"
  caption="Memory usage metric builder query"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/mem-usage-alert-builder-condition.webp"
  alt="metrics builder query for memory usage"
  caption="Memory usage alert condition"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/mem-usage-alert-percentage-builder.webp"
  alt="metrics builder query for memory usage"
  caption="Memory usage percentage query"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/mem-usage-alert-percentage-builder-condition.webp"
  alt="metrics builder query for memory usage"
  caption="Memory usage percentage condition"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/error-percentage-builder.webp"
  alt="metrics builder query for error percentage"
  caption="Error percentage query"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/error-percentage-builder-condition.webp"
  alt="metrics builder query for error percentage"
  caption="Error percentage condition"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/p95-latency-builder.webp"
  alt="metrics builder query for latency"
  caption="Endpoint latency query"
/>
...
<Figure
  src="/img/docs/product-features/alerts/metrics/p95-latency-builder-condition.webp"
  alt="metrics builder query for latency"
  caption="Endpoint latency condition"
/>
