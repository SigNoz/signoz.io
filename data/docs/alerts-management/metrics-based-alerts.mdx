...
| --- | --- |
| `{{$value}}` | The current aggregated value that triggered the alert, formatted using the query's Y-axis unit |
| `{{$threshold}}` | The threshold value that was breached, formatted using the threshold unit |
| `$<attribute-name>` | Any attribute used in the Group By clause (e.g., `$service.name`) |

`{{$value}}` and `{{$threshold}}` use different unit sources for formatting. `{{$value}}` is formatted according to the Y-axis unit set on the query (Step 1), while `{{$threshold}}` is formatted according to the threshold unit (Step 2). If these units differ, the two variables render in different scales. For example, if the Y-axis unit is set to binary Bytes, `{{$value}}` renders as GiB/MiB/KiB, but if the threshold unit is set to decimal Bytes (SI), `{{$threshold}}` renders as GB/MB/KB. To align them, set the query's Y-axis unit to the same unit family as the threshold unit.
