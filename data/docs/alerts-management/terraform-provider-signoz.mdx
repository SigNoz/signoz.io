...
- SigNoz Instance
  - `Self-hosted Community Edition (version v0.85.0 or later)`
  - `SigNoz Enterprise`
  - `SigNoz Cloud`
...
By setting the environmental variables below, the SigNoz provider will automatically read these variables and authorize your SigNoz account:
``` bash
...
### Step 2: Check the Correctness of the Definition
...
After receiving the success message on the console, you'll find the alert provisioned on the remote:
