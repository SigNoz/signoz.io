...
By setting the environment variables below, the signoz provider will automatically read these variables and authorize your SigNoz account. Use an API key for `SIGNOZ_ACCESS_TOKEN` — see [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) to create one.
``` bash
...
Once an alert is managed by Terraform — either created with a `signoz_alert` resource or imported — you can update it by modifying its definition in your `.tf` file. Run `terraform plan` to preview the changes and `terraform apply` to push them to SigNoz. This follows the standard Terraform resource lifecycle.
