...
You can explicitly set these variables in the `provider {}` block as shown below, but avoid hardcoding your ingestion key if you plan to commit your `.tf` files to an online version control system.
```hcl
provider "signoz" {
  endpoint="https://your-signoz-endpoint"
  INGESTION_KEY="your-ingestion-key"  # Do not type your token directly if you push codes online
}
```
## Create new alert
