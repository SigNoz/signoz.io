---
date: 2026-05-12
id: faqs
description: Frequently asked questions about SigNoz alerts and monitoring features
title: Alert Management FAQs
doc_type: reference
---

### Q. How can I setup synthetic monitoring checks/alerts with SigNoz?
A. Currently synthetic monitoring is not supported in SigNoz. But you can use httpcheck to create HTTP-based health checks for your services. More details [here](https://signoz.io/docs/monitor-http-endpoints/).

### Q. Can I edit the subject of the alerts?
A. Yes, you can edit the subject of alerts for Slack, MS Teams, PagerDuty, and Opsgenie notification channels.
To edit the alert subject:
1. Navigate to **Settings**
2. Go to **Account Settings**
3. Select **Notification channel**
4. Click on the desired notification channel
5. Edit the **Title** field

You can also refer to the <a href="https://prometheus.io/docs/alerting/latest/notifications/" target="_blank" rel="noopener noreferrer nofollow">Prometheus Alertmanager notifications guide</a> for more details on customizing alert templates.

<Admonition>
Note: You cannot edit the subject of email alerts.
</Admonition>

