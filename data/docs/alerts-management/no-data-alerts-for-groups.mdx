...
description: This document describes how to use alerting feature to detect when specific groups of resources/entities stop sending data.
...
- *Aggregate within time series*: Select `Count`
- *Aggregate across time series*: Select `Sum` and group by `host.name`
...
- Condition: Set to `Equal to`
- Threshold: Enter `0`
- Match type: Select `All the time` to ensure the condition is met consistently
- Evaluation window: Choose `Last 10 minutes` with `Rolling` mode — should match the timeshift duration
