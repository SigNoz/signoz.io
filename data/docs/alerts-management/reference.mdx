---
date: 2026-09-25
title: Alerts Reference
description: Reference documentation for alert evaluation patterns and time aggregation in SigNoz.
---

<DocCardContainer>

<DocCard
    title="Understanding Alert Evaluation Patterns"
    description="Learn about different alert evaluation patterns and how to use them effectively"
    href="https://signoz.io/docs/alerts-management/user-guides/understanding-alert-evaluation-patterns/"
/>

<DocCard
    title="Time Aggregation Best Practices"
    description="Learn how to effectively use time aggregation in your alerts"
    href="https://signoz.io/docs/alerts-management/user-guides/time-aggregation-best-practices/"
/>

<DocCard
    title="Migrate Alert History API v1 to v2"
    description="Migrate from the deprecated v1 alert history API endpoints to the v2 replacements"
    href="https://signoz.io/docs/alerts-management/migrate-alert-history-api-v1-to-v2/"
/>

<DocCard
    title="Migrate Notification Channel API v1 to v2"
    description="Migrate from the deprecated v1 notification channel API endpoints to the v2 replacements"
    href="https://signoz.io/docs/alerts-management/migrate-notification-channel-api-v1-to-v2/"
/>

</DocCardContainer>
