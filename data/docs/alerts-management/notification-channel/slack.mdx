...
To create a new Slack notification channel, send a `POST` request to the `/api/v1/channels` endpoint with the following `curl` command. Replace `YOUR_SERVICE_ACCOUNT_KEY` with a service account key from **Settings → Service Accounts** — see [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) to create one.
