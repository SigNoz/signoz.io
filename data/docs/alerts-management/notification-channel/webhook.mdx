...
| `webhook_configs[].http_config.basic_auth.username` | `string` | Username for basic authentication. Leave empty for bearer auth or when authentication is not necessary. |
| `webhook_configs[].http_config.basic_auth.password` | `string` | Password for basic authentication. Specify a password or bearer token. |
