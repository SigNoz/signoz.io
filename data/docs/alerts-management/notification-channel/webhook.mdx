...
description: Configure Webhook channels in SigNoz to send alerts to custom endpoints or third-party services.
doc_type: howto
---
...
- **SigNoz Version**: Ensure you are using SigNoz version Webhook <a href="https://github.com/SigNoz/signoz/releases/tag/v0.7.4" target="_blank" rel="noopener noreferrer nofollow">v0.7.4</a> or later
...
- **Username and Password** (Optional): Provide credentials for basic authentication. You can set a username, a password, or both. Leave both empty when the endpoint needs no authentication.

<Admonition type="info">
Basic auth credentials are independent: setting only a username or only a password is accepted. A bearer token cannot be combined with basic auth credentials.
</Admonition>
...
To create a new Webhook notification channel, send a `POST` request to the `/api/v1/channels` endpoint with the following `curl` command. Replace `YOUR_API_KEY` with an API key from **Settings → Service Accounts** — see [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) to create one.
...
| `webhook_configs` | `array` | List of webhook configurations. |
| `webhook_configs[].send_resolved` | `boolean` | Whether to send notifications when an alert is resolved. Default: `true`. |
| `webhook_configs[].url` | `string` | The Webhook URL endpoint. **Mandatory**. Must be a valid URL. |
| `webhook_configs[].http_config` | `object` | HTTP configuration for the webhook, such as basic auth. |
| `webhook_configs[].http_config.basic_auth` | `object` | Basic authentication credentials. |
| `webhook_configs[].http_config.basic_auth.username` | `string` | Username for basic authentication. Optional and independent of the password. Leave empty for bearer auth or when authentication is not necessary. |
| `webhook_configs[].http_config.basic_auth.password` | `string` | Password for basic authentication, or a bearer token when the username is empty. Optional and independent of the username. |
...
- [Integrate with Incident.io](https://signoz.io/docs/alerts-management/notification-channel/incident-io/)
- [Integrate with Rootly](https://signoz.io/docs/alerts-management/notification-channel/rootly/)
- [Integrate with Zenduty](https://signoz.io/docs/alerts-management/notification-channel/zenduty/)
