...
date: 2026-08-10
doc_type: howto
title: Jira Cloud Notification Channel Setup for Alerts
...
- Issues are reported by the **service account, not a teammate's name** — a clean audit trail.
...
5. Note the generated `…@serviceaccount.atlassian.com` email — that's the username you'll enter in SigNoz.

### 2. Give it access to your project

The service account is treated like a regular user, so add it to each project it will file issues in.
...
2. Name the token (e.g. `signoz-jira-alert-channel`) and choose an expiry (1–365 days — tokens don't auto-rotate, so put the renewal on a calendar).
...
SigNoz automatically resolves your cloud ID and routes the service-account token correctly — you don't need to deal with cloud IDs or gateway URLs.

<Admonition type="info">
A personal API token also works — create one at <a href="https://id.atlassian.com/manage-profile/security/api-tokens" target="_blank" rel="noopener noreferrer nofollow">id.atlassian.com → Security → API tokens</a> and use your own email as the username — but issues are then reported under your name and the channel breaks if you leave or rotate the token. Prefer a service account where you can.
</Admonition>
...
- **Email**: the account email used for authentication — the service account's `…@serviceaccount.atlassian.com` (recommended) or a personal account email.
