...
- **SigNoz Version**: vX.Y.Z or later.
...
When several alerts are grouped into one notification, the card shows **one section per alert** — each with its own summary and logs/traces buttons — up to **30 alerts**. Beyond that, the card adds a **"…and N more alerts"** note with an **Open in SigNoz** button for the full list. This cap keeps the card within Google Chat's <a href="https://developers.google.com/workspace/chat/api/reference/rpc/google.apps.card.v1#card" target="_blank" rel="noopener noreferrer nofollow">100-widgets-per-card limit</a>.

<Figure src="/img/docs/google-chat-grouped-alerts.webp" alt="Grouped alerts shown as per-alert sections with a more-alerts note." caption="Grouped alerts: one section per alert, then a '+N more' note." />
...
Send a `POST` request to `/api/v1/channels` with the following `curl` command. Replace `YOUR_API_KEY` with an API key from **Settings → Service Accounts** — see <a href="https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/" target="_blank" rel="noopener noreferrer nofollow">Service Accounts</a> to create one.
