...
- **SigNoz Version**: <a href="https://github.com/SigNoz/signoz/releases/tag/v0.41.0" target="_blank" rel="noopener noreferrer nofollow">v0.41.0</a> or later
...
To create a new Email notification channel, send a `POST` request to the `/api/v1/channels` endpoint with the following `curl` command. Replace `YOUR_API_KEY` with an API key from **Settings → Service Accounts** — see [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) to create one.
...
<Admonition type="note" title="Self-Hosted Only">
**Check the SMTP Server info**: If you are running SigNoz self-hosted, ensure that the SMTP Host, Port, User, and Password entered in SigNoz match the ones provided by your email provider. These are configured via environment variables as described in the [Configuring Alertmanager](#configuring-alertmanager-self-hosted-only) section above.
</Admonition>
