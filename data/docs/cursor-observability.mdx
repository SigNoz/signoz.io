...
<Admonition type="warning" title="Include the /v1/traces path">
On `http/protobuf` the runner passes `OTEL_EXPORTER_OTLP_ENDPOINT` to the exporter unchanged, so it does not append the signal path for you. Without `/v1/traces` every export fails with a 404 that surfaces only as `"last_error": {"type": "ExportFailure"}` in `delivery_health.json`. The logs endpoint is derived from this value, so `/v1/logs` is handled automatically.
</Admonition>
