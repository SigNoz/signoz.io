...
To fix this issue, first disable to schema migrator.
* On k8s you can delete the job.
* On docker you can delete the schema migrator container.

#### Exec into the clickhouse container/pod and run the following commands.
...
* Check the mutations table and clear it.
    ...
    * If some mutation is stuck with error message you can kill it by running.
        ...
        ```
    * If you don't feel the confidence to run it please reach out us on our community [slack](https://signoz.io/slack)
* Run the migrations manually by running each command from the below files on the clickhouse console. If the 10th migration failed then start with 10th, else start with the newer version.
...
* Once the commands run successfully, update the schema migrations table.
    ...
    * Insert data to the migrations table
        ...
        ```
#### Verify if all the migrations are completed
