...
To collect host metrics from your Kubernetes nodes, you need to have the SigNoz `k8s-infra` [Helm chart](https://signoz.io/docs/tutorial/kubernetes-infra-metrics/) installed on your Kubernetes cluster.

### Step 1: Deploy the SigNoz Helm Chart

To collect host metrics from your Kubernetes nodes, use the SigNoz `k8s-infra` [Helm chart](https://signoz.io/docs/tutorial/kubernetes-infra-metrics/). This chart deploys an agent on every node to gather and forward system metrics.

### Step 2: Customize Host Metrics Collection

Host metrics collection is enabled by default and can be customized via the `hostMetrics` preset in your `values.yaml`.
...
### Configure Different Frequencies (Optional)
