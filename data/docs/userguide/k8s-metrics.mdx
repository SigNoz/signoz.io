...
## Learn More About the k8s-infra Helm Chart

- For a complete list of configuration options and details on customizing the OpenTelemetry Collector presets in SigNoz, refer to the [K8s Infra Chart documentation](/docs/metrics-management/k8s-infra-otel-config).
- For a step-by-step installation walkthrough, see the [Kubernetes Infra Metrics and Logs Collection guide](/docs/tutorial/kubernetes-infra-metrics).