...
In the above example there should be two log lines, but by default each since a newline is treated as end of log line, multiple log lines will be created as seen in the image below.
...
2) Recombine the multiline logs later
...
For the above log lines we can see that the new log line starts with `timestamp`, so our `line_start_pattern` will be 
...
There are more options to play around with the multiline operator which can be checked [here](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/pkg/stanza/docs/operators/recombine.md) 