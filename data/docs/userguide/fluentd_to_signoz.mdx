...
description: Send Fluentd logs to SigNoz using the OpenTelemetry Collector.
...
1. **Add OpenTelemetry Collector Binary**
   ...
   Follow this [guide](https://signoz.io/docs/opentelemetry-collection-agents/vm/install/) to add the OpenTelemetry collector binary to your VM.
...
4. **Update FluentD Configuration**
   ...
   ```
   Replace `<otel-collector-host>` with the host where the OpenTelemetry collector is running. For troubleshooting, check [here](/docs/setup/docker/troubleshooting/faq/#signoz-otel-collector-address-grid).
