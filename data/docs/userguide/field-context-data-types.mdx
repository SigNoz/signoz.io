...
If the same field exists in multiple contexts, you'll see this warning:
```
service.name = 'payment'
# Warning: key `service.name` is ambiguous
```

If you want to remove the warning, fix it by specifying the context:
```
...
If you see an ambiguity warning, it typically means a resource attribute is being incorrectly sent as both a resource attribute AND a regular attribute. This is an instrumentation issue that should be fixed.
...
If the same field is sent in multiple contexts, the system applies the following rules:

**Resource vs Attribute Priority** : 
If a field is sent as both a resource attribute and an attribute attribute (e.g., `resource.context` and `attribute.context`), the system prioritizes the resource value.

**Same Field in Multiple Contexts** : 
If a field such as `body` is sent in more than one context (e.g., `attribute.body` and `log.body`), the system will check all provided contexts. In this case, it will search in both `attribute.body` and `log.body`.
</Admonition>
