---
date: 2026-06-04
id: authentication
title: Authentication and Login
description: Understand authentication in SigNoz — supported roles, inviting members, and managing user permissions.
doc_type: explanation
---

<Admonition type="info">
SigNoz now supports fine-grained role-based access control. For detailed information on roles, permissions, and service accounts, see the [Authorization Overview](https://signoz.io/docs/manage/administrator-guide/iam/overview).
</Admonition>

Authentication is available in SigNoz from `v0.8.0` If you are on an earlier version, please upgrade to see this.

## Supported Roles
SigNoz currently supports 3 roles:
- Admin
- Editor
- Viewer

The person who installs SigNoz and registers for the first time is by default assigned the role of an admin.

- If you are logging in SigNoz for the first time, just create an account. This will make you an admin.
- If you would like to invite more members, they would need an invite link from admin. You can create invite link from `Settings-> Org Settings` tab

Admins can add new users by creating invites for them and sending them invite link to create an account.

![user-invite](/img/docs/user-invite.webp)


<Admonition>

As of `version 0.8.0`, news users can only be invited by Admins to a SigNoz instance. New users will get an invite link from admin which they can use to signup to SigNoz.

When new user is invited by an admin, he can be assigned a role of Admin, Editor or Viewer 

</Admonition>

## How to Edit Member Details?

You can edit permission levels of members by going to `Settings-> Org Settings` tab and then Members table. 
You can also generate password reset link if they have forgotten their password and it needs to be reset.

![edit-member](/img/docs/edit-member.webp)

## Password Reset Link Behavior

When a member opens a password reset link, SigNoz validates the token before showing the form. While the token is being checked, a `Validating your reset password token...` spinner is displayed. If the token is valid, the password reset form appears and the member can set a new password.

Password reset links are **single-use** and **expire after a set period**. If the token cannot be used, the form is not shown and one of the following screens appears instead:

- **Reset Password token is expired** — the link has expired. Ask an admin to generate a new password reset link from `Settings -> Org Settings -> Members`.
- **Invalid Reset Link** — the link is invalid or has already been used. Request a new link.
- **Reset Link Unavailable** — a fallback shown for any other token error. Request a new link or contact your admin if the issue persists.

<Figure src="/img/docs/invalid-reset-link.webp" alt="Invalid Reset Link error screen shown when a password reset token is invalid or already used" caption="Error screen shown when a reset password token is invalid or has already been used" />

Opening `/password-reset` without a token redirects to the login page.

<Admonition type="info">
For self-hosted deployments, password reset emails require SMTP to be configured. See [Enable SMTP for Email Invitations](https://signoz.io/docs/manage/administrator-guide/configuration/smtp-email-invitations/).
</Admonition>


## Permission Matrix for Admin, Editor and Viewer:

| Features                                        | Admin | Editor | Viewer |
| ----------------------------------------------- | ----- | ------ | ------ |
| View Application List                           | ✅    | ✅     | ✅     |
| Search/Filter Application List                  | ✅    | ✅     | ✅     |
| View Metrics Detail Page                        | ✅    | ✅     | ✅     |
| View Dashboards                                 | ✅    | ✅     | ✅     |
| Create New Dashboards                           | ✅    | ✅     | ❌     |
| Import Dashboards                               | ✅    | ✅     | ❌     |
| Export Dashboards                               | ✅    | ✅     | ✅     |
| View Trace Filter Page & do Filtering           | ✅    | ✅     | ✅     |
| View Trace Detail Page                          | ✅    | ✅     | ✅     |
| View Alerts                                     | ✅    | ✅     | ✅     |
| Search / Filter Alerts                          | ✅    | ✅     | ✅     |
| Add New Alerts                                  | ✅    | ✅     | ❌     |
| View Settings Page                              | ✅    | ❌     | ❌     |
| View Service Map                                | ✅    | ✅     | ✅     |
| Invite New Members                              | ✅    | ❌     | ❌     |
| Revoke Invite of Members                        | ✅    | ❌     | ❌     |
| Create New Alert Channels                       | ✅    | ❌     | ❌     |
| Set Retention Period                            | ✅    | ❌     | ❌     |
| Set own Password and Name                       | ✅    | ✅     | ✅     |
| Change Org Name                                 | ✅    | ❌     | ❌     |
| Edit / Delete Member Details                    | ✅    | ❌     | ❌     |
| View Version Information Page (from left panel) | ✅    | ✅     | ✅     |
| Change Theme (dark/light)                       | ✅    | ✅     | ✅     |

