---
date: 2026-06-26
title: Log Schema - Write ClickHouse Queries & Build Dashboard Panels
description: Explore the SigNoz logs schema and learn to write ClickHouse queries for timeseries, value, and table panels using resource attributes for optimized performance.
---
...
To write ClickHouse queries for logs, go to **[Dashboards](https://signoz.io/docs/userguide/manage-dashboards/)**, add or edit a panel (for example, Time Series or Table), and select the **ClickHouse Query** tab in the panel editor. The Logs Explorer does not support ClickHouse SQL queries — it uses the Query Builder interface only.
...
The attributes and resources can be added and transformed using different processors and operators. You can read more about them [here](/docs/userguide/logs_query_builder/#operators-for-parsing-and-manipulating-logs)
...
If you use an AI assistant to generate ClickHouse queries, review the output before relying on it. AI-generated queries can return **incorrect or misleading results** (for example, wrong filters, columns, or time range), may be **unoptimized** (leading to slow or expensive scans), and in some cases can lead to **potential data loss**. Always double-check the query and verify that it returns what you expect before using it in a dashboard panel.
</Admonition>
...
#### Count of log lines per minute filtered on an attribute and a resource attribute

Filters: `severity_text = 'INFO'`, `method = 'GET'`, `service.name = 'demo'`. Here `method` is an attribute while `service.name` is a resource attribute.
...
#### Count of log lines per minute using a selected field
...
Same filters as above (`severity_text = 'INFO'`, `method = 'GET'`, `service.name = 'demo'`), except `method` is a [selected field](#selected-indexed-attributes) here, so it's referenced as `attribute_string_method` instead of `attributes_string['method']`.
...
#### Average count of log lines filtered on an attribute and a resource attribute
...
The query is similar to timeseries query but instead of using time interval we just use `now() as ts` in select.

#### Examples

#### Count of log lines grouped by a resource attribute

Filters: `severity_text = 'INFO'`, `method = 'GET'`, grouped by `service.name`. Here `method` is an attribute while `service.name` is a resource attribute.
