...
* Attributes and Resources data type is changed from array to map.
    ...
    * `resources_string_key`, `resources_string_value` is now a single map `resources_string`
    ...
    Eg:- 
        ...
        * `resources_string_value[indexOf(resources_string_key, 'service_name')]` will change to `resources_string['service_name']`
...
WHERE 
    ...
    resources_string['service_name'] = 'demo'
GROUP BY ts 
ORDER BY ts ASC;
```
...
```
<Admonition>
`attributes_string['method']` will change to `attribute_string_method` if `method` is a [selected field](#selected-attributesresources-) and `resources_string['service_name']` will change to `resource_string_service_name` if `service_name` is a [selected field](#selected-attributesresources-).
</Admonition>
...
SELECT 
    now() as ts,
    resources_string['service_name'] as service_name,
    toFloat64(count()) AS value 
FROM 
    signoz_logs.distributed_logs_v2  
WHERE 
...
WHERE 
    (timestamp >= {{.start_timestamp_nano}} AND timestamp <= {{.end_timestamp_nano}}) AND 
    mapContains(resources_string, 'k8s_cluster_name')
...
WHERE 
    ...
    mapContains(resources_string, 'service_name')
GROUP BY service_name,ts 
ORDER BY ts ASC;
```

<Admonition>
`resources_string['service_name']` will change to `resource_string_service_name` if `service_name` is a [selected field](#selected-attributesresources-) and
`mapContains(resources_string, 'service_name')` will change to `resource_string_service_name_exists = true`
</Admonition>
...
<Admonition type="warning" >
If you are using SigNoz version `>= 0.53.0` please follow this guide [here](#logs-schema-v2)
</Admonition>
