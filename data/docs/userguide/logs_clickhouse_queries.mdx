...
To write ClickHouse queries for logs, go to **Dashboards**, add or edit a panel (for example, Time Series or Table), and select the **ClickHouse Query** tab in the panel editor. The Logs Explorer does not support ClickHouse SQL queries — it uses the Query Builder interface only.
