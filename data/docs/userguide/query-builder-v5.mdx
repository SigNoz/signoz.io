...
In the **Logs Explorer**, you can search without specifying a field name. Wrap your search phrase in single quotes and enter it in the filter bar:

```
'database connection timeout'
```

This searches log body content only, and evaluates your text as a regular expression when it compiles as one. For details on quoted vs unquoted behavior, see [Free-Text Log Search](https://signoz.io/docs/userguide/free-text-search/).

To search every field of a log record at once, including attributes, resource attributes, and severity, use `search()`:

```
search('database connection timeout')
```

Both forms are available for logs only. Traces and metrics require field-based filters. See [Full-Text Search](https://signoz.io/docs/userguide/full-text-search/) for scopes and query cost limits.
...
- [Full-Text Search](https://signoz.io/docs/userguide/full-text-search/) — search every field of a log record with `search()`
- [Free-Text Log Search](https://signoz.io/docs/userguide/free-text-search/) — search log bodies without specifying field names
