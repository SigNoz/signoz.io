...
This error no longer applies to **logs** or **traces**. A logs or traces query that references a key missing from field metadata runs against the raw data and returns a warning instead of failing. See [Querying Logs](https://signoz.io/docs/logs-management/querying-logs/#missing-attribute-keys) and [Querying Traces](https://signoz.io/docs/apm-and-distributed-tracing/querying-traces/#missing-attribute-keys). This entry applies to metrics and other signals, which still return `key not found`. If a logs or traces query returns no rows, a typo in the key name is still a likely cause, so check the fixes below.
</Admonition>
