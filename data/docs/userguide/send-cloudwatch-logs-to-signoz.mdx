...
<Admonition title="What is the OpenTelemetry Collector?" defaultCollapsed="true">
...
<Admonition title="Using named profiles?" defaultCollapsed="true">
...
### Logs are not ordered correctly by timestamp

**Symptoms**: CloudWatch logs appear out of order in SigNoz, especially when multiple log lines from the same log stream have timestamps close together (e.g., CI/CD pipeline logs, CodeBuild logs).

**Root cause**: The AWS CloudWatch receiver uses the timestamp provided by CloudWatch as the event timestamp, which often has lower precision than the actual log entries. This can cause logs from the same CloudWatch log stream to appear out of order, especially when multiple log lines share the same CloudWatch timestamp.
...
**Fix**: Use [Pipelines](https://signoz.io/docs/logs-pipelines/introduction/) to parse and extract the timestamp from your log payload:

1. Create a pipeline to parse and extract the timestamp from your log message content
...
For detailed instructions on parsing timestamps, see the [Timestamp Parser documentation](https://signoz.io/docs/logs-pipelines/guides/timestamp/).
...
- [Search and filter logs](https://signoz.io/docs/logs-management/overview/) - Learn to query your CloudWatch logs in SigNoz
- [Create log-based alerts](https://signoz.io/docs/alerts-management/log-based-alerts/) - Set up alerts based on log patterns
- [Correlate logs with traces](https://signoz.io/docs/logs-management/overview/#related-trace) - Connect logs to distributed traces for debugging
