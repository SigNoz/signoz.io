...
description: Learn how to configure and manage alerts in SigNoz to monitor your application and infrastructure, set thresholds, and receive notifications for critical issues.
title: Alerts 
---
...
- **Filter by Tags**: You can apply filters to narrow down the list of triggered alerts based on specific tags. For example, filter by `severity:warning` to see only warning-level alerts.
...
- **[Anomaly-based Alert](https://signoz.io/docs/alerts-management/anomaly-based-alerts)**: Sends a notification when a condition occurs in metric data (e.g., CPU usage, memory utilization, request rates). You can set thresholds or rate-based conditions.

- **[Metric-based Alert](https://signoz.io/docs/alerts-management/metrics-based-alerts)**: Sends a notification when a condition occurs in metric data (e.g., CPU usage, memory utilization, request rates). You can set thresholds or rate-based conditions.

- **[Log-based Alert](https://signoz.io/docs/alerts-management/log-based-alerts)**: Sends a notification when a condition occurs in log data (e.g., specific patterns, keywords, error messages). You can set conditions based on log entries or error codes.

- **[Trace-based Alert](https://signoz.io/docs/alerts-management/trace-based-alerts)**: Sends a notification when a condition occurs in trace data (e.g., latency, errors, specific trace events). You can define conditions to trigger the alert based on distributed system traces.

- **[Exceptions-based Alert](https://signoz.io/docs/alerts-management/exceptions-based-alerts)**: Sends a notification when a condition occurs in exceptions data (e.g., application exceptions or errors). You can set conditions to trigger the alert when specific exceptions are detected.

These five types of alerts offer flexibility in monitoring different system aspects. 

### Repeat Notification
Configure repeat notifications to retrigger alerts at specified intervals if they remain unresolved. This feature allows you to receive follow-up notifications for ongoing issues.

To enable repeat notifications:
1. Scroll to the bottom of the alert configuration
2. Look for the "Repeat Notification" option
...
You can manage alert rules programmatically using the SigNoz API. Use `GET /api/v1/rules` to retrieve all alert rules and `POST /api/v1/rules` to create a new alert rule. This is useful for transferring alerts between SigNoz instances — retrieve the rules from one instance and create them on another.
...
See the [SigNoz API reference](https://signoz.io/api-reference/) for full details on available endpoints and request formats.
