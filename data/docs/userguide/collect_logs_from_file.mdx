...
The chart has [presets.logsCollection.enabled](https://signoz.io/docs/collection-agents/k8s/k8s-infra/configure-k8s-infra/#3-container-log-collection) property to **true** which adds a `filelogreceiver`. 
This receiver is configured to read the files where Kubernetes container runtime writes all containers console output **(/var/log/pods/*/*/*.log)**.

## Application running on Docker

If you application is running on Docker, you can follow [this doc](https://signoz.io/docs/userguide/collect_docker_logs/) to get you container logs into SigNoz 
