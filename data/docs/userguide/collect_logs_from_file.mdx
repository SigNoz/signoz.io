...
For production use, especially when using `start_at: beginning`, configure a <a href="https://opentelemetry.io/docs/collector/storage/" target="_blank" rel="noopener noreferrer nofollow">storage extension</a> to prevent log duplication on collector restarts. The storage extension saves the read position for each file to disk.
