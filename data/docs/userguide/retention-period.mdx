...
<Admonition>
When retention settings are updated, the changes apply only to newly ingested data. Existing data retains the previous retention settings, as updating retention for existing data is a resource-intensive process. Once data reaches its TTL, it is permanently deleted and cannot be recovered. Increasing the retention period later does not restore previously expired data.
</Admonition>
