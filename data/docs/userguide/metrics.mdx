...
You can add the `service.namespace` and `deployment.environment` attributes to your applications and filter the Services list by them.
...
<Admonition type="info" title="Self-hosted Kubernetes (Helm): add more filter dimensions" defaultCollapsed="true">
Self-hosted deployments on Kubernetes can add more filter dimensions to the Services section.

1. Send the attribute as a resource attribute on your spans.
2. Add the attribute name to the `dimensions` list in your Helm values. List every dimension you keep, because Helm replaces the list.
    ...
    processors:
      signozspanmetrics/delta:
        dimensions:
          ...
          - name: team # your custom dimension
```

3. Apply the values file:

```bash
helm upgrade <release-name> signoz/signoz -n <namespace> -f values.yaml
```
