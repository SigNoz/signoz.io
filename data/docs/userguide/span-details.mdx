...
- **Span:** Spans are the building blocks of a trace, representing individual units of work performed by an application or service during the request's lifecycle. Key components of a span include:
  ...
  - **Span Links:** References that establish causal relationships between spans, useful for correlating asynchronous operations or background tasks with the main trace. See [Span Links](https://signoz.io/docs/traces-management/guides/span-links/) for when to use them and how to instrument them.
...
Hovering over a span row in the waterfall also reveals a **link icon**. Click it to copy a shareable URL to your clipboard. Opening that URL takes the viewer directly to the trace with the specific span focused, making it easy to share a particular span with teammates.
