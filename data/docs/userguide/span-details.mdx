...
- **Visualize the Entire Trace:** Render large traces seamlessly using synchronized <Tooltip text="flame graphs" content="A visualization of a trace that shows each span as a horizontal bar positioned by start time and sized by duration, making the critical path and the slowest child operations obvious." link="https://signoz.io/docs/glossary/#flamegraph" /> and waterfall charts.
...
- **Trace:** A <Tooltip text="trace" content="The full record of one request's path through a system: a tree of spans connected by parent-child relationships, sharing a trace ID." link="https://signoz.io/docs/glossary/#trace" /> represents the complete path a request takes through your system, helping you understand its flow across various components and services.
- **Span:** <Tooltip text="Spans" content="A single operation within a trace: an HTTP handler, a database query, or a queue publish." link="https://signoz.io/docs/glossary/#span" /> are the building blocks of a trace, representing individual units of work performed by an application or service during the request's lifecycle. Key components of a span include:
  - **Span Context:** The <Tooltip text="span context" content="The part of a span that crosses process boundaries: the trace ID, the span ID, trace flags, and trace state." link="https://signoz.io/docs/glossary/#span-context" /> is the metadata that links spans across services, enabling <Tooltip text="distributed tracing" content="Following a single request across every service, database call, and queue hop it touches, by stitching spans into a trace." link="https://signoz.io/docs/glossary/#distributed-tracing" /> and hierarchical structuring.
  - **Span Attributes:** A <Tooltip text="span attribute" content="A key-value pair that annotates a span with information about the operation it records." link="https://signoz.io/docs/glossary/#attribute" /> is a key-value pair that stores additional metadata about the operation performed by the span.
  - **Span Links:** A <Tooltip text="span link" content="A pointer from one span to another causally related span, in the same trace or a different one." link="https://signoz.io/docs/glossary/#span-link" /> establishes a causal relationship between spans, useful for correlating asynchronous operations or background tasks with the main trace. See [Span Links](https://signoz.io/docs/traces-management/guides/span-links/) for when to use them and how to instrument them.
  - **Span Events:** A <Tooltip text="span event" content="A timestamped annotation on a span, used for things that happen at a moment rather than over a duration: a retry, a cache miss, a recorded exception." link="https://signoz.io/docs/glossary/#span-event" /> is a structured log message that records an important event at a specific timestamp during the span's execution.
