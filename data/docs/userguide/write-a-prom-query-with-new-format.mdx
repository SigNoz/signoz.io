...
PromQL only supports querying **metrics**, in **Dashboards** and in **metric alerts**. In a dashboard, add a panel and select the **PromQL** tab. In an alert, create a **Metric based Alert** and select the **PromQL** tab. To query logs and traces, use the [Query Builder](https://signoz.io/docs/userguide/query-builder-v5/) or [ClickHouse SQL](https://signoz.io/docs/userguide/writing-clickhouse-traces-query/).

PromQL reads metrics stored with cumulative or unspecified temporality. A delta metric returns no data through PromQL, so use the Query Builder for those.
</Admonition>
...
- [PromQL Alert Examples](https://signoz.io/docs/alerts-management/user-guides/promql-alert-examples/)
