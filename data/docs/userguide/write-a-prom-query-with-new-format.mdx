...
We're excited to announce that our monitoring platform now supports **UTF-8 characters in Prometheus metric and label names**! This enhancement enables compatibility with OpenTelemetry and other modern observability standards that use characters like dots (`.`) in metric names.

## What's Changed

Due to our ongoing migration to support UTF-8 metric names, **PromQL queries will now return results in a new format**. This change allows us to handle metrics with previously unsupported characters while maintaining full backward compatibility.
