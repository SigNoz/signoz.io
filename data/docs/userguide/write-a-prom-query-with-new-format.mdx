...
The regex matchers `=~` and `!~` are fully anchored, following standard Prometheus semantics. This means `=~"foo"` is evaluated as `=~"^foo$"` and must match the **entire** label value, not just a substring.
