...
For example, if you have grouped by `serviceName` attribute then you can write `{{serviceName}}` in your legend. You can also add text along with the attribute like `{{serviceName}} - This is a service` will be shown as `sampleService - This is a service name`.
