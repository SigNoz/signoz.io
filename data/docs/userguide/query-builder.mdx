...
date: 2026-05-18
id: query-builder
title: Query Builder Guide - Filter & Aggregate Data
description: Learn to use SigNoz Query Builder for filtering, aggregating, and visualizing logs, traces, and metrics with multi-query support and spatial-temporal aggregations.
---
...
For example, if you have grouped by `serviceName` attribute then you can write `{{serviceName}}` in your legend. You can also add text along with the attribute like `{{serviceName}} - This is a service` which will be shown as `sampleService - This is a service name`.
...
To extend the data analysis capabilities of the Metrics Query Builder, you can use the following categories of functions.
...
- **Fill Zero:** Replaces missing data points with zero. This is useful for sparse or event-only metrics that only report values when a specific condition occurs, such as a failure counter. Without `Fill Zero`, time intervals with no reported data appear as gaps in charts. This function is the Query Builder equivalent of the `or vector(0)` pattern in PromQL.
