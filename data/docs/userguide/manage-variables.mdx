...
<Figure
  src="/img/docs/dashboards/variables/variable-configuration-v2.webp"
  alt="Variable editor in the Variables tab of the dashboard configuration drawer, showing the variable type tabs, source, sort, default value, and Apply to panels"
  caption="The variable editor: set the name, type, source, default value, and the panels the variable applies to."
/>
...
- **Query** is marked **Not Recommended**, because ClickHouse query variables can be slow and brittle. Prefer a Dynamic variable where one fits. See [Why avoid ClickHouse query variables](#why-avoid-clickhouse-query-variables).
...
This helps you quickly focus on relevant data within your selected context. The two sections appear only when a sibling dynamic variable has an active selection. Without one, the dropdown shows a single flat list with no section headings.

    ![Related values for service](/img/docs/dashboards/variables/service-related-values.png)

#### Search a large value list

When an attribute has more values than SigNoz can return in one response, the dropdown loads a first batch and marks the list as incomplete, so you know not every value is shown.

To reach a value outside that first batch, type in the dropdown's search box. For an incomplete list, SigNoz sends the search term to the API and replaces the options with matching results, including values that were not in the initial batch. For a list returned in full, the search box filters the already-loaded values on the client.

<Admonition type="info">
If a dynamic variable shows an error with no **Retry** button, the variable query itself is invalid, for example an unknown attribute name or an unsupported signal type. Retrying would send the same request, so no retry is offered. Fix the field or source in the variable editor. A **Retry** button still appears for server-side errors, which can succeed on a second attempt.
</Admonition>
...
<Figure
  ...
  alt="The variables bar pinned at the top of a scrolled dashboard, showing five variables and a +13 overflow chip"
  caption="The variables bar stays pinned as you scroll, so you can change a selection from anywhere on the dashboard."
/>
...
- **Add a variable inline**: The **+** button at the end of the bar opens the variable editor directly, so you can add a variable without going through **Configure** → **Variables**.
...
<Admonition type="info">
**Apply to panels** is available for Dynamic variables. For Query, Custom, and Textbox variables, reference the variable in the panel query yourself. See [Using Variables in Queries](https://signoz.io/docs/dashboards/using-variables-in-queries/).
</Admonition>
...
This is the same review dialog used when you [rename or delete a referenced variable](#rename-or-delete-a-referenced-variable).
...
<Figure
  src="/img/docs/dashboards/variables/variable-impact-dialog.webp"
  alt="Delete impact dialog listing every usage of a variable with Current and Result query text and a checkbox per row"
  caption="The impact dialog before a delete. Each row shows the current query, an editable proposed result, and a checkbox to include or exclude that usage."
/>
