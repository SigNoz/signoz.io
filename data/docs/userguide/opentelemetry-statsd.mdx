...
[StatsD](https://github.com/statsd/statsd) is a popular network daemon that listens for statistics (like counters, timers, and gauges) sent over UDP or TCP and aggregates them before forwarding to a backend. You can send custom metrics from your applications using StatsD clients to SigNoz by configuring the OpenTelemetry Collector's `statsd` receiver.
...
- You have installed the OpenTelemetry Collector. If you haven't already, follow the [installation instructions](https://signoz.io/docs/opentelemetry-collection-agents/get-started/) for your environment
...
The OpenTelemetry Collector uses the [StatsD receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/statsdreceiver) to ingest metrics in the StatsD format.
...
- `enable_metric_type`: When `true`, it preserves the original StatsD metric type (counter, gauge, timer) as a resource attribute or metric description.
...
- `timer_histogram_mapping`: Specifies what OpenTelemetry data type to convert received timing or histogram data into. In the example above:
  ...
  See the [StatsD receiver documentation](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/statsdreceiver#configuration) for more mapping options.
...
2. Add a new panel and select the **Query Builder**.
3. Under the metric dropdown, search for `custom_test_metric`. Keep in mind that OpenTelemetry converts dot separators to underscores.
4. If you see data points, your setup is complete and working correctly!
