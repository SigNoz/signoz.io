...
SigNoz Cloud Account: Create an account on SigNoz Cloud
Ingestion Key: Obtain your ingestion key from the SigNoz Cloud dashboard

### 2: For Self-Hosted SigNoz

Running Instance of SigNoz: Follow these steps to [install](https://signoz.io/docs/install/docker/) and run a self-hosted instance of SigNoz.
...
`OTLPLogExporter` defines the endpoint where logs are exported. It uses the environment variables `OTEL_EXPORTER_OTLP_ENDPOINT` and `SIGNOZ_ACCESS_TOKEN`
