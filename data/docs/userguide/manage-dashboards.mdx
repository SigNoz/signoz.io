...
SigNoz supports seven panel types: Time Series, Number, Table, List, Bar, Pie, and Histogram. All panel types visualize query results. There is no dedicated text or markdown panel for free-form notes. To add context or notes for dashboard viewers, use the dashboard-level Description field (set via the **Configure** icon) or individual panel Description fields.

To add a panel to a dashboard, follow the steps below:
