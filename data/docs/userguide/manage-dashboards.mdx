...
You can seed a new alert directly from a panel's query, so you don't have to rebuild the query in the alert builder. The action opens the alert builder in a **new browser tab** with the panel's query pre-loaded, including its formatting unit (for example, bytes or milliseconds).
...
1. Hover over the panel and select the **...** (three-dot) icon in the panel header.
2. Choose **Create Alerts**.
...
1. Open the panel editor by selecting the pencil icon on the panel (or **Edit** from the three-dot menu).
2. In the right-hand config pane, scroll to the **Actions** section at the bottom.
3. Select **Create alert**.

The alert builder opens with the panel's current draft query, including any in-progress changes you haven't saved to the dashboard yet.

Either entry point lands on the alert builder with the query, panel type, and unit already applied. Set your threshold and notification channel to finish creating the alert.

<Figure
  src="/img/docs/userguide/manage-dashboards/create-alert-seeded-builder.webp"
  alt="SigNoz alert builder pre-loaded with a panel query and its Nanoseconds unit"
  caption="The alert builder opens in a new tab with the panel query plotted and its formatting unit applied."
/>

### Supported panel types

Alert creation is available for panel types whose query can seed an alert: **Time Series**, **Bar**, **Number**, and **Histogram**. The action is hidden for **Table**, **List**, and **Pie** panels.

<Admonition type="info">
Dashboard variables are not substituted into the seeded alert query. Any variable references in the panel query pass through to the alert builder verbatim, so replace them with concrete values before saving the alert.
</Admonition>
