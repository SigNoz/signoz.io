...
<Admonition type="info">
`updated_by` and `source` can be displayed as columns but cannot be used as [filter keys](#filter-keys).
</Admonition>
...
## Row Actions and Access on the Dashboards List

Every user sees the per-row actions menu, and each entry checks its own transactions on that dashboard. An action you cannot use stays visible but disabled, and hovering it names what is missing.

**Available to everyone who can open the list:**
...
| Action | Transactions |
|--------|--------------|
| **Rename** | `read` and `update` on the dashboard |
| **Add Tags** or **Edit Tags** (see [Manage Dashboard Tags](#manage-dashboard-tags)) | `read` and `update` on the dashboard |
| **Duplicate** | `read` on the dashboard and `create` |
| **Lock Dashboard** or **Unlock Dashboard** | `read` and `update` on the dashboard |
| **Delete** | `delete` on the dashboard |

The SigNoz-Editor and SigNoz-Admin roles hold all of these for every dashboard, and SigNoz-Viewer holds none of them. To grant them per dashboard, see [Restrict Dashboard Access](https://signoz.io/docs/manage/administrator-guide/iam/user-guides/restrict-dashboard-access/).

The list itself needs `list`, which covers the whole workspace and is not filtered by `read`. A dashboard you cannot read still appears as a row, and opening it reports the denial.

On a locked dashboard, **Rename** and the tag actions are disabled, and hovering them explains why. Unlock the dashboard first to use them. An integration dashboard can never be locked or unlocked.
...
<Figure
  src="/img/docs/userguide/manage-dashboards/dashboard-options-menu-v2.webp"
...
- **Lock dashboard** or **Unlock dashboard**: prevent accidental edits. When locked, panels cannot be added, removed, resized, or rearranged. Needs `read` and `update` on the dashboard, and the API accepts the change only from the dashboard's creator or a SigNoz-Admin. Integration and system dashboards can never be locked or unlocked.
...
<Figure
  src="/img/docs/userguide/manage-panels/new-panel-dialog-v2.webp"
