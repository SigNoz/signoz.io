...
OpenTelemetry .NET gives several ways to report an exception on an `Activity`. The simplest way sets only the status. The <a href="https://opentelemetry.io/docs/languages/dotnet/instrumentation/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry .NET documentation</a> gives the other ways.
