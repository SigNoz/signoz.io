...
A span is missing when its parent span never reached SigNoz. The span still exists in your system, but nothing in the collected trace matches its Parent Span ID, so the waterfall view shows a gap where the parent should sit. SigNoz marks these traces with a "This trace has missing spans" banner in the trace details view.
