...
<Admonition type="info">
Access to quick filters is controlled by the **Quick Filters** role permission. Viewing the panel requires `quick-filter:read` and editing filters requires `quick-filter:update`. By default the Admin role can edit, while the Viewer and Editor roles can view but not edit. Admins can grant these permissions to any role from **Settings > Roles**. See the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/#quick-filters).
</Admonition>
