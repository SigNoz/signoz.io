...
Use this page to understand what each panel shows and confirm your application emits the attributes the dashboard needs.
...
AI Observability is available to every user on SigNoz Cloud and self-hosted, with no feature flag or setup step required. The section is still labeled **Beta** in the side navigation while it stabilizes.
</Admonition>
...
2. Select **AI Observability** (Brain icon, marked with a **Beta** badge).
