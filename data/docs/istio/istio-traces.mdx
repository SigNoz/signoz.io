...
![Services List in SigNoz](./screenshots/istio-services-list.png)
...
![Trace Explorer in SigNoz](./screenshots/istio-trace-explorer.png)

### Trace Detail View

Clicking on a specific trace shows the detailed view with the span hierarchy and timing:

### Flamegraphs

The Flamegraph view helps visualize the time spent in each service and operation:
...
for i in {1..50}; do
  curl -s -H "X-B3-Sampled: 1" http://$(kubectl get svc istio-ingressgateway -n istio-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')/productpage
  sleep 1
done
```
