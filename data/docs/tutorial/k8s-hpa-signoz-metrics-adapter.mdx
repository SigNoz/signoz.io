...
- [SigNoz Cloud](https://signoz.io/teams/) running with at least one metric ingested, and a service account API key
- Your pods must send metrics to SigNoz with `k8s.pod.name` set as a resource attribute
  ...
  - For custom app metrics, configure your OTel SDK or Collector to include `k8s.pod.name` as a resource attribute
...
- `<your-signoz-url>` — your SigNoz Cloud instance URL, e.g. `https://<your-instance>.signoz.cloud`
...
| `filterExpression` | Optional SigNoz filter expression to scope metric values             |

To find available metric names, open **Metrics Explorer** in SigNoz.

## Step 4: Deploy the adapter

The adapter image is published to GHCR. Check the <a href="https://github.com/orgs/brainpodnl/packages?repo_name=signoz-metrics-adapter" target="_blank" rel="noopener noreferrer nofollow">GHCR package page</a> for the latest tag and replace `<latest-tag>` below:

```bash
helm upgrade --install signoz-metrics-adapter helm/ \
  --namespace signoz-metric-adapter \
  --values my-values.yaml \
  --set steiger.adapter.image=ghcr.io/brainpodnl/signoz-metrics-adapter/adapter:<latest-tag>
```
