...
- Your pods must send metrics to SigNoz with `k8s.pod.name` set as a resource attribute
  - The [SigNoz k8s-infra Helm chart](https://signoz.io/docs/tutorial/kubernetes-infra-metrics/) handles this automatically via an OTel Collector with Kubernetes resource detection
...
kubectl create secret generic signoz-credentials \
  ...
  --from-literal=url=https://<your-signoz-url> \
