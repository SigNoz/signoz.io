...
To export Kubernetes metrics, enable receivers in OpenTelemetry collector to send metrics about your Kubernetes infrastructure to SigNoz. These collectors act as agents for this purpose.

OtelCollector can also tail and parse logs from containers using the `filelog` receiver, sending them to the desired receiver.

`K8s-Infra` helm chart mainly:
- Tails and parses logs from Kubernetes containers, sending to SigNoz
- Collects kubelet and host metrics from each Kubernetes node
- Collects cluster metrics from the API server
- Acts as a gateway for OTLP telemetry data to SigNoz OtelCollector.

To gain more understanding of the k8s-infra, please check out the [doc](/docs/infrastructure-monitoring/k8s-infra-architecture)
