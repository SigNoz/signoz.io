...
Install the follwing OpenTelemetry packages:
...
**Step 2.** Set Environemnt Variables and run application<br></br>

Use the below command to set the OpenTelemetry environment variables and run your node app:
...
`<your-app-name>` - Name of your service

`<SIGNOZ_INGESTION_KEY>` - The ingestion key provided by SigNoz. It can be found out in the cloud account details sent to you over email.
...
**Step 3.** You can validate if your application is sending traces to SigNoz cloud [here](#validating-instrumentation-by-checking-for-traces).
...
In case you have OtelCollector Agent in different VM, replace localhost:4317 with` <IP Address of the VM>:4318`.

We use the port`4317` for gRPC exporter and `4318` for HTTP exporter.
