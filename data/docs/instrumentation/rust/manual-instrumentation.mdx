...
<Admonition type="warning">
Do not call `cx.attach()` inside a spawned task that awaits. `attach` returns a `ContextGuard` that is deliberately `!Send`. Holding it across an `await` makes the future `!Send`, and `tokio::spawn` then rejects it at compile time with "future cannot be sent between threads safely". `with_context` attaches the context around each poll instead, so no guard is held across the await.
</Admonition>
...
Use <Tooltip text="span links" content="A pointer from one span to another causally related span, in the same trace or a different one." link="https://signoz.io/docs/glossary/#span-link" /> to connect causally related traces, such as batch processing where one span triggers multiple independent operations:
