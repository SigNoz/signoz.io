...
- `OTEL_SERVICE_NAME` - Name of your service as it will appear in SigNoz
- `OTEL_EXPORTER_OTLP_ENDPOINT` - SigNoz Cloud endpoint for your region
- `OTEL_EXPORTER_OTLP_HEADERS` - Authentication header with your SigNoz ingestion key  
- `OTEL_EXPORTER_OTLP_PROTOCOL` - Protocol used to send telemetry data (grpc)
...
### Step 5. Validate if your application is sending traces to SigNoz cloud

Follow the instructions [here](#validating-instrumentation-by-checking-for-traces).
...
- **Operation details** like model name, temperature, max tokens
- **Token usage** including input, output, and total tokens consumed
...
- **Request/Response logs** containing model parameters, token usage, and response metadata
...
**What gets logged:**
- API call start and completion events
- Model parameters (temperature, max_tokens, etc.)
- Token usage statistics (input, output, total tokens)
- Request and response timing
- Error messages and exception details
- When content capture is enabled: actual prompts sent and completions received
