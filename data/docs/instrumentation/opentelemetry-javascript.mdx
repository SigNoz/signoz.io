...
function generateLinkedLog(severity, message) {
  const logMessage = {
...
const span = tracer.startSpan('span-with-log');
context.with(trace.setSpan(context.active(), span), () => {
  generateLinkedLog('INFO', 'Log message linked with trace and sent to SigNoz');
  span.end();
});
```
