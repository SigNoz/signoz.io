...
Trigger requests in your app to generate traces… then check the Services and Traces tabs in SigNoz after refreshing to confirm your application shows up.

If traces don’t appear right away, interact with the app a few more times and wait a bit since OpenTelemetry buffers data before sending.

## Collecting Logs from Your Node.js Application

Node.js doesn’t ship with a full-fledged logging framework, but popular libraries like Winston and Pino make it easy to add structured logging.
