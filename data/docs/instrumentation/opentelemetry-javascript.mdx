...
![An image displaying linked logs and traces to SigNoz](images/logs-traces-js.png)
...
const exporter = new OTLPTraceExporter({ url: '<signoz-traces-endpoint>' });
...
function getTraceId() {
  ...
  return span ? span.spanContext().traceId : '00000000000000000000000000000000';
}

function getSpanId() {
  const span = trace.getSpan(context.active());
  return span ? span.spanContext().spanId : '0000000000000000';
}
```
...
function getCoffee(coffeeId) {
  const span = tracer.startSpan('getCoffee');
  try {
    if (!coffeeId) {
      generateLinkedLog('INFO', 'Coffee ID not found');
...
const logExporter = new OTLPLogExporter({ url: '<signoz-log-endpoint>' });
...
function generateLog() {
  logger.emit({ severityText: 'INFO', body: 'Example log message', attributes: { key: 'value' } });
}

function logRequest(method, url) {
  const span = trace.getSpan(context.active());
  logger.emit({ severityText: 'INFO', body: `Incoming request: ${method} ${url}`, attributes: { trace_id: getTraceId() } });
}

function logResponse(url, statusCode) {
  const span = trace.getSpan(context.active());
  logger.emit({ severityText: 'INFO', body: `Response: ${statusCode} for ${url}`, attributes: { trace_id: getTraceId() } });
}
```
...
const SIGNOZ_INGESTION_KEY = '<SigNoz-ingestion-key>';
const SIGNOZ_LOGS_URL = 'https://ingest.{region}.signoz.cloud:443/logs/json';

function sendLogToSigNoz(severity, message) {
  const logMessage = {
    severityText: severity,
    body: message,
    attributes: { trace_id: getTraceId(), span_id: getSpanId() }
  };
  logger.emit(logMessage);
  console.log('Sending log to SigNoz:', JSON.stringify(logMessage));
}

 try {
        const response = await fetch(SIGNOZ_LOGS_URL, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            console.error("Failed to send log to SigNoz:", response.statusText);
        } else {
            console.log("Log successfully sent to SigNoz.");
        }
    } catch (error) {
