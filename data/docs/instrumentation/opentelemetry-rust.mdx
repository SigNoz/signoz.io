...
- `<service-version>`: Your release version, image tag, or git SHA (e.g., `1.4.2`, `a01dbef8`).

<Admonition type="tip" title="Recommended: Set service.version for Deployment Markers">
Set `service.version` to your release version, image tag, or git SHA (e.g., `1.4.2`, `a01dbef8`). SigNoz uses this to automatically detect deployments and show change markers on your dashboards.
</Admonition>
...
fn init_tracer() -> SdkTracerProvider {
    ...
    // Resource::builder() does not read OTEL_RESOURCE_ATTRIBUTES automatically;
    // parse it and forward all key=value pairs to the resource.
    let resource_attrs: Vec<opentelemetry::KeyValue> = std::env::var("OTEL_RESOURCE_ATTRIBUTES")
        .unwrap_or_default()
        .split(',')
        .filter_map(|kv| {
            let (k, v) = kv.trim().split_once('=')?;
            Some(opentelemetry::KeyValue::new(k.trim().to_owned(), v.trim().to_owned()))
        })
        .collect();
