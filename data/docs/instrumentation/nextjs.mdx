...
While creating the `otel-config.yaml` during the installation fo the OTel Collector Binary, you need to enable CORS under the receivers section of the config file. This is needed so that you don't get 
...
**Step 3.** In the `tracing.js` file, remove the headers from the exporterOptions since you're already using the signoz access token in the `otel-config.yml` file.
...
If you're trying to send instrumentation data to SigNoz self-hosted way, the only minor thing that you'd be required is to change the `exporterOptions` in the `tracing.js` file
...
Once you're done with this, add your required changes in the `otel-config.yml` file which can be generally found [here](https://github.com/SigNoz/signoz/blob/develop/deploy/docker/clickhouse-setup/otel-collector-config.yaml).
