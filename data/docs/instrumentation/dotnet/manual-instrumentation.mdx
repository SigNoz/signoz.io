...
```csharp:Consumer.cs defaultCollapsed
using System.Linq;
...
Read the property at the point of use, as above. The SDK swaps in a different propagator object rather than mutating the existing one. A `static readonly` field initialized before startup therefore holds the no-op forever, and every later inject writes nothing.
</Admonition>
