---
date: 2026-08-26
title: Nuxt.js OpenTelemetry Instrumentation Guide
description: Learn how to instrument your Nuxt.js application with OpenTelemetry and send traces to SigNoz. Use auto or code-level setup for real-time monitoring.
doc_type: howto
---
...
Verify these values:
- `<service-name>`: A descriptive name for your service (e.g., `my-nuxtjs-app`).
- `<service-version>` (optional): Your release version, image tag, or git SHA (e.g., `1.4.2`, `a01dbef8`).
...
Verify these values:
- `<service_name>`: Your service name (e.g., `nuxt-app`).
- `<service-version>` (optional): Your release version, image tag, or git SHA (e.g., `1.4.2`, `a01dbef8`).
- `<region>`: Your [SigNoz Cloud region](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint).
- `<your-ingestion-key>`: Your SigNoz [ingestion key](https://signoz.io/docs/ingestion/signoz-cloud/keys/).
...
```js:server/tracing.js defaultCollapsed
'use strict'
...
```ts:server/tracing.ts defaultCollapsed
