...
<Admonition type="info" title="Why do I see two spans per network request?">
React Native's `fetch()` is built on top of `XMLHttpRequest`. Since both are instrumented, each network request produces two spans — one from `FetchInstrumentation` and one from `XMLHttpRequestInstrumentation`. This is a known characteristic of React Native's networking architecture, not a bug in the instrumentation.
</Admonition>
...
React Native's iOS networking layer (NSURLSession) appends a trailing slash to URLs. The OTLP exporter sends to `/v1/traces/` instead of `/v1/traces`, and the collector returns 404.
