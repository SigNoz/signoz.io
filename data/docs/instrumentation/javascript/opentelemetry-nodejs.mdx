...
Import the tracing file at the top of your entry point, before any other import:
...
Then run your app with <a href="https://tsx.is/" target="_blank" rel="noopener noreferrer nofollow">tsx</a>, which executes TypeScript directly and needs no build step or `tsconfig.json`:
...
Avoid `ts-node` here. On current TypeScript releases it fails before your app starts with `TypeError: Cannot read properties of undefined (reading 'fileExists')`.
</Admonition>

If you would rather compile ahead of time, add a `tsconfig.json` with an `outDir` and run the build output:
...
Without a `tsconfig.json`, `npx tsc` emits no files and `node dist/index.js` fails with `MODULE_NOT_FOUND`.
