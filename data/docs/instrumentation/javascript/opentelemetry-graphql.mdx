...
- If you're on Node 18, use <a href="https://github.com/open-telemetry/opentelemetry-js/blob/main/doc/upgrade-to-2.x.md#-nodejs-supported-versions" target="_blank" rel="noopener noreferrer nofollow">18.19.0+</a>
...
<KeyPointCallout title="VM deployment targets" defaultCollapsed="true">
Use the VM path for:
...
3. Open a trace. The trace includes:
   - A span named after the operation type: `query`, `mutation`, or `subscription`. If the operation is named (e.g. `query GetUser { ... }`), the span name is `query GetUser`.
   - `graphql.parse` and `graphql.validate` spans for parsing and validation.
   - `graphql.resolve` spans nested underneath the operation span, one per field resolved.

The operation span carries these attributes:
...
Resolver spans appear as children of the operation span, one per field resolved. Field path is encoded in the span name (e.g. `graphql.resolve users.0.name`).
