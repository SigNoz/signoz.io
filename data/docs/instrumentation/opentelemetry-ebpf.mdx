...
Send a few requests to your instrumented service so OBI has traffic to capture. In OBI's logs, confirm an `instrumenting process ... service=<name>` line and no export errors.
...
Mount `/var/run/obi` as writable storage for OBI runtime state. Mount host `/sys/fs/cgroup` so OBI can inspect cgroup metadata. For the capability mapping, see the <a href="https://opentelemetry.io/docs/zero-code/obi/security/" target="_blank" rel="noopener noreferrer nofollow">OBI security and capabilities guide</a>.
