...
description: Auto-instrument applications with eBPF using OpenTelemetry eBPF Instrumentation (OBI) and send metrics and traces to SigNoz with no code changes.
doc_type: howto
---
...
- **Request metrics** (rate, errors, duration) for HTTP, HTTPS, and gRPC traffic.
...
<Figure
  ...
  alt="OBI-instrumented service with request rate, error rate, and latency on the SigNoz Services page"
  caption="Request rate, error rate, and latency for an OBI-instrumented service, with no code changes"
/>
...
- [Create dashboards](https://signoz.io/docs/dashboards/dashboard-templates/overview/) from OBI's metrics.
