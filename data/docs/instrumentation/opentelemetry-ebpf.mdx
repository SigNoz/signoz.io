...
Tell OBI which process to instrument with one of these selectors, then reuse it in the steps below:
...
The Docker and Linux host examples use `OTEL_EBPF_OPEN_PORT`. The Kubernetes DaemonSet example uses `OTEL_EBPF_AUTO_TARGET_EXE` for node-wide process discovery.
...
Run OBI as a sidecar container alongside the container you want to observe. OBI needs the host PID namespace and elevated privileges to load eBPF probes.
...
Deploy OBI as a DaemonSet so one instance runs on each node. Use this manifest to set host PID access, grant RBAC read access for pod metadata, and read the ingestion key from a Secret.
