...
![An image displaying linked logs and traces to SigNoz](images/logs-trace-spring-boot.png)
...
public class OpenTelemetryConfig {
    ...
    public static String getTraceId() {
        ...
        return spanContext.isValid() ? spanContext.getTraceId() : "00000000000000000000000000000000";
    }

    public static String getSpanId() {
        Span currentSpan = Span.current();
        SpanContext spanContext = currentSpan.getSpanContext();
        return spanContext.isValid() ? spanContext.getSpanId() : "0000000000000000";
    }
}
```
...
```java
public String getCoffee(String coffeeId) {
    Span span = OpenTelemetryConfig.startSpan("getCoffee");
    try {
        if (coffeeId == null) {
            logMessage("INFO", "Coffee ID not found");
            return "Coffee not found";
        }
        return "Serving coffee: " + coffeeId;
    } finally {
        span.end();
    }
}
...
public class LoggingUtils {
    ...
    public static void logRequest(String method, String url) {
        ...
        logger.info("Incoming request: {} {} - Trace ID: {}", method, url, span.getSpanContext().getTraceId());
    }

    public static void logResponse(String url, int statusCode) {
        Span span = Span.current();
        logger.info("Response: {} for {} - Trace ID: {}", statusCode, url, span.getSpanContext().getTraceId());
    }
}
```
...
public class SigNozLogger {
    private static final Logger logger = LoggerFactory.getLogger(SigNozLogger.class);

    private static final String SIGNOZ_INGESTION_KEY = "<SigNoz-ingestion-key>";
    private static final String SIGNOZ_LOGS_URL = "https://ingest.{region}.signoz.cloud:443/logs/json";
    ...
    public static void sendLog(String level, String message) {
        ...
        logger.info("{} - {} - Trace ID: {}, Span ID: {}", level, message, traceId, spanId);

        JSONObject logEntry = new JSONObject();
        logEntry.put("trace_id", traceId);
        logEntry.put("span_id", spanId);
        logEntry.put("severity_text", level);
        logEntry.put("message", message);
        logEntry.put("attributes", new JSONObject().put("service", "coffee-shop"));

        try {
            URL url = new URL(SIGNOZ_LOGS_URL);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setRequestMethod("POST");
            connection.setRequestProperty("signoz-ingestion-key", SIGNOZ_INGESTION_KEY);
            connection.setRequestProperty("Content-Type", "application/json");
            connection.setDoOutput(true);

            try (OutputStream os = connection.getOutputStream()) {
                byte[] input = logEntry.toString().getBytes(StandardCharsets.UTF_8);
                os.write(input, 0, input.length);
            }

            int responseCode = connection.getResponseCode();
            if (responseCode != 200) {
                logger.error("Failed to send log to SigNoz. Response code: {}", responseCode);
            }
        } catch (Exception e) {
            logger.error("Error sending log to SigNoz: ", e);
        }
    }
}
```
