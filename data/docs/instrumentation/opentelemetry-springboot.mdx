...
Note that we have created the above deployment file for [spring-petclinic](https://github.com/SigNoz/spring-petclinic) Java application.
...
```bash
$ export POD_NAME=$(kubectl get pod -l app=<springboot-application> -o jsonpath="{.items[0].metadata.name}")  # springboot application is `spring-petclinic` in this case.   
