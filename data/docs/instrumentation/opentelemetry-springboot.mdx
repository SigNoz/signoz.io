...
You can instrument sending traces from Java application using one of the following methods:
...
To create a simple instance of the OpenTelemetry Collector, create a file `otel-collector.yaml` with the followiung contents:
...
Note that the ecporter endpoint is based on OpenTelemetryCollector instance name that we have setup in Step 3.
...
Note that we have created the above deployment file for spring-petclinic Java application.
...
The application will start running on port 8080 on your local machine. You can access the application by entering 
the following URL on your browser: http://localhost:8080
