...
![An image displaying linked logs and traces to SigNoz](images/logs-traces-NET.png)
...
1. Configure tracing using OpenTelemetry. 
    ...
    {
      public static string GetTraceId() => Activity.Current?.TraceId.ToString() ?? string.Empty;
      public static string GetSpanId() => Activity.Current?.SpanId.ToString() ?? string.Empty;
    }
    ```
1. Create spans for each method to track its execution. For example, consider a method to get coffee.
    ...
    {
      private static readonly ActivitySource ActivitySource = new("CoffeeShopService");

      public async Task GetCoffeeAsync()
      {
        using var activity = ActivitySource.StartActivity("GetCoffee", ActivityKind.Internal);
        
        await LogRequest("GetCoffee");

        // Simulate coffee preparation
        await Task.Delay(500);

        await LogResponse("GetCoffee");
      }
...
1. Attach log information each method. You can optionally add logs before and after each request.
    ...
    public class CoffeeShopService
    {
      ...
      {
        string message = $"Incoming request: {methodName}";
        Console.WriteLine($"INFO: {message} - Trace ID: {TraceHelper.GetTraceId()}");
        await SigNozLogger.SendLogToSigNoz("INFO", message);
      }

      private async Task LogResponse(string methodName)
      {
        string message = $"Response sent from: {methodName}";
        Console.WriteLine($"INFO: {message} - Trace ID: {TraceHelper.GetTraceId()}");
        await SigNozLogger.SendLogToSigNoz("INFO", message);
      }
    }
...
1. Add trace and span information in the log message.
    ...
    public class SigNozLogger
    {
    private static readonly HttpClient httpClient = new HttpClient();
    private const string SIGNOZ_INGESTION_KEY = "<SigNoz-ingestion-key>";
    private const string SIGNOZ_LOGS_URL = "https://ingest.{region}.signoz.cloud:443/logs/json";

    public static async Task SendLogToSigNoz(string level, string message)
    {
        var traceId = TraceHelper.GetTraceId();
        var spanId = TraceHelper.GetSpanId();
        ...
        var logEntry = new[]
        {
            new
            {
                ...
                severity_text = level,
                message = message,
                attributes = new { service = "CoffeeShopService" }
            }
        };

        var jsonPayload = JsonSerializer.Serialize(logEntry);
        var content = new StringContent(jsonPayload, Encoding.UTF8, "application/json");
        content.Headers.Add("signoz-ingestion-key", SIGNOZ_INGESTION_KEY);

        try
        {
            var response = await httpClient.PostAsync(SIGNOZ_LOGS_URL, content);
            response.EnsureSuccessStatusCode();
            Console.WriteLine("Log successfully sent to SigNoz.");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error sending log to SigNoz: {ex.Message}");
        }
    }
    }
    ```
