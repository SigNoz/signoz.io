---
date: 2026-08-27
title: Python OpenTelemetry Instrumentation Guide
description: Learn how to instrument your Python app with OpenTelemetry and send traces to SigNoz. Auto-instrument Django, Flask, FastAPI, and more.
doc_type: howto
---
...
Replace `<service-version>` in `instrumentation.yaml` with your release version, image tag, or git SHA (e.g., `1.4.2`, `a01dbef8`).
...
```bash
docker run -e OTEL_RESOURCE_ATTRIBUTES="service.name=<service-name>,service.version=<service-version>" \
...
<Tabs variant="pill" className="mb-8">
    <TabItem value="web-frameworks" label="Web Frameworks" default>
        ...
        <Tabs variant="pill" className="mb-8">
            ...
            <TabItem value="falcon" label="Falcon">
            ...
            <TabItem value="hypercorn-unicorn" label="Hypercorn/Unicorn">
    ...
    <TabItem value="background-workers" label="Background Workers">

        <Tabs variant="pill" className="mb-8">
            <TabItem value="celery" label="Celery" default>
...
If you see JSON span output in your terminal but traces don't appear in SigNoz, the usual causes are missing context propagation or an exporter misconfiguration (endpoint, auth, or network). If no output appears, the instrumentation isn't capturing your requests. Our deeper walkthrough on [troubleshooting Python OpenTelemetry tracing](https://signoz.io/blog/troubleshooting-python-with-opentelemetry-tracing/) covers these failure modes step by step, and [Debug missing traces, logs, and metrics in SigNoz](https://signoz.io/docs/ingestion/signoz-cloud/troubleshooting/data-not-appearing/) covers the same checks for logs and metrics too.
...
### Why does force_flush() return True when nothing arrives?

`force_flush()` reports whether the flush finished inside its timeout, not
whether SigNoz accepted the batch. It returns `True` even when the export failed
with `401 Unauthorized` from a wrong ingestion key, so it cannot be used as
proof that data was delivered.

To see the export result, enable exporter logging and look for a failure line:
...
Confirm delivery with the [Traces API check](#validate) instead of a flush
return value.
