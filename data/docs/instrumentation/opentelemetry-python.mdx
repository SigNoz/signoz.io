...
You can auto-instrument sending traces from the Python application using one of the following methods:
...
For Python application deployed on Kubernetes, you can auto-instrument the traces using Kubernetes OpenTelemetry Operator. 
...
Here are the steps you need to follow to auto-instrument the Python application using OTel Operator:
...
**Step 5.** Auto-instrument your Python app with OpenTelemetry

Create `deployment.yaml` file for your Python application as follows:
...
This helps in auto-instrumenting the traces from the Python application.
...
With this, the auto-instrumentation of traces for Python application is ready.

**Step 6.** Running the Python application

In order to run the application on port 8080, run the following commands:

```bash
export POD_NAME=$(kubectl get pod -l app=<service-name> -o jsonpath="{.items[0].metadata.name}")  # service name is `python-app` in this case.   
...
**Step 1.** Dockerize and Build your application
...
# Run app.py with OpenTelemetry instrumentation when the container launches
CMD sh -c "/otelcol-contrib --config=/etc/otelcol-contrib/config.yaml & opentelemetry-instrument python ./app.py"
...
```

This updated Dockerfile instructions will install required packages from `requirements.txt`, including OpenTelemetry components. It sets the environment variable for OpenTelemetry configuration specifying the service name, OTLP exporter endpoint, and protocol. The container executes 'app.py' with OpenTelemetry tracing, exposing port 5000. 

Make sure you have `config.yaml` in root of the application. This config.yaml should be copied from third step of [this](https://signoz.io/docs/tutorial/opentelemetry-binary-usage-in-virtual-machine/) .
