...
To correlate logs with traces in a Python app, you should use:

* OTEL_PYTHON_LOG_CORRELATION environment variable.
* LoggingInstrumentor Python module for native logging instrumentation. 

1. Set the environment variables. 
    ```bash
    OTEL_EXPORTER_OTLP_ENDPOINT="<your-signoz-otlp-endpoint>"
    OTEL_EXPORTER_OTLP_HEADERS="signoz-ingestion-key=<your-signoz-key>"
    OTEL_PYTHON_LOG_CORRELATION=true
    ```
1. Configure the logging formatter to include trace_id and span_id.
    ```python
    import logging
    import requests
    import os
    ...
    class SigNozLogFormatter(logging.Formatter):
        def format(self, record):
            ...
            log_data = {
                ...
                "attributes": {
                    "service": os.getenv("SERVICE_NAME", "coffee-shop"),
    ...
    logger = logging.getLogger("signozLogger")
    logger.setLevel(logging.INFO)

    handler = logging.StreamHandler()
    handler.setFormatter(SigNozLogFormatter())
    logger.addHandler(handler)
    ```

1. Send logs to SigNoz to link them with traces.
    ```python
    def send_log_to_signoz(log_data):
        SIGNOZ_LOGS_URL = os.getenv("SIGNOZ_LOGS_URL", "<SigNoz-logs-url>")
        SIGNOZ_INGESTION_KEY = os.getenv("SIGNOZ_INGESTION_KEY", "<SigNoz-ingestion-key>")
        ...
        try:
            response = requests.post(SIGNOZ_LOGS_URL, headers=headers, json=[log_data])
            response.raise_for_status()
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to send log to SigNoz: {e}")
    ```

For example, consider a coffee shop app. You have to log a coffee order request using OpenTelemetry’s auto-instrumentation and structured logging.

```python
def get_coffee(coffee_id):
    logger.info(f"Request received for coffee ID {coffee_id}")

    coffee = next((c for c in coffees if c["id"] == coffee_id), None)
    if not coffee:
        log_record = logger.handlers[0].formatter.format(
            logger.makeRecord(
                name="signozLogger",
                level=logging.INFO,
                fn="app.py",
                lno=33,
                msg=f"Coffee ID {coffee_id} not found",
                args=None,
                exc_info=None
            )
        )
        send_log_to_signoz(log_record)
    ...
    log_record = logger.handlers[0].formatter.format(
        logger.makeRecord(
            ...
            fn="app.py",
            lno=41,
            msg=f"Coffee order for {coffee['name']} processed successfully",
            args=None,
            exc_info=None
        )
    )
