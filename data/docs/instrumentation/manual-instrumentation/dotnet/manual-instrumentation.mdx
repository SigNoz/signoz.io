...
The `RecordException` method adds the exception as an event with stack trace details.
...
2. Trigger the code paths that create custom spans.

3. In SigNoz, navigate to **Traces** and filter by your service name.

4. Verify:
   - Custom spans appear with correct names
   - Attributes contain your business context
   - Events show up at the right timestamps
   - Errors display with exception details
...
- Return to the [.NET instrumentation guide](https://signoz.io/docs/instrumentation/opentelemetry-dotnet/) for automatic instrumentation setup
- [Set up alerts](https://signoz.io/docs/alerts-management/notification-channel/slack/) based on span attributes
- [Send logs from .NET](https://signoz.io/docs/logs-management/send-logs/collect-dotnet-logs/) and correlate with traces
