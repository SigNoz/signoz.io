---
date: 2026-09-12
title: How to add manual instrumentation in Go
description: Learn how to add manual instrumentation in Go applications using OpenTelemetry. Create custom spans, attach attributes, and trace business logic for deeper observability.

doc_type: howto
---

## Overview

Automatic instrumentation traces the libraries your application calls. Manual instrumentation traces your own code. Use it to time a checkout step, attach an order ID to a span, or record a failure with the context that explains it.

<Admonition type="info" title="Using self-hosted SigNoz?" defaultCollapsed="true">
  The steps on this page are identical for both deployments. Only the OTLP endpoint and the ingestion key differ, as shown in [Cloud to Self-Hosted](https://signoz.io/docs/ingestion/cloud-vs-self-hosted/#cloud-to-self-hosted).
</Admonition>

## Prerequisites

- A tracer provider and an exporter must already run in your application. If you use the SDK, finish the core setup in the [OpenTelemetry Go instrumentation guide](https://signoz.io/docs/instrumentation/opentelemetry-golang/).
- Propagate `context.Context` through handlers and business logic. Manual spans derive from these contexts so traces stay connected.
- Tested with <a href="https://go.dev/doc/go1.25" target="_blank" rel="noopener noreferrer nofollow">Go 1.25.1</a> and OpenTelemetry Go SDK <a href="https://pkg.go.dev/go.opentelemetry.io/otel/sdk@v1.38.0" target="_blank" rel="noopener noreferrer nofollow">v1.38.0</a>.

<Admonition type="info" title="Already using zero-code instrumentation?" defaultCollapsed="true">
Manual spans work alongside it. With [compile-time instrumentation](https://signoz.io/docs/instrumentation/golang/compile-time-instrumentation/), the injected SDK picks up the spans you create here. With the [eBPF agent](https://signoz.io/docs/instrumentation/golang/ebpf-instrumentation/), set `OTEL_GO_AUTO_GLOBAL=true` so the agent exports them too.
</Admonition>

## Add manual instrumentation

### Step 1. Create manual spans

Use the SDK tracer to wrap important work inside custom spans:

```go:manual-span.go defaultCollapsed
import (
	"context"

	"go.opentelemetry.io/otel"
	"go.opentelemetry.io/otel/attribute"
)

func processOrder(ctx context.Context, orderID string) error {
	tracer := otel.Tracer("order-service")

	ctx, span := tracer.Start(ctx, "process-order")
	defer span.End()

	span.SetAttributes(
		attribute.String("order.id", orderID),
		attribute.String("order.status", "processing"),
	)

	// Use ctx for downstream work so child spans stay linked.
	return nil
}
```

Tips:

- Reuse tracer instances instead of creating a new one for each request.
- Start spans with descriptive names that match business steps (`checkout`, `fetch-user`, etc.).
- End every span. `defer span.End()` is the safest place for it.

### Step 2. Propagate context

Go keeps no ambient trace state. A span finds its parent only through the `context.Context` you hand it, so Go breaks traces more often than Java or Python. See [Context Propagation](https://signoz.io/docs/traces-management/guides/context-propagation/) for the concepts behind this step.

#### Inside your process

Pass the `ctx` that `tracer.Start` returns into every function you want nested under that span:

```go:in-process.go
func processOrder(ctx context.Context, orderID string) error {
	ctx, span := tracer.Start(ctx, "process-order")
	defer span.End()

	// Child of process-order.
	validateOrder(ctx, orderID)

	// Root span of a brand new trace. The link to process-order is gone.
	validateOrder(context.Background(), orderID)

	return nil
}
```

A goroutine that outlives the request needs a context you derive on purpose. The request context is canceled once the handler returns, so background work that keeps using it starts from an already-canceled context. `context.WithoutCancel` keeps the values, including the span, while dropping the cancellation and the deadline:

```go:goroutine.go
// Keeps the trace context, drops the request's cancellation. Go 1.21 and later.
bgCtx := context.WithoutCancel(ctx)

go func() {
	_, span := tracer.Start(bgCtx, "settle-order")
	defer span.End()
	// Background work here.
}()
```

#### Across a service boundary

Instrumentation libraries such as `otelhttp` inject and extract for you. Write the calls yourself for any hop they do not cover.

Set the span kind on both ends. SigNoz reads `Client` and `Server` to build the service map and APM metrics.

On the way out, inject the context into the outgoing headers:

```go:inject.go minimap defaultCollapsed
import (
	"context"
	"io"
	"net/http"

	"go.opentelemetry.io/otel"
	"go.opentelemetry.io/otel/propagation"
	"go.opentelemetry.io/otel/trace"
)

func chargeCard(ctx context.Context, body io.Reader) error {
	ctx, span := tracer.Start(ctx, "charge-card", trace.WithSpanKind(trace.SpanKindClient))
	defer span.End()

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, "https://payments.internal/charge", body)
	if err != nil {
		return err
	}

	// Writes the traceparent header onto req.
	otel.GetTextMapPropagator().Inject(ctx, propagation.HeaderCarrier(req.Header))

	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return err
	}
	defer resp.Body.Close()

	// Drain the body so the connection can be reused.
	_, err = io.Copy(io.Discard, resp.Body)
	return err
}
```

On the way in, extract before you start the entry span:

```go:extract.go
func handleCharge(w http.ResponseWriter, r *http.Request) {
	// Returns a context carrying the caller's span, or r.Context() unchanged
	// when no traceparent header is present.
	ctx := otel.GetTextMapPropagator().Extract(r.Context(), propagation.HeaderCarrier(r.Header))

	ctx, span := tracer.Start(ctx, "handle-charge", trace.WithSpanKind(trace.SpanKindServer))
	defer span.End()

	// handle-charge is now a child of charge-card in the caller's trace.
	capturePayment(ctx)
}
```

For a carrier that is not `http.Header`, such as queue message metadata, use `propagation.MapCarrier`:

```go:queue.go
// Publisher
carrier := propagation.MapCarrier{}
otel.GetTextMapPropagator().Inject(ctx, carrier)
msg.Metadata = carrier // map[string]string

// Consumer
ctx := otel.GetTextMapPropagator().Extract(context.Background(), propagation.MapCarrier(msg.Metadata))
```

<Admonition type="info">
`Inject` and `Extract` do nothing unless you registered a propagator with `otel.SetTextMapPropagator`. The [Go instrumentation guide](https://signoz.io/docs/instrumentation/opentelemetry-golang/) sets up `propagation.TraceContext{}` and `propagation.Baggage{}` in `initTracer`.
</Admonition>

### Step 3. Add attributes and events

Attributes show up as key-value pairs in SigNoz so you can filter and aggregate spans. Events capture notable moments inside a span.

```go:attributes.go
import (
	"go.opentelemetry.io/otel/attribute"
	"go.opentelemetry.io/otel/trace"
)

func handlePayment(ctx context.Context, amount float64) {
	span := trace.SpanFromContext(ctx)

	span.SetAttributes(
		attribute.Float64("payment.amount", amount),
		attribute.String("payment.currency", "USD"),
		attribute.String("payment.method", "credit_card"),
	)

	span.AddEvent("payment.processed", trace.WithAttributes(
		attribute.String("status", "success"),
		attribute.String("transaction.id", "txn_123456"),
	))
}
```

- Keep attribute keys consistent (use semantic conventions when possible).
- Use events to mark retries, cache hits/misses, queue waits, and similar milestones.

### Step 4. Record errors

Flag failures on the span so they are easy to query in SigNoz.

```go:error-handling.go
import (
	"go.opentelemetry.io/otel/codes"
	"go.opentelemetry.io/otel/trace"
)

func riskyOperation(ctx context.Context) error {
	span := trace.SpanFromContext(ctx)

	err := doSomethingRisky()
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, err.Error())
		return err
	}

	span.SetStatus(codes.Ok, "")
	return nil
}
```

- `RecordError` attaches stack and message details.
- Setting status to `codes.Error` surfaces the span in SigNoz error views and alerts.
- Continue propagating the error upstream so calling code can respond.

## Validate

1. Trigger the code paths that emit manual spans.
2. In SigNoz **Traces**, filter by `service.name` or your span name.
3. Open a trace and verify attributes, events, and error status.
4. Check the **Errors** view to confirm failures show up with recorded exceptions.

<details>
<ToggleHeading>

## Troubleshooting

</ToggleHeading>

Still not seeing data in SigNoz? Work through [Debug missing traces, logs, and metrics](https://signoz.io/docs/ingestion/signoz-cloud/troubleshooting/data-not-appearing/), which covers SDK diagnostics, Collector connectivity, and the common ingestion errors for all three signals.

### Why don't I see my custom spans in SigNoz?

- Confirm `initTracer` (and automatic instrumentation) runs before your application handlers.
- Check sampler configuration. Using `TraceIDRatioBased(0.01)` in dev may drop most manual spans.
- Verify traffic hits the functions where you inserted spans.

### Why are child spans missing even though I create them?

- Ensure you pass the `ctx` returned by `tracer.Start` (or `trace.SpanFromContext`) into downstream functions.
- HTTP/database client calls must receive the propagated context as well, otherwise traces break across boundaries.

### Why does the downstream service start its own trace?

- Make sure that the caller runs `Inject` on the outgoing request and that the request carries a `traceparent` header.
- Make sure that the receiver runs `Extract` and passes the returned context into `tracer.Start`, rather than `r.Context()` or `context.Background()`.
- Make sure that both services register the same propagator format. See [Context Propagation](https://signoz.io/docs/traces-management/guides/context-propagation/).

### Why don't attributes or events appear on the span?

- Attribute keys must be strings and values must use the correct helper (`attribute.String`, `attribute.Float64`, etc.).
- Call `span.AddEvent` or `span.SetAttributes` before `span.End()`. Post-end mutations are ignored.
- Avoid reusing finished spans. Always create a new span for each invocation.

</details>

## Next steps

- Send logs from your Go application using popular logging libraries: [Logrus](https://signoz.io/docs/logs-management/send-logs/logrus-to-signoz/), [Zap](https://signoz.io/docs/logs-management/send-logs/zap-to-signoz/), or [Zerolog](https://signoz.io/docs/logs-management/send-logs/zerolog-to-signoz/)
- [Correlate traces with logs](https://signoz.io/docs/traces-management/guides/correlate-traces-and-logs/) to accelerate triage across signals
- Add instrumentation to common Go frameworks via the [Go instrumentation library catalog](https://signoz.io/docs/instrumentation/opentelemetry-golang/#library-instrumentation)
- Read [Context Propagation](https://signoz.io/docs/traces-management/guides/context-propagation/) for the wire format and the failure modes shared across languages

## Get Help

<GetHelp />
