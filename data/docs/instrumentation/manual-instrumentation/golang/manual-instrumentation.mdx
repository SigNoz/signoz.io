...
Go keeps no ambient trace state. A span finds its parent only through the `context.Context` you hand it, which is why Go breaks traces more readily than Java or Python. See [Context Propagation](https://signoz.io/docs/traces-management/guides/context-propagation/) for the concepts behind this step.
...
A goroutine that outlives the request needs a context you derive on purpose. The request context is cancelled once the handler returns, so background work that keeps using it starts from an already-cancelled context. `context.WithoutCancel` keeps the values, including the span, while dropping the cancellation and the deadline:
