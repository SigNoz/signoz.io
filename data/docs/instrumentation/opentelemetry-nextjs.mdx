...
**Step 2.** Create `instrumentation.node.ts` file<br></br>
...
- For older versions of NextJS(14 below), the [experimental instrumentation hook](https://nextjs.org/docs/14/pages/api-reference/next-config-js/instrumentationHook) needs to be enabled
</Admonition>
...
**Step 5.** Once we are done with the above configurations, you can run the collector service with the following command:
```bash
./otelcol-contrib --config ./config.yaml
```

**Step 6.** Start running your application and wait for few seconds to start receiving instrumentation data on the SigNoz Cloud. 
...
**Step 2.** Create `instrumentation.ts` file<br></br>
```tsx
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    await import('./instrumentation.node')
...
**Step 4.** Once you're done configuring the exporter options, try running your application and validate if your application is sending traces to SigNoz [here](#validating-instrumentation-by-checking-for-traces).
