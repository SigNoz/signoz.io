...
Use log attributes like `workflow_id`, `run_id`, and `namespace` to filter and analyze logs specific to your Temporal workflows.

### Correlating Telemetry Data

SigNoz allows you to correlate different types of telemetry data:
1. **Traces to Logs**: Click on a trace span to view associated logs
2. **Metrics to Traces**: Use exemplars to jump from metrics to relevant traces
3. **Logs to Traces**: Click on trace IDs in logs to view the complete trace

This correlation helps debug issues by providing full context of your Temporal workflows.
