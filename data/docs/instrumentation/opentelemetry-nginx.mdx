...
The YAML config does not support `service_version` directly. Create a systemd drop-in so the NGINX service process inherits the variable:

```bash
sudo mkdir -p /etc/systemd/system/nginx.service.d
sudo tee /etc/systemd/system/nginx.service.d/otel.conf > /dev/null <<EOF
[Service]
Environment="OTEL_RESOURCE_ATTRIBUTES=service.version=<service-version>"
EOF
...
```nginx:opentelemetry_module.conf
