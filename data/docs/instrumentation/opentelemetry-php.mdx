---
date: 2026-08-27
title: PHP OpenTelemetry Instrumentation Guide
description: Instrument your PHP application with OpenTelemetry and send traces to SigNoz. Works with Laravel, WordPress, Symfony, and Slim using auto-instrumentation.
doc_type: howto
---
...
- <a href="https://packagist.org/packages/open-telemetry/exporter-otlp" target="_blank" rel="noopener noreferrer nofollow">PHP 8.1+</a>
...
Verify these values:
- `<region>`: Your SigNoz Cloud region. See [endpoints](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint).
...
```yaml:deployment.yaml defaultCollapsed
env:
...
For a framework-focused walkthrough, our [PHP OpenTelemetry tutorial](https://signoz.io/blog/opentelemetry-php/) covers Laravel and Symfony setup in more detail.
...
<Tabs variant="pill" className="mb-8">
    <TabItem value="frameworks" label="Frameworks" default>
        <Tabs variant="pill" className="mb-8">
            ...
            <TabItem value="slim" label="Slim">
                ...
                Slim auto-instrumentation requires `open-telemetry/opentelemetry-auto-slim`.
    ...
    <TabItem value="runtime" label="Runtime">
        <Tabs variant="pill" className="mb-8">
            <TabItem value="php-fpm" label="PHP-FPM" default>

                ### Additional package

                No additional Composer package is required specifically for PHP-FPM.

                You still need your framework-specific auto-instrumentation package. For example, Slim requires `open-telemetry/opentelemetry-auto-slim`.

                ### Configuration

                `clear_env` controls whether PHP-FPM clears inherited environment variables before starting worker processes.

                It defaults to `yes` for security, but this can strip `OTEL_*` variables before PHP request handlers run.

                Update your PHP-FPM pool configuration file (common paths):
                - Debian/Ubuntu: `/etc/php/<php-version>/fpm/pool.d/www.conf`
                - RHEL/CentOS/Amazon Linux: `/etc/php-fpm.d/www.conf`

                See [PHP-FPM pool directives](https://www.php.net/manual/en/install.fpm.configuration.php) for `clear_env` and `env[...]` details.

                Option 1 (simple, less restrictive): allow inherited environment variables for the pool:

                ```ini
                clear_env = no
                ```

                Option 2 (recommended): keep `clear_env = yes` and pass only required OpenTelemetry variables:
                ...
                If PHP-FPM is already running and you updated the pool configuration, reload it:

                ```bash
                # Debian/Ubuntu
                sudo systemctl reload php<php-version>-fpm
                ...
                After starting or reloading PHP-FPM, send a request to your application and verify that traces appear in the **Services** tab in SigNoz.
...
If you see span output in your terminal but traces don't appear in SigNoz, the issue is with export configuration (endpoint, auth, or network). If no output appears, the instrumentation isn't capturing your requests. See [Debug missing traces, logs, and metrics in SigNoz](https://signoz.io/docs/ingestion/signoz-cloud/troubleshooting/data-not-appearing/) for a full walkthrough of these checks, also covering logs and metrics.
...
2. Check your <a href="https://packagist.org/packages/open-telemetry/exporter-otlp" target="_blank" rel="noopener noreferrer nofollow">PHP version meets the 8.1+ requirement</a>: `php -v`
