...
>
    ...
    <LibraryTab value="php-fpm" label="PHP-FPM" category="frameworks">

        ### Configuration

        `clear_env` controls whether PHP-FPM clears inherited environment variables before starting worker processes.

        It defaults to `yes`, which is safer by default, but it can strip `OTEL_*` variables before PHP request handlers run.
        ...
        In your PHP-FPM pool configuration, set:
