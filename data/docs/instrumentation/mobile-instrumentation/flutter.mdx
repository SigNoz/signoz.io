...
<Admonition type="info">
You can test sample application for [Flutter](https://github.com/SigNoz/mobile-monitoring-sample-apps/android-ios-flutter) (Replace the variables inside `main.dart`)
</Admonition>
...
final headers = {
  ...
  final exporter = CollectorExporter(
    ...
    Uri.parse('<SIGNOZ_INGESTION_URL>:<PORT>/v1/traces'),
    headers: headers,
  );
