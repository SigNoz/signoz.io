...
AWS requires following this configuration for Fluent Bit:
...
### 3. CloudWatch Receiver in OpenTelemetry Collector

We will be using the [awscloudwatchreceiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/awscloudwatchreceiver) to ingest the logs from AWS CloudWatch to the OpenTelemetry Collector. If you've opted for output in Fluent Bit such as Kinesis or Firehose, then you can use other receivers made available by the OpenTelemetry community.

Add the following to your [OpenTelemetry Collector ConfigMap]() created during installation:

```yaml
# otel-agent-sidecar-ConfigMap
receivers:
  awscloudwatch:
    region: <REGION>
    logs:
