...
description: How to Configure the dameonset and deployment with OpenTelemetry Operator in your Kubernetes Cluster
---
...
spec:
  ...
  config:
    ...
    processors:
      ...
      k8sattributes: {}
      resourcedetection: {detectors: [k8snode, env, system]}
    receivers:
    ...
    service:
      ...
      pipelines:
        logs: {receivers: [otlp, filelog/k8s], processors: [resourcedetection, k8sattributes, batch], exporters: [otlp/signoz]}
        metrics: {receivers: [otlp, hostmetrics, kubeletstats], processors: [resourcedetection, k8sattributes, batch], exporters: [otlp/signoz]}
        traces: {receivers: [otlp], processors: [resourcedetection, k8sattributes, batch], exporters: [otlp/signoz]}
```
...
spec:
  ...
  config:
    ...
    service:
      pipelines:
        logs: {receivers: [k8s_events], processors: [k8sattributes, resourcedetection, batch], exporters: [otlp/signoz]}
        metrics/internal: {receivers: [k8s_cluster], processors: [k8sattributes, resourcedetection, batch], exporters: [otlp/signoz]}
```
...
- In these docs, Kubernetes auto-instrumentation is performed via the OpenTelemetry Operator.
...
We provide separate, language-specific docs for auto-instrumentation (Java, Node, Python, etc.), and the OpenTelemetry Operator supports auto-instrumentation across these languages via the `Instrumentation` CRD and per-language inject annotations. Prefer the language docs for end-to-end steps and use this page to wire your cluster-level collector with the Operator.

High-level flow:
- Create an `Instrumentation` resource that points to your SigNoz OTLP endpoint/headers
- Add the language-specific inject annotation on your workloads (e.g., `instrumentation.opentelemetry.io/inject-java: "true"`)

### Verify the Installation

```bash
# List collector CRs
kubectl get opentelemetrycollectors.oteltel.opentelemetry.io -A || kubectl get otelcol -A || kubectl get opentelemetrycollector -A

# List pods and check status
kubectl get pods -A -l app.kubernetes.io/name=opentelemetry-collector

# Tail logs (replace <NAMESPACE> if different)
kubectl logs -n <NAMESPACE> ds/signoz-collector-daemonset --since=5m
kubectl logs -n <NAMESPACE> deploy/signoz-collector-deployment --since=5m
```
