...
Install the [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) and add the Container Apps extension:
...
```xml:config.xml minimap defaultCollapsed
<?xml version="1.0"?>
          <clickhouse>
...
```xml:users.xml defaultCollapsed minimap
<?xml version="1.0"?>
<clickhouse>
    <profiles>
        <default>
...
```xml:custom-function.xml defaultCollapsed
<functions>
    <function>
...
```yaml:signoz-otel-collector-config.yaml minimap defaultCollapsed
```yaml
receivers:
  otlp:
    protocols:
...
### 3. Deploy Telemetrystore Migrator Job

The telemetrystore migrator runs all ClickHouse migrations (bootstrap, sync, and async) needed.

<details>
<summary>telemetrystore-migrator.yaml (Click to expand config)</summary>
...
Deploy and run the migrator job:
...
az containerapp job create \
  ...
  --resource-group $RG_NAME \
  --environment $ENV_NAME \
  --yaml telemetrystore-migrator.yaml
