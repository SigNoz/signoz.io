...
<a href="https://github.com/SigNoz/foundry" target="_blank" rel="noopener noreferrer nofollow">Foundry</a> can deploy SigNoz on ECS for you using <a href="https://github.com/SigNoz/foundry/tree/main/docs/examples/ecs/ec2/terraform" target="_blank" rel="noopener noreferrer nofollow">ECS Terraform casting</a> (a configuration file). You provide a casting, and Foundry generates the Terraform modules for your ECS deployment.
</KeyPointCallout>
...
```jsonc minimap defaultCollapsed
{
...
```json defaultCollapsed minimap
{
  ...
  "environment": [
    { "name": "SIGNOZ_OTEL_COLLECTOR_CLICKHOUSE_DSN",         "value": "tcp://clickhouse:9000" },
    { "name": "SIGNOZ_OTEL_COLLECTOR_CLICKHOUSE_CLUSTER",     "value": "cluster" },
    { "name": "SIGNOZ_OTEL_COLLECTOR_CLICKHOUSE_REPLICATION", "value": "true" },
    { "name": "SIGNOZ_OTEL_COLLECTOR_TIMEOUT",                "value": "10m" },
...
### 7. signoz-telemetrystore-migrator
...
```json defaultCollapsed
{
  ...
  "logConfiguration": {
    ...
    "options": {
      "awslogs-group": "/aws/ecs/<LOG_GROUP>",
      "awslogs-region": "<AWS_REGION>",
      "awslogs-stream-prefix": "signoz-telemetrystore-migrator"
    }
  }
}
```
...
### 9. General Task Definitions config
