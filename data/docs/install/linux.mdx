...
<a href="https://github.com/SigNoz/foundry" target="_blank" rel="noopener noreferrer nofollow">Foundry</a> can deploy SigNoz on Linux for you using <a href="https://github.com/SigNoz/foundry/tree/main/docs/examples/systemd/binary" target="_blank" rel="noopener noreferrer nofollow">systemd binary casting</a> (a configuration file). You install the dependencies, and Foundry handles everything else, from service configuration to startup.
</Admonition>
...
```bash
./signoz-otel-collector_linux_$(uname -m | sed 's/x86_64/amd64/g' | sed 's/aarch64/arm64/g')/bin/signoz-otel-collector migrate bootstrap --clickhouse-dsn="tcp://localhost:9000?password=password" --clickhouse-replication=true
./signoz-otel-collector_linux_$(uname -m | sed 's/x86_64/amd64/g' | sed 's/aarch64/arm64/g')/bin/signoz-otel-collector migrate sync up --clickhouse-dsn="tcp://localhost:9000?password=password" --clickhouse-replication=true
./signoz-otel-collector_linux_$(uname -m | sed 's/x86_64/amd64/g' | sed 's/aarch64/arm64/g')/bin/signoz-otel-collector migrate async up --clickhouse-dsn="tcp://localhost:9000?password=password" --clickhouse-replication=true
```
...
2. Change the ownership of the files to the `signoz` user and group:
...
3. Create a `config.yaml` file in the `/opt/signoz-otel-collector/conf` directory:
...
4. Create a `opamp.yaml` file in the `/opt/signoz-otel-collector/conf` directory:
...
5. Create the SigNoz Otel Collector systemd service:
...
6. Reload the systemd daemon and start the SigNoz Otel Collector service:
