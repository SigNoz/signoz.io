...
The final goal is to be able to capture the local metrics (CPU, RAM, DIsk, Network...) of the local machine and start playing with the Dashboard. Further questions some [RTFM](https://en.wikipedia.org/wiki/RTFM) work and a [Slack channel](https://app.slack.com/client/T01HWUTP0LT/C01LWQ8KS7M) is also available.
...
> 
> Listing : Bridge City Siner - Midnight to Vice
...
```bash
sudo dnf update
sudo dnf install -y java-11-openjdk  # CentOS/RHEL
```
...
> On RedHat 8.10 is compiled with the GLibC v2.28. I had to recompile the schema-migrator and the otel-collector (see below).
...
0. (Optional) Rebuild the `schema-migrator`
    ...
    * Make directory and add to `PATH`:
    ```bash
    sudo mkdir -p /opt/signoz-otel-collector/bin
    cp schema-migrator /opt/signoz-migrator/bin/
    # TODO add /opt/signoz-migrator/bin/ to path
    ```
...
1. Download and extract the SigNoz binary (use a specific version for stability, e.g., v0.79.1):
...
2. Create the OTel Collector configuration file (replace <password> with the signoz user password and update 10.24.208.77 to localhost if running locally):
...
exporters:
  clickhousemetricswrite:
    endpoint: tcp://10.24.208.77:9000/signoz_metrics?username=signoz&password=signoz
    tls:
      insecure: true
  ...
  metadataexporter:
    dsn: tcp://10.24.208.77:9000/signoz_metadata?username=signoz&password=signoz
    timeout: 10s
    tenant_id: default
    cache:
      provider: in_memory
...
3. Access the SigNoz UI by navigating to http://<your-server-ip>:8080 in a web browser. Log in and verify that traces, metrics, and logs are being collected.
