...
5. Create a `opamp.yaml` file in the `/opt/signoz-otel-collector/conf` directory. This connects the collector to the SigNoz OpAMP server, which manages collector config remotely. See [OpAMP - Remote Collector Management](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/opamp/) for details.
