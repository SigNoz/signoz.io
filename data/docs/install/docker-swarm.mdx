...
1. Using the `docker stack services` command, monitor the SigNoz deployment process.
    ...
    vpu9ub4rnr4g   signoz_alertmanager      replicated   1/1        signoz/alertmanager:0.23.7                   
    loqa6qbq7lk3   signoz_clickhouse        replicated   1/1        clickhouse/clickhouse-server:24.1.2-alpine   
    qjrn3lfxdvdn   signoz_frontend          replicated   1/1        signoz/frontend:0.69.0                       *:80->3301/tcp
    ...
    eu5fbg0v1bps   signoz_query-service     replicated   1/1        signoz/query-service:0.69.0                  
...
1. Change directory to `deploy/docker-swarm/generator/hotrod`

  ```bash
  cd deploy/docker-swarm/generator/hotrod
  ```

2. Deploy the HOTRod sample application by running the `docker stack deploy` command:

  ```bash
  docker stack deploy -d --prune -c docker-compose.yaml hotrod
  ```
...
3. Verify that the HOTRod sample application is running correctly. To view the status of your containers, run the following command:

  ```bash
  docker stack services hotrod
  ```
  ...
  ```output
  ID             NAME                 MODE         REPLICAS   IMAGE                                 PORTS
  ly84gnfxw3br   hotrod_hotrod        replicated   1/1        jaegertracing/example-hotrod:1.61.0   
  954b2gfskvlg   hotrod_load-hotrod   replicated   1/1        signoz/locust:1.2.3                   
  ```

4. _(Optional)_ To stop the HOTRod sample application, run the following command:

  ```bash
  docker stack rm hotrod
  ```
