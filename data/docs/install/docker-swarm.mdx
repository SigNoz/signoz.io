...
`cast` renders the stack Compose file into `pours/deployment/` and deploys it as the `signoz` stack with `docker stack deploy`.

<Admonition type="tip" title="Skip cast and deploy the stack yourself" defaultCollapsed="true">
If you prefer to review the stack file before deploying, render it with `forge` and run `docker stack deploy` yourself:

```bash
foundryctl forge -f casting.yaml                              # generate the stack compose file
docker stack deploy -c pours/deployment/compose.yaml signoz   # deploy it as the "signoz" stack
```
</Admonition>
...
- `Swarm: active` — Swarm mode is on. If it shows `inactive`, initialize it with `docker swarm init` (or join an existing swarm).
- `Manager: true` — this node is a manager. If it shows `false`, run the deploy from a manager node instead.

### A service keeps restarting right after deploy

Docker Swarm starts every service in parallel — it does not honor `depends_on`. Until ClickHouse and Postgres are ready, dependent services such as `signoz_signoz-signoz-0` and `signoz_signoz-telemetrystore-migrator` may fail and restart a few times before they settle. Inspect any service that does not recover:
