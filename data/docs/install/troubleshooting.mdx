...
## Docker Installation in Selinux

When running SigNoz with Docker on SELinux-enabled hosts, you need to apply an SELinux label to bind-mount volumes by appending either `:z` or `:Z` to the volume suffix. For example:
