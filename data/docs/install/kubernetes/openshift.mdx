...
This guide walks you through creating the necessary SCC, installing SigNoz with the official [Helm](https://helm.sh/) chart, and verifying the deployment, so you can start collecting logs, metrics, and traces from your OpenShift workloads in just a few steps.
...
- Either the `oc` or `kubectl` CLI configured and authenticated  
...
For OpenShift cluster, you need to setup the [storageclasses](https://docs.redhat.com/en/documentation/red_hat_openshift_container_storage/4.6/html-single/managing_and_allocating_storage_resources/index) with compatible provisioner
...
We require following SCCs to be added the openShift cluster for the installation of K8s-infra for Kubernetes Monitoring
