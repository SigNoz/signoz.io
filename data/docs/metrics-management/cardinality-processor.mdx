...
**2. Confirm the processor is acting.** It publishes diagnostics on the collector's Prometheus endpoint, port **8888**. Read them with `curl localhost:8888/metrics`, or use the optional [diagnostics scraper](#export-diagnostics-to-signoz-optional) to view them in SigNoz. The useful ones, as they appear in SigNoz:
