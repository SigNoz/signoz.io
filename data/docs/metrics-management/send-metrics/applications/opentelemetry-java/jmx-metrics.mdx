...
The OTel Collector JMX receiver requires the JMX Metric Gatherer JAR file (version 1.9+). Download it from the <a href="https://github.com/open-telemetry/opentelemetry-java-contrib/releases" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Java Contrib releases</a>.

Look for the file named `opentelemetry-java-contrib-jmx-metrics.jar` in the release assets and place it in a location accessible to the OTel Collector (e.g., `/opt/opentelemetry-java-contrib-jmx-metrics.jar`).
...
- [Runtime Metrics](https://signoz.io/docs/metrics-management/send-metrics/applications/opentelemetry-java/#runtime-metrics) for JVM runtime metrics using the OpenTelemetry Java agent
