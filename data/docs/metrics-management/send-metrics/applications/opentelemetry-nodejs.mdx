...
<Admonition type="info">
The auto-instrumentation package automatically generates HTTP metrics (`http.server.request.duration` and `http.client.request.duration`) for Node.js built-in `http` and `https` modules. Since frameworks like Express, Fastify, and Koa use these modules internally, HTTP metrics are captured automatically. See the <a href="https://github.com/open-telemetry/opentelemetry-js/blob/main/experimental/packages/opentelemetry-instrumentation-http/README.md" target="_blank" rel="noopener noreferrer nofollow">HTTP instrumentation documentation</a> for more details.
</Admonition>
...
1. **Check Environment Variables**: Ensure `OTEL_EXPORTER_OTLP_ENDPOINT` is set correctly:
    - `https://ingest.<region>.signoz.cloud:443` (the SDK appends `/v1/metrics` automatically when using `http/protobuf`)
