...
This guide installs only metrics packages (`opentelemetry-exporter-otlp-metrics`). If `OTEL_TRACES_EXPORTER` is unset, the Ruby SDK defaults traces exporter to `otlp`. Without a traces OTLP exporter gem, the SDK shows a warning and spans are not exported.
...
```ruby:lib/runtime_metrics.rb minimap defaultCollapsed
require 'get_process_mem'
