...
description: Send custom and runtime metrics from your Java application to SigNoz using the OpenTelemetry Java agent.

doc_type: howto
---

This guide shows you how to send custom and runtime metrics from your Java application to SigNoz using the OpenTelemetry Java agent.
...
4. JVM metrics like `jvm.memory.used` and `jvm.cpu.time` are collected automatically by the agent.

<Admonition type="info">
JVM runtime metrics (memory, GC, threads, CPU) are collected automatically by the agent. See [JVM Runtime Metrics](#jvm-runtime-metrics) for the full list.
</Admonition>
...
The Java agent automatically collects JVM runtime metrics. No additional configuration is needed.
...
<Admonition type="info">
Use the pre-configured [JVM Metrics Dashboard](https://signoz.io/docs/dashboards/dashboard-templates/jvm-metrics/) to visualize these metrics.
</Admonition>
