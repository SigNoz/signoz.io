...
<KeyPointCallout title="Want only JVM and custom metrics?" defaultCollapsed={true}>
By default, the Java agent generates HTTP metrics for web frameworks. To collect only JVM runtime metrics and custom metrics (via `getMeter()`), add these environment variables:
...
This disables all auto-instrumentation (including HTTP metrics) while keeping:
- **JVM runtime metrics**: `jvm.memory.used`, `jvm.gc.duration`, `jvm.thread.count`, etc.
- **Custom metrics**: Any metrics you create via `GlobalOpenTelemetry.getMeter()`
</KeyPointCallout>
