...
In the **Time Series** view, each chart card has its own **download** icon in the chart header. Because every chart corresponds to a separate query, the buttons are per-chart, and each chart exports independently. The screen-level Y-axis unit selector stays separate from these per-chart buttons.

Click the **download** icon on a chart to export the data behind it. A popover opens with a **FORMAT** selector offering **csv** (default) and **jsonl**. Choose a format and click **Export** to download the file.
...
<Admonition type="info">
The download button appears whenever a Time Series chart has data to export. No configuration or feature flag is required, and export behaves the same on SigNoz Cloud and self-hosted deployments.
</Admonition>
