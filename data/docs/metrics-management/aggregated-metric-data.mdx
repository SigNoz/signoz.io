...
description: What it means when a panel renders from aggregated metric data - what volume control rules preserve, what they trade away, and why values can differ from what an alert saw.
doc_type: explanation
---

## Overview

If a chart shows a banner saying it's rendered from aggregated data, the metric behind it has a [volume control](https://signoz.io/docs/metrics-management/volume-control/) rule: some of its attributes have been aggregated away to reduce stored cardinality and cost. The data you're looking at is mathematically correct, but it's stored at a coarser granularity than the original — and for some questions, that changes the answer.
...
- **Counters and histograms** (counts, sums, and buckets): the **reset-aware increase** for the window — how much each original series actually grew, summed across the collapsed series, correctly accounting for counter resets and process restarts. Rates and increases over any window of 60 seconds or more remain exact.
