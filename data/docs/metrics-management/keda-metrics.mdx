...
The **keda-operator** Deployment runs an OpenTelemetry SDK in-process and pushes metrics to an OTLP endpoint, skipping Prometheus scrape jobs and sidecars. Set two flags on the operator:

- `--enable-opentelemetry-metrics=true` on the operator binary
- **OTEL_EXPORTER_OTLP_ENDPOINT** (with **OTEL_EXPORTER_OTLP_HEADERS** for auth) as container env vars
