...
This snippet only shows the additions. Keep the rest of your pipeline intact. The `resourcedetection`, `batch`, and `otlp` exporter come from the [VM install guide](https://signoz.io/docs/opentelemetry-collection-agents/vm/install/); reuse the ones already in your config.
...
| `nginx.connections_current`  | Sum  | connections | Current connections, split by the `state` attribute (`active`, `reading`, `writing`, `waiting`).                                              |
