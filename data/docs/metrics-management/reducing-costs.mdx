...
### 4. Use SDK-Level Delta Temporality for Idle Series

Setting delta temporality at the SDK level (via `OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE=delta` or during SDK initialization) causes synchronous instruments to emit no datapoint during intervals where nothing was recorded. This eliminates cost for inactive series entirely.

<Admonition type="warning">
This benefit applies only to **synchronous instruments** (counters, histograms recorded inline with application code). Observable/async instruments (registered callbacks) emit a datapoint every collection cycle regardless of temporality.
</Admonition>

<Admonition type="info">
Using the `cumulativetodelta` collector processor does **not** provide the same savings. The SDK still exports cumulative values every interval even when nothing changed, and the processor converts each of those unchanged points into a zero-value delta point. To benefit from idle-series elimination, configure delta temporality in the SDK itself.
</Admonition>
...
- **Aggregate labels** - Use the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/metricstransformprocessor/README.md"> MetricsTransform Processor
    </a> to aggregate metrics, which actually reduces cardinality and sample count.
