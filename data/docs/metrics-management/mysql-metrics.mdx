...
### Step 4: Collect query samples and top queries

</ToggleHeading>

Beyond metrics, the `mysql` receiver can emit query-monitoring **logs** from `performance_schema`: `db.server.query_sample` (statements running now) and `db.server.top_query` (most resource-intensive queries). To collect MySQL's error, slow, and general log files instead, use the <a href="https://signoz.io/docs/integrations/opentelemetry-mysql/">filelog approach</a>.
