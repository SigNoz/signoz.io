...
This Gauge query in SigNoz's Metrics Explorer uses the same Avg (temporal) and Sum (spatial) aggregation, grouped by pod:
...
This cumulative Counter query uses the same Increase (temporal) and Sum (spatial) aggregation, grouped by service:
...
This Histogram query uses P95 spatial aggregation grouped by service. There's no separate temporal aggregation control, since histogram values are already bucketed:
