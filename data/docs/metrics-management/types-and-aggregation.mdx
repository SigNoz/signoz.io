...
- **Counter**: A <Tooltip text="counter" content="A monotonic sum: it only ever increases, like an odometer." link="https://signoz.io/docs/glossary/#counter" /> is a metric that represents a single numerical value that monotonically increases over time. A counter resets to zero when the process restarts. Counters are used to track the number of events that occur in a system. For example, the number of requests received by a server.

- **Gauge**: A <Tooltip text="gauge" content="A metric that reports a value read at a single moment, such as memory in use, queue depth, or CPU temperature." link="https://signoz.io/docs/glossary/#gauge" /> is a metric that represents a single numerical value that can arbitrarily go up and down. Gauges are typically used for measured values like temperatures or current memory usage.

- **Histogram**: A <Tooltip text="histogram" content="A metric that aggregates many observations client-side into buckets, plus a count and optional sum, min, and max." link="https://signoz.io/docs/glossary/#histogram" /> is a metric that represents the distribution of a set of values. Histograms are used to track the distribution of values over time. For example, the response time of a web service.

- **Exponential Histogram**: An <Tooltip text="exponential histogram" content="A histogram whose bucket boundaries are computed from a scale parameter rather than listed explicitly." link="https://signoz.io/docs/glossary/#exponential-histogram" /> is a metric that represents the distribution of a set of values on a logarithmic scale. Exponential histograms are used to track the distribution of values that span several orders of magnitude. For example, the latency of a network request.
...
The <Tooltip text="temporality" content="Whether a metric's data points report a running total or only the change since the last export." link="https://signoz.io/docs/glossary/#temporality" /> of a metric determines how the measurement is reported over time. SigNoz supports the following types of temporality:

- **Cumulative**: A <Tooltip text="cumulative" content="A metric temporality in which each successive data point repeats the same start timestamp and reports a running total." link="https://signoz.io/docs/glossary/#cumulative-temporality" /> metric stores the accumulated value of the metric since the start of the process. For example, an odometer in a vehicle.

- **Delta**: A <Tooltip text="delta" content="A metric temporality in which each successive data point advances the start timestamp and reports only the change since the previous export." link="https://signoz.io/docs/glossary/#delta-temporality" /> metric stores the change in the value of the metric since the last time it was reported. For example, the number of requests received by a server. Each time the metric is reported, the value is reset to zero.
...
- **Temporal Aggregation**: <Tooltip text="Temporal aggregation" content="Combining the data points inside one time series into one value for each interval." link="https://signoz.io/docs/glossary/#temporal-aggregation" /> is the process of aggregating the measurements over a specific period within the same time series.

- **Spatial Aggregation**: <Tooltip text="Spatial aggregation" content="Combining the values of several time series into one value for each interval, after temporal aggregation." link="https://signoz.io/docs/glossary/#spatial-aggregation" /> is the process of aggregating the measurements across multiple time series after temporal aggregation.
...
<Figure
  ...
  caption="Raw memory usage measurements"
/>
...
<Figure
  ...
  caption="Grouped memory usage measurements"
/>
...
<Admonition type="tip">
Gauge metrics do not support Rate or Increase temporal aggregation. To compute changes over time for a Gauge metric, use the **Time Shift** function with a formula:

1. Create your Gauge query (query **A**).
2. Clone it as query **B** and add a **Time Shift** function to **B**, with the shift value in seconds (for example, `3600` for a 1-hour comparison).
3. Add a formula **A − B** to calculate the difference between the current and shifted values.

See the <a href="https://signoz.io/docs/userguide/query-builder-v5/#functions-for-extended-data-analysis" target="_blank" rel="noopener noreferrer nofollow">Time Shift function documentation</a> for details.
</Admonition>
...
<Figure
  ...
  caption="Aggregated memory usage measurements within series"
/>
...
<Figure
  ...
  caption="Grouped memory usage measurements by host"
/>
...
<Figure
  ...
  caption="Aggregated memory usage measurements by host"
/>

The final result is the memory usage per host for each interval.

Pods are the real-world equivalent of hosts here. SigNoz's Metrics Explorer applies the same Avg (temporal) and Sum (spatial) aggregation, grouped by pod:

<Figure
  src="/img/docs/metrics/aggregation/gauge_metrics_explorer.webp"
  alt="Gauge metric query in SigNoz Metrics Explorer with Avg temporal and Sum spatial aggregation, grouped by pod"
  caption="Avg (temporal) and Sum (spatial) aggregation on a Gauge metric in SigNoz's Metrics Explorer"
/>
...
<Figure
  ...
  caption="Raw request count measurements (cumulative)"
/>
...
<Figure
  ...
  caption="Aggregated request count measurements within series"
/>
...
<Figure
  ...
  caption="Grouped request count measurements by service"
/>
...
<Figure
  ...
  caption="Aggregated request count measurements by service"
/>

The final result is the number of requests received per service for each interval.

SigNoz's Metrics Explorer applies the same Increase (temporal) and Sum (spatial) aggregation on a cumulative Counter metric, grouped by service:

<Figure
  src="/img/docs/metrics/aggregation/counter_metrics_explorer.webp"
  alt="Counter metric query in SigNoz Metrics Explorer with Increase temporal and Sum spatial aggregation, grouped by service"
  caption="Increase (temporal) and Sum (spatial) aggregation on a Counter metric in SigNoz's Metrics Explorer"
/>
...
<Figure
  ...
  caption="Raw request count measurements (delta)"
/>
...
A Histogram metric works the same way in SigNoz's Metrics Explorer: P95 spatial aggregation grouped by service, with no separate temporal aggregation control since the values are already bucketed:

<Figure
  src="/img/docs/metrics/aggregation/histogram_metrics_explorer.webp"
  alt="Histogram metric query in SigNoz Metrics Explorer with P95 spatial aggregation, grouped by service"
  caption="P95 spatial aggregation on a Histogram metric in SigNoz's Metrics Explorer"
/>
