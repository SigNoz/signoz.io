...
<Admonition type="tip">
Gauge metrics do not support Rate or Increase temporal aggregation. To compute changes over time for a Gauge metric, you can use the Time Shift function with a formula: clone the query (e.g., as query B), apply a Time Shift function to the clone (with the shift value in seconds matching your desired comparison interval), and create a formula `A - B` to calculate the difference between the current and shifted values. See the <a href="https://signoz.io/docs/userguide/query-builder/#functions-for-extended-data-analysis">Time Shift function documentation</a> for details.
</Admonition>
