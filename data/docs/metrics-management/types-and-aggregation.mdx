...
<Admonition type="tip">
Gauge metrics do not support Rate or Increase temporal aggregation. To calculate changes over time for a Gauge metric (for example, a cumulative gauge like `k8s.container.restarts`), you can combine the Time Shift function with a formula:

1. Create query **A** for the Gauge metric.
2. Clone it as query **B** and apply a **Time Shift** function to B (the shift value is in seconds, e.g., `3600` for a 1-hour comparison interval).
3. Add a formula **A − B** to compute the difference between the current and time-shifted values.
<Admonition type="tip">
...
See the <a href="https://signoz.io/docs/userguide/query-builder/#functions-for-extended-data-analysis">Time Shift function documentation</a> for details.
</Admonition>
