...
See the <a href="https://signoz.io/docs/userguide/query-builder/#functions-for-extended-data-analysis" target="_blank" rel="noopener noreferrer nofollow">Time Shift function documentation</a> for details.
</Admonition>
...
### How Histograms Affect Metric Volume

In SigNoz, each histogram metric is decomposed into multiple time series per unique label combination:

- **One series per bucket** — each bucket (e.g., `le="0.005"`, `le="0.01"`, ..., `le="+Inf"`) is stored as a separate time series. The default OpenTelemetry SDK histogram configuration defines 14 explicit bucket boundaries, which produces 15 bucket series (one per boundary plus the always-present `+Inf` bucket).
- **A `_sum` series** — tracks the sum of all observed values.
- **A `_count` series** — tracks the total number of observations.

Each of these series produces a separate datapoint every scrape/export interval. For example, a histogram with the default 14 bucket boundaries and one unique label combination generates 17 datapoints per interval (15 bucket series + `_sum` + `_count`). If that histogram has 10 unique label combinations, it generates 170 datapoints per interval.

This is standard OTLP histogram behavior. When evaluating metric volume, account for this multiplier effect — histograms naturally produce significantly more datapoints than counters or gauges with the same number of label combinations.
