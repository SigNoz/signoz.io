...
1. **Aggregation is per-batch only.** As the processor's own documentation states, it <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/metricstransformprocessor/README.md" target="_blank" rel="noopener noreferrer nofollow">only aggregates within a single batch and is not suitable for aggregating metrics from multiple sources</a>. Series exported by different applications, hosts, or scrape targets arrive in different batches (and often at different collector instances), so they never meet in one batch and never collapse.

2. **Resource attributes aren't aggregatable.** The labels that carry the most cardinality (`service.instance.id`, `host.name`, `k8s.pod.uid`) are *resource* attributes. `aggregate_labels` merges data points within a metric, but data points under different resources are never merged, so series that differ only in resource attributes don't collapse at all.
