...
2. **Resource attributes aren't aggregatable.** The labels that carry most cardinality — `service.instance.id`, `host.name`, `k8s.pod.uid` — are *resource* attributes. `aggregate_labels` merges data points within a metric, but data points under different resources are never merged, so series that differ only in resource attributes don't collapse at all.
