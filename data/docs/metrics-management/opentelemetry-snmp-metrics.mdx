...
<Admonition type="info">
  The snmp receiver is at `alpha` stability upstream. Its configuration format can change between Collector releases.
</Admonition>
...
Set the community string and restart the Collector. On a systemd install from the VM guide, add the variable to the Collector's environment file and restart the service:

```bash
echo 'SNMP_COMMUNITY=<community-string>' | sudo tee -a /etc/otelcol-contrib/otelcol-contrib.conf
...
Each device reports its own `sysName` as `device.name`. If devices use different community strings, set `community` on each receiver.
