...
`severity_text` and `severity_number` are top-level fields, so **Parse Severity Value From** can't point at them directly (see the field path warning above). If the same value also exists in an attribute, parse from that attribute instead. Otherwise, copy `severity_text` into an attribute at the collector first, then parse from the copy:
...
Set **Parse Severity Value From** to `attributes.level`.
