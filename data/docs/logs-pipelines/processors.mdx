...
description: Complete list of log processors available in Signoz.
doc_type: reference
---
...
<Admonition type="warning" title="Field paths must start with body, attributes, or resource">

Field paths (Parse From, Parse To, From, To, Field) must start with `body`, `attributes`, or `resource`. Top-level fields like `severity_text`, `severity_number`, and `timestamp` are not valid here, even though the UI offers them as options. Using one fails with `value should have prefix of body, attributes, resource`.

</Admonition>

If the original value also exists in the body or an attribute, point the processor there instead, for example `attributes.level`. Severity matching is case insensitive, so `info` and `INFO` map to the same level.

If `severity_text` is the only copy, normalize it at the collector before ingestion, for example with a <a href="https://opentelemetry.io/docs/collector/transforming-telemetry/" target="_blank" rel="noopener noreferrer nofollow">transform processor</a>:
...
using <a href="https://www3.ntu.edu.sg/home/ehchua/programming/howto/Regexe.html" target="_blank" rel="noopener noreferrer nofollow">regular expressions</a>.
...
You can follow [Parsing Logs with Regex](https://signoz.io/docs/logs-pipelines/guides/regex-parser/) guide for more details.

## Grok

The Grok processor can be used to extract information out of text
using grok patterns.  
<a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/grok.html" target="_blank" rel="noopener noreferrer nofollow">Grok</a> is a regular expression dialect with convenient <a href="https://github.com/vjeantet/grok/blob/master/patterns.go" target="_blank" rel="noopener noreferrer nofollow">aliases</a> for commonly used expressions.
...
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Name            | A descriptive name for the processor.                                                                                                               |
| Pattern         | The <a href="https://grokdebugger.com/" target="_blank" rel="noopener noreferrer nofollow">grok pattern</a> to be used. Must include atleast one named capture group                                              |
| Parse&#160;From | The log text field to parse from. Eg: `body` or `attributes.sessionInfo`                                                                            |
...
You can follow [Parsing Logs with Grok](https://signoz.io/docs/logs-pipelines/guides/grok-parser/) guide for more details.
...
| Timestamp&#160;Format&#160;Type                         | Type of timestamp value to be parsed. <br/> `epoch` can be used for parsing <a href="https://en.wikipedia.org/wiki/Unix_time" target="_blank" rel="noopener noreferrer nofollow">unix time</a> and `strptime` can be used for parsing human readable values using <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/internal/coreinternal/timeutils/internal/ctimefmt/ctimefmt.go#L68" target="_blank" rel="noopener noreferrer nofollow">ctime-like directives</a> such as %Y (4-digit year) and %H (2-digit hour). |
...
The value field can be set to an <a href="https://github.com/open-telemetry/opentelemetry-log-collection/blob/main/docs/types/expression.md" target="_blank" rel="noopener noreferrer nofollow">expression</a> which will get evaluated for each entry to set the value.  
