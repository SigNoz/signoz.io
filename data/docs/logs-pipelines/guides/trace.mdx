...
description: Parse trace ID, span ID, and trace flags from log attributes into standard OpenTelemetry fields using SigNoz Log Pipelines.
doc_type: tutorial
---

## Overview
If your logs contain trace information outside the <a href="https://opentelemetry.io/docs/specs/otel/logs/data-model/#log-and-event-record-definition" target="_blank" rel="noopener noreferrer nofollow">standard OpenTelemetry fields</a>, you can use log pipelines to parse that information into the right fields and enable correlation of your logs to and from corresponding traces.  
...
If your logs contain trace information in the body, you can [parse them](https://signoz.io/docs/logs-pipelines/guides/json/) out into their own attributes before populating trace information based on them.
...
- **Filter:** Use the filter field to select the logs you want to process. For example, `service.name = checkout`.
...
<Figure
  ...
  caption="Expanded Pipeline with Processors for parsing trace information out of log attributes."
/>
