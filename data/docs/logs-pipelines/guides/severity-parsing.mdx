...
```json
{
  ...
  "resource": {
    "k8s.component.name": "controller",
    "k8s.container.name": "controller",
    "k8s.pod.name": "nginx-ingress-data-controller-ccc549bb9-sb4bp"
  },
  "attributes": {
    "day.sale.count": "1"
  }
...
We'll use four processors in SigNoz Logs Pipeline to transform the example log into the desired format:
...
4. **Remove Processor** – Clean up temporary fields we no longer need.
...
```json
{
  ...
  "attributes": {
    "parsed_body": {
      "message": "applying telemetry collection strategy for region",
      "otel": {
        "trace_id": "000000000000000018c51935df0b93b9",
        "span_id": "18c51935df0b93b9", 
        "severity_text": "info",
...
{
  "timestamp": "2025-01-25T10:00:00Z",
  "body": "{\"message\":\"applying telemetry collection strategy for region\",\"otel\":{...}}",
  "attributes": {
    "parsed_body": {
      "message": "applying telemetry collection strategy for region",
      "otel": {
...
## Step 4: Move and Clean Up – Organize Final Structure
...
- Clean trace information preserved in `attributes.otel`.
- All original resource and attribute information maintained.
...
You can test this pipeline using the following cURL request:
            ...
            "resource": {
                "attributes": [
                    ...
                    },
                    {
                        "key": "k8s.pod.name",
                        "value": {
                            "stringValue": "nginx-ingress-data-controller-ccc549bb9-sb4bp"
                        }
                    }
                ]
            },
                    ...
                    "logRecords": [
                        {
                            ...
                            "flags": 0,
                            "body": {
                                "stringValue": "{\"message\":\"applying telemetry collection strategy for region\",\"otel\":{\"trace_id\":\"000000000000000018c51935df0b93b9\",\"span_id\":\"18c51935df0b93b9\",\"severity_text\":\"info\",\"severity_number\":\"4\"}}"
                            },
                            "attributes": [
                                {
                                    "key": "day.sale.count",
...
After setting up your pipeline and sending this test log, you should see the severity information properly extracted and mapped in your SigNoz logs explorer.
