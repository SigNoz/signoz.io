...
description: Parse serialized JSON log bodies into structured attributes using SigNoz Log Pipelines with flattening and mapping options.
doc_type: tutorial
---
...
[filter your logs based on JSON data in the body](https://signoz.io/docs/userguide/logs_query_builder/#filtering-and-search).  
...
[populate trace details](https://signoz.io/docs/logs-pipelines/guides/trace/) in your log records from the parsed attributes, enabling correlation of your logs to and from corresponding traces.  
...
## Prerequisites
 - You are [sending logs to SigNoz](https://signoz.io/docs/userguide/logs_query_builder/).
...
- **Filter:** Use the filter field to select the logs you want to process. For example, `service.name = checkout`. Note that while it is not ideal, it is ok if your filter selects other non-JSON logs too.
...
<Figure
  ...
  caption="Creating a Pipeline adds it to the end of the Pipelines List. It can be expanded by clicking the highlighted icon."
/>
...
<Figure
  ...
  caption="Expanding a pipeline shows the Add Processor button"
/>
...
<Figure
  ...
  caption="Add Move Processor Dialog"
/>
...
<Figure
  ...
  caption="Remove Processor Dialog"
/>
...
<Figure
  ...
  caption="Expanded Pipeline with Processors for parsing desired fields out of JSON body into their own log attributes"
/>
...
<Figure
  ...
  caption="Pipeline Preview with Processed Output"
/>
