...
<KeyPointCallout title="Why this deprecation?" defaultCollapsed={false}>
The old setup used a separate **signoz-schema-migrator** Job; otel-collector (and init containers) had to wait for that Job to exist and complete before starting. That caused "Job not found" on installs/upgrades, brittle workarounds, and stacks getting stuck when the migration Job retried or timed out.

**Now:** Migrations live inside **signoz-otel-collector** (`migrate` subcommands). One **signoz-telemetrystore-migrator** replaces sync + async migrators, and the collector uses a ClickHouse-based check (`migrate sync check`) instead of depending on the Job — so startup no longer hinges on Job ordering. [Details →](https://github.com/SigNoz/charts/issues/828)
</KeyPointCallout>
...
If you are using the default Docker Compose files from the repo, no manual changes are required. Follow the [standard upgrade guide](https://signoz.io/docs/operate/migration/upgrade-standard/#docker-standalone) to pull the latest changes and restart.
</Admonition>
...
<Admonition type="info">
If you are using the default Docker Compose files from the repo, no manual changes are required. Follow the [standard upgrade guide](https://signoz.io/docs/operate/migration/upgrade-standard/#docker-swarm-cluster) to pull the latest changes and restart.
</Admonition>
