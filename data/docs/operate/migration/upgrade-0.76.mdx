...
```
Replace `platform` with your SigNoz namespace and `my-release-signoz-query-service-0` with your query-service pod name.

If you don't need to preserve your SQLite data, you can skip the migration steps and directly upgrade the helm release.
</Admonition>
...
If no PVCs are found, skip to step 3.
...
> Note that the `claimName` is the name of the PVC for the old `query-service` i.e. `signoz-db-my-release-signoz-query-service-0`. Replace it with the name of the PVC for your release.
...
with a single container (`signoz-signoz`).
