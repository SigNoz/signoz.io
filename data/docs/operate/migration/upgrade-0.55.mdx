...
For migrating the data data you will need to get start and end timestamp which is the duration of logs data that you want to migrate from the old table to the new table.
...
* Get the ending timestamp
  ...
  ```
  Incase you find empty response for the above query, please make sure that you upgraded to version `>=0.53` first.
...
Here 
* **start_ts** is the starting timestamp for the logs data to be migrated
* **end_ts** is the ending timestamp for the logs data to be migrated
* **batch_duration** is the window size of the data that will be fetched from the old table
* **batch_size** is the size of the batches that will be written to the new table
...
In case of failure and have to run again, 
  ...
  containers:
    - name: signoz-migrate-data
      ...
      args:
        ...
        - "-dest_host=my-release-clickhouse"
        - "-username=admin"
        - "-dest_username=admin"
...
Please replace `<ADMIN_PASSWORD>`, `<START_TIMESTAMP>` and `<END_TIMESTAMP>`
