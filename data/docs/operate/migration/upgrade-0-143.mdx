...
The default session provider changes from JWT to opaque tokens. Sessions issued before the upgrade are not valid afterwards, so every user is signed out once and logs back in. If you want to keep JWT sessions, see [Keep the JWT session provider](#keep-the-jwt-session-provider) before you upgrade, because JWT without a secret now refuses to start.
</Admonition>
...
| Set `SIGNOZ_TOKENIZER_PROVIDER=jwt` explicitly | Set `SIGNOZ_TOKENIZER_JWT_SECRET` too, or SigNoz will not start |
...
SigNoz v0.143.0 issues opaque session tokens by default. To keep JWT sessions, set both variables on the SigNoz container before you upgrade:

```bash
SIGNOZ_TOKENIZER_PROVIDER=jwt
SIGNOZ_TOKENIZER_JWT_SECRET=<a long random secret>
```

SigNoz refuses to start with the `jwt` provider and an empty secret. If your deployment sets `SIGNOZ_JWT_SECRET`, rename it to `SIGNOZ_TOKENIZER_JWT_SECRET`. See [Configure the JWT Secret](https://signoz.io/docs/manage/administrator-guide/configuration/jwt-secret/) for details.
