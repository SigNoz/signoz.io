...
<Admonition type="info" title="The collector config needs two new processors">
...
`cast` regenerates the collector config with the two processors when the ingester image is `latest` or v0.144.11 and later, and keeps the previous config for an older pin. If your <a href="https://github.com/SigNoz/foundry/blob/main/docs/reference/casting-file.md" target="_blank" rel="noopener noreferrer nofollow">casting.yaml</a> pins image versions, bump SigNoz to v0.143.0 and the collector to v0.144.11 yourself; `cast` refuses SigNoz v0.143.0 paired with a collector below v0.144.11.
