...
5. Starting with release v0.88, you should remove the deprecated `clickhousemetricswrite` exporter from the `otel-collector` configuration and use only the `dot-metrics` exporter `signozclickhousemetrics`. 
```diff
processors:
  signozspanmetrics/delta:
-    metrics_exporters: clickhousemetricswrite, signozclickhousemetrics
