...
Add the following to your `docker-compose.yml` file, Kindly make sure that your Clickhouse conttainer is running and accessible.

```yaml
  migration-job:
    !!merge <<: *db-depend
    image: signoz/migrate:v0.70.5
    # Equivalent to the Job’s args
    command: >
      migrate-data
...
```bash
docker-compose logs -f migrate-job
...
Add the following service in your docker compose setup or run the container seprately 
