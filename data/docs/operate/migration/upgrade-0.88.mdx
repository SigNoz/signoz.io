...
Before starting the migration, make sure that your `signoz-otel-collector` setup meets the following requirements:
    ...
    - Both `clickhousemetricswrite` and `signozclickhousemetrics` exporters are enabled ([dual exporter setup](#step-0-enable-dual-exporter)) during your retention period for metrics
</Admonition>
## Migration
