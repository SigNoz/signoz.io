...
- Ensure the following requirements are met for `signoz-otel-collector`:
    - Version is greater or equal to `v0.111.29`
    - Both `clickhousemetricswrite` and `signozclickhousemetrics` exporters are enabled ([dual exporter setup](#step-0-enable-dual-exporter))
...
```
<Admonition type="info">
After upgrading, make sure to complete the step [Remove Deprecated Exporter and Use Only Dot-Metrics Exporter](#remove-deprecated-exporter-and-use-only-dot-metrics-exporter) as old exporter is no longer supported in releases after v0.88.
</Admonition>
...
## Remove Deprecated Exporter and Use Only Dot-Metrics Exporter

Starting with release v0.88, you must remove the legacy `clickhousemetricswrite` exporter from your otel-collector configuration and use only the dot-metrics exporter `signozclickhousemetrics`.

Below is the configuration section where you should remove all references to "clickhousemetricswrite":
```diff
processors:
  signozspanmetrics/delta:
-    metrics_exporters: clickhousemetricswrite, signozclickhousemetrics
