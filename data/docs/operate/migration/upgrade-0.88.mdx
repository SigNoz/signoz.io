...
```
<Admonition type="info">
After upgrading, make sure to complete the step [Remove Old Exporter and Use Only Dot-Metrics Exporter](#remove-old-exporter-and-use-only-dot-metrics-exporter) as old exporter is no longer supported in releases after v0.88.
</Admonition>
...
## Remove Old Exporter and Use Only Dot-Metrics Exporter
Starting with release v0.88, make sure to remove the `clickhousemetricswrite` exporter from your otel-collector configuration and use only the dot-metrics exporter `signozclickhousemetrics`.

Below is the configuration section where "clickhousemetricswrite" should be removed:
```diff
processors:
  signozspanmetrics/delta:
-    metrics_exporters: clickhousemetricswrite, signozclickhousemetrics
