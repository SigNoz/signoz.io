...
date: 2024-12-17
id: upgrade-0.63
title: Upgrade to v0.63 from earlier versions
---

This setup assumes you've installed SigNoz version `>=0.60.0` && `<v0.63.0` and have kept it running for at least 15 days (the default retention period). 

If you changed the default retention period, please make the adjustments outlined in the [following step](#config-to-keep-using-old-table) before upgrading to v0.63.0. 
...
After upgrading to SigNoz version v0.63 i.e. SigNoz chart version v0.61.0, you need to run the migration script to copy the TTL settings and materialized columns from the old table.
...
If you already kept SigNoz version `>=0.60.0` && `<v0.63.0` running for at-least 15 days i.e the default retention period this is not required as data will already be backfilled.
