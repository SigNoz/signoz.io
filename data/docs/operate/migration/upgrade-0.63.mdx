...
date: 2024-12-17
id: upgrade-0.63
title: Upgrade to v0.63.1 from earlier versions
---

This setup assumes you've installed SigNoz version `>=0.60.0` && `<v0.63.1` and have kept it running for at least 15 days (the default retention period). 

If you changed the default retention period, please make the adjustments outlined in the [following step](#config-to-keep-using-old-table) before upgrading to v0.63.1. 
...
After upgrading to SigNoz version v0.63.1 i.e. SigNoz chart version v0.61.1, you need to run the migration script to copy the TTL settings and materialized columns from the old table.
...
### For Docker
```bash
docker run --name signoz-migrate-63 --network clickhouse-setup_default \
  -it -d signoz/migrate:0.63 -host=clickhouse -port=9000
```

Steps to check logs:

```bash
docker logs -f signoz-migrate-63
```

In case of failure and have to run again, make sure to cleanup the container before running the migration script again.

```bash
docker stop signoz-migrate-63

docker rm signoz-migrate-63
```
...
kubectl -n $NAMESPACE run -i -t signoz-migrate-63 --image=signoz/migrate:0.63 --restart='Never' \
...
If you already kept SigNoz version `>=0.60.0` && `<v0.63.1` running for at-least 15 days i.e the default retention period this is not required as data will already be backfilled.
...
docker run --name signoz-migrate-55-data --network clickhouse-setup_default \
  -it -d signoz/migrate:0.63-data -host=clickhouse -port=9000 -dest_host=clickhouse -dest_port=9000 -start_ts=<START_TIMESTAMP> -end_ts=<END_TIMESTAMP> -batch_size=40000 -batch_duration=5 -count_delta_allowed=10000
```
...
```bash
docker logs -f signoz-migrate-63-data
```
...
* Make sure to cleanup the container before running the migration script again. 
  ```bash
  docker stop signoz-migrate-63-data

  docker rm signoz-migrate-63-data
  ```
...
kind: Pod
metadata:
  name: signoz-migrate-data-63
spec:
  restartPolicy: Never
  containers:
    - name: signoz-migrate-data
      image: signoz/migrate:0.63-data
      args:
...
Now, create the `signoz-migrate-data-63` pod using the YAML:
...
```bash
kubectl -n $NAMESPACE logs -f signoz-migrate-data-63
```
...
* Make sure to cleanup the container before running the migration script again. 
  ```bash
  kubectl -n $NAMESPACE delete pod signoz-migrate-data-63
  ```
