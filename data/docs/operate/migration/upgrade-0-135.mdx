...
In v0.135.0 the public sharing routes (`/api/v1/dashboards/{id}/public` and `/api/v1/public/dashboards/{id}`) are not affected and keep working. This changed later: `/api/v1/public/dashboards/{id}` (and its widget query sub-path) is removed in v0.143.0. See the [v0.143.0 upgrade guide](https://signoz.io/docs/operate/migration/upgrade-0-143/#v1-dashboard-routes-are-deleted) for the public-route migration. The share-config route `/api/v1/dashboards/{id}/public` still works.
