...
**Dashboard pins reset on upgrade.** Upgrading to `<version>` or later runs a database migration
that drops and recreates the dashboard pin table. Previously pinned dashboards become unpinned, and
existing pin data is not preserved. No action is required after the upgrade — re-pin dashboards
manually as needed.
</Admonition>
