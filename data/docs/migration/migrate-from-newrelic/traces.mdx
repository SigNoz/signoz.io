...
```sql
SELECT uniques(entity.name), latest(instrumentation.provider), latest(language) 
FROM Span 
FACET entity.name 
SINCE 7 days ago 
LIMIT MAX
...
func initTracer() (*trace.TracerProvider, error) {
    exporter, err := otlptracegrpc.New(context.Background(),
        otlptracegrpc.WithEndpoint("ingest.<region>.signoz.cloud:443"),
...
For detailed configuration, see [Go Instrumentation](https://signoz.io/docs/instrumentation/opentelemetry-golang/).
