...
FACET 
  CASES(
    WHERE entity.name IS NOT NULL THEN 'service',
    WHERE hostname IS NOT NULL THEN 'host',
    WHERE filePath IS NOT NULL THEN 'file'
  ) AS type,
  coalesce(entity.name, hostname, filePath) AS value
LIMIT MAX
```
...
FROM Log
FACET
  coalesce(instrumentation.provider, 'unknown'),
  coalesce(entity.name, 'unknown')
SINCE 7 days ago
LIMIT MAX
```
