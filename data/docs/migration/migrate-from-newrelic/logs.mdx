...
1. **Full-text search**: Search across all logs with powerful text search capabilities
2. **Advanced filtering**: Filter logs by any attribute or field
3. **Live Tail**: Watch logs in real-time as they arrive
4. **Trace correlation**: Connect logs to related traces for context
