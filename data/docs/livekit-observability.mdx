...
This guide walks you through setting up observability and monitoring for LiveKit using [OpenTelemetry](https://opentelemetry.io/) and exporting logs, traces, and metrics to SigNoz. With this integration, you can observe performance of various models, capture request/response details, and track system-level metrics in SigNoz, giving you real-time visibility into latency, error rates, and usage trends for your LiveKit applications.
...
- **`OTEL_RESOURCE_ATTRIBUTES`**: [Resource attributes](https://opentelemetry.io/docs/languages/sdk-configuration/general/) to identify your service
- **`OTEL_EXPORTER_OTLP_ENDPOINT`**: The SigNoz Cloud OTLP endpoint  
- **`OTEL_EXPORTER_OTLP_HEADERS`**: Authentication header with your ingestion key
- **`OTEL_EXPORTER_OTLP_PROTOCOL`**: Protocol for OTLP export (grpc or http/protobuf)
- **`OTEL_TRACES_EXPORTER`**: Enables OTLP trace export
- **`OTEL_METRICS_EXPORTER`**: Enables OTLP metrics export  
- **`OTEL_LOGS_EXPORTER`**: Enables OTLP logs export
- **`OTEL_PYTHON_LOG_CORRELATION`**: [Enables trace context injection](https://opentelemetry.io/docs/zero-code/python/logs-example/) into logs for correlation
- **`OTEL_PYTHON_LOGGING_AUTO_INSTRUMENTATION_ENABLED`**: [Enables automatic instrumentation](https://opentelemetry.io/docs/zero-code/python/logs-example/) of Python logging
