...
The Traces Explorer in SigNoz provides a feature for quick filtering of traces using various parameters. 
This functionality allows users to quickly search their specific traces, making it easier to analyze and debug applications. 
...
1. **Duration**:
   - **Description**: Filter traces based on their duration.
   - **Usage**: Specify a minimum and maximum duration in milliseconds to find traces that fall within this time range. This is useful for identifying traces that are unusually long or short.
   
2. **Status**:
   - **Description**: Filter traces by their status.
   - **Usage**: Options can include `Error` and `Ok`. Use this filter to quickly identify traces that resulted in errors or were successful.

3. **Service Name**:
   - **Description**: Filter traces by the name of the service that generated them.
   - **Usage**: Select from a list of service names to isolate traces from a specific service. This can be helpful in microservices architectures where multiple services generate traces.

4. **Operation / Name**:
   - **Description**: Filter traces based on the operation or name.
   - **Usage**: Select specific operations, such as HTTP GET requests or SQL SELECT queries, to focus on particular types of operations within your traces.

5. **RPC Method**:
   - **Description**: Filter traces by the RPC method used.
   - **Usage**: Useful for isolating traces based on the remote procedure call methods.

6. **Status Code**:
   - **Description**: Filter traces by their HTTP status code.
   - **Usage**: Select from common status codes like `200`, `404`, `500`, etc., to find traces that resulted in specific HTTP responses.

7. **HTTP Host**:
   - **Description**: Filter traces by the HTTP host.
   - **Usage**: Specify the host to find traces related to particular HTTP hosts.

8. **HTTP Method**:
   - **Description**: Filter traces by the HTTP method.
   - **Usage**: Choose HTTP methods such as `GET`, `POST`, etc., to filter traces based on the type of HTTP request.

9. **HTTP Route**:
   - **Description**: Filter traces by the HTTP route.
   - **Usage**: Specify routes to find traces related to specific paths in your application.

10. **HTTP URL**:
    - **Description**: Filter traces by the full HTTP URL.
    - **Usage**: Useful for isolating traces related to specific URLs, providing a more granular filter compared to HTTP routes.

11. **Trace ID**:
    - **Description**: Filter traces by their unique Trace ID.
...
4. **Applying Filters**: Once you set the filter values, the Traces Explorer will automatically update the displayed traces based on the selected filters.
