...
2. Your **API key** — go to **Settings → Service Accounts** in SigNoz, create a service account, and generate an API key (requires **Admin** role). The service account must have at least a **Viewer** role assigned for MCP tools to access observability data. See [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) for details.
...
3. Create a service account, assign at least a **Viewer** role, open its **Keys** tab, click **Add Key**, and copy the generated key. See [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) for details.
...
### 403 Forbidden errors on tool calls

If the MCP connection succeeds but tool calls fail with a 403 error, the service account's API key is valid but the account lacks the required role. Go to **Settings → Service Accounts** in SigNoz and assign at least a **Viewer** role to the service account. See [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) for details.
