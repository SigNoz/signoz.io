...
Only **Admin** users can create service account keys. If you don't see the option, contact your workspace administrator.
</Admonition>
...
<Admonition type="warning">
Only **Admin** users can create service account(API) keys. Keep your key secure — never commit it to version control.
</Admonition>
...
docker run -p 8000:8000 \
  ...
  -e SIGNOZ_API_KEY=<YOUR_SERVICE_ACCOUNT_KEY> \
  signoz/signoz-mcp-server:latest
```

- `<your-signoz-url>`: Your self-hosted SigNoz instance URL (e.g., `http://localhost:3301`)
- `<YOUR_SERVICE_ACCOUNT_KEY>`: The API key from [Fetch your API key](#fetch-your-api-key)
  ...
  "mcpServers": {
    "signoz": {
      ...
      "env": {
        ...
        "SIGNOZ_API_KEY": "<YOUR_SERVICE_ACCOUNT_KEY>",
        "LOG_LEVEL": "info"
      }
    }
  }
...
codex mcp add signoz \
  ...
  --env SIGNOZ_API_KEY="<YOUR_SERVICE_ACCOUNT_KEY>" \
  --env LOG_LEVEL=info \
...
| `SIGNOZ_API_KEY` | Service account key for authenticating with SigNoz — see [Service Accounts](https://signoz.io/docs/manage/administrator-guide/iam/service-accounts/) | _(required for stdio; optional for http with OAuth)_ |
...
- For self-hosted HTTP mode, ensure the `Authorization` header format is `Bearer <YOUR_SERVICE_ACCOUNT_KEY>`.
