...
3. **Error rate increased**: Error rate increased from pre to psot deployment, with the newly introduced error accounting for the majority. 
...
| 1 | `signoz_aggregate_traces` | Computed p99 latency, error rates, and request counts for each operation in the 30-minute window before deployment (9:30 AM - 10:00 AM PST) |
