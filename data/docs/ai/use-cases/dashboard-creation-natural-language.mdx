...
<Figure
  src="/img/docs/ai/use-cases/recommendation-dashboard.webp"
  alt="Recommendation Service Dashboard"
  caption="Recommendation Service Dashboard Overview"
/>

<Figure
  src="/img/docs/ai/use-cases/recommendation-dashboard-detailed.webp"
...
- **Start with golden signals.** For service monitoring, prioritize latency (p95/p99), error rate, and request throughput.
- **Group related panels together.** When building multi-panel dashboards, organize by concern: service health, dependency status, infrastructure, etc. This makes incident triage faster.
- **Include service verification upfront.** Before building a dashboard, confirm the service is actually sending data. This avoids creating empty dashboards for typos or inactive services.
- **Iterate after creation.** Dashboards created from prompts are starting points. You can refine them in the SigNoz UI or ask your assistant to update specific panels.
