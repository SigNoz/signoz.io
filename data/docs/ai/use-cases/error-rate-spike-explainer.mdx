...
```text
Show me errors for the frontend service in the last 15 minutes grouped by error. 
```
...
ProductCatalogService/GetProduct is the dominant error source at 288 — far ahead of
everything else. Worth investigating that downstream dependency first.
```

The majority of errors (288 out of 811) are coming from `ProductCatalogService/GetProduct`. That is 35% of all failures. This points to a issue with the product catalog service, not a bug in the frontend itself.
...
- The flow is recommendations → GetProduct loop. The frontend calls
  ListRecommendations, which returns product IDs, and then the frontend loops
  and calls GetProduct for each one. One of those product IDs is coming back
  NOT_FOUND. This explains why you saw 288 GetProduct errors vs only 133
  /api/recommendations errors — each failing recommendations request produces
  multiple failing GetProduct calls.
```
...
With this information, you can now escalate to the database team with specific evidence: which service is affected (`productcatalog`), what the dependency is (`postgresql`), and the failure pattern (connection pool exhaustion, not query logic issues).
