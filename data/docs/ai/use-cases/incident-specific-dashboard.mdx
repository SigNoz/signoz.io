...
The checkout service is throwing errors. Your error rate alert fired 10 minutes ago and you need visibility into what's happening right now. Instead of manually parsing through all your telemetry data, you want a focused dashboard that shows exactly what's broken.
...
The dashboard provides a complete incident view, clearly showing that payment-service is the likely root cause with elevated errors, high latency, and recent pod restarts.
...
<Figure
  src="/img/docs/ai/use-cases/incident-dashboard.webp"
  alt="Recommendation Service Dashboard"
  caption="Recommendation Service Dashboard Overview"
/>

<Figure
  src="/img/docs/ai/use-cases/incident-dashboard-detailed.webp"
...
- **Include baseline comparisons in incident dashboards.** Comparing current metrics against the same time last week (or 7 days ago) immediately shows what's abnormal versus normal behavior patterns.
- **Start with dependencies, not just the alerting service.** The root cause is often in a downstream service. A dashboard showing dependency health alongside the primary service saves investigation time.
- **Group errors by operation name.** Instead of just counting total errors, group by operation/endpoint to quickly identify which specific code paths are failing.
- **Check infrastructure metrics last.** In most incidents, elevated CPU or memory is a symptom, not the cause. Start with service-level errors and latency, then verify infrastructure if needed.
