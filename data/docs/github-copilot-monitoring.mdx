...
When you click on a trace in SigNoz, you'll see a detailed view of the trace, including all associated spans, along with their events and attributes. The `invoke_agent` span wraps one agent turn and is the root, `chat` spans are the individual model calls carrying `gen_ai.request.model` and `gen_ai.usage.*`, `execute_tool` spans carry `gen_ai.tool.name` for each tool the agent ran, and `embeddings` spans come from workspace indexing.
...
Copilot also exports metrics on the standard GenAI instruments (`gen_ai.client.token.usage`, `gen_ai.client.operation.duration`) alongside Copilot-specific ones (`copilot_chat.tool.call.count`, `copilot_chat.agent.invocation.duration`, `copilot_chat.session.count`).
