...
None of the `github.copilot.chat.otel.*` settings are license gated in the shipped Copilot Chat extension manifest, so this works on a personal account with no enterprise configuration.
</Admonition>
...
`github.copilot.chat.otel.captureContent` is `false` by default and this guide leaves it off. Turning it on records full prompts, model responses, tool arguments, tool results, and system instructions as span attributes, which means source code and anything else in the context window leaves your machine. It is genuinely useful for debugging agent behavior, so decide deliberately. `github.copilot.chat.otel.maxAttributeSizeChars` caps how much content lands in any single attribute and defaults to `0`, meaning no truncation.
</Admonition>
