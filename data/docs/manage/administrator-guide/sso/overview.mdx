...
description: Overview of single sign-on (SSO) in SigNoz, including setup, role mapping, and user lifecycle management.
...
5. **Role Mapping**: Automatically assign SigNoz roles (VIEWER, EDITOR, ADMIN) based on IdP group memberships or a direct role attribute, so you don't need to assign roles manually.

<Admonition type="info">
On enterprise deployments, auth domain management uses fine-grained authorization. Creating or updating a domain with a role mapping requires the configuring user to also hold `role:attach` on every role the mapping assigns (and `role:detach` on any role the previous mapping assigned, since an update replaces the full mapping). If no default role is set, SSO users fall back to VIEWER, so the caller needs `role:attach` on `signoz-viewer`. See the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/#auth-domain).
</Admonition>
