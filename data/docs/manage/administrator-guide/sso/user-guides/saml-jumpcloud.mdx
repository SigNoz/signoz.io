...
- Centralize user management through JumpCloud
...
- **SigNoz account** (Cloud or Self-Hosted with License)
- **Admin access** to both JumpCloud and SigNoz platforms
...
1. **Entity ID**: Enter your SigNoz instance URL (e.g., `https://signoz.example.com`)
   - This uniquely identifies your SigNoz instance to JumpCloud
   - Use the same URL in both Entity ID fields shown in the image below

![JumpCloud SAML Application Configuration 1](/img/docs/jumpcloud-01.png)

2. **ACS URL**: Enter your SigNoz instance URL (e.g., `https://signoz.example.com`)
   - This is where JumpCloud will send authentication responses
   - Should match your SigNoz instance URL

### Step 3: Configure Additional Settings

Ensure the following settings match the configuration shown in the image below:

![JumpCloud SAML Application Configuration 2](/img/docs/jumpcloud-02.png)

**Important settings to verify:**
- **Name ID Format**: Set to "Email" or "Unspecified"
- **Signing Certificate**: Use the default certificate
- **Encryption**: Disabled (unless specifically required)
...
3. **Click** the SSO/SAML login option (if available) or try logging in with a JumpCloud user email
...
- **Redirect loops**: Verify the ACS URL in JumpCloud matches your SigNoz instance URL

## Next Steps

After successful configuration:
- Assign users to the SAML application in JumpCloud
- Test with different user accounts
- Configure user role mappings if needed
- Set up additional security policies in JumpCloud if required
