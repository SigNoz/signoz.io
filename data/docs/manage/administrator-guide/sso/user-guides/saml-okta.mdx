...
description: Configure SAML single sign-on between Okta and SigNoz for team authentication.
...
IdP-initiated login allows users to launch SigNoz directly from the Okta dashboard without visiting the SigNoz login page first.
...
3. In Okta, go to **Applications** → select your SigNoz app → **General** tab
4. Under **SAML Settings**, click **Edit**
5. On the **Configure SAML** step, set **Default RelayState** to the URL copied from SigNoz in step 2
6. Click **Next** and then **Finish** to save
...
- **IdP-initiated login fails**: Confirm that the **Default RelayState** in Okta's SAML settings matches the **IDP Initiated SSO URL** from SigNoz's Authenticated Domains page
