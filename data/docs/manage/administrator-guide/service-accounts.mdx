...
Service accounts replace the legacy API keys. If you previously used API keys, they have been migrated to service accounts automatically.
...
```bash
curl -X GET https://<SIGNOZ_URL>/api/v1/dashboards \
...
```bash
curl -s -o /dev/null -w "%{http_code}" https://<SIGNOZ_URL>/api/v1/dashboards \
  -H "SIGNOZ-API-KEY: <YOUR_SERVICE_ACCOUNT_KEY>"
```

A `200` status code indicates the key is valid and the service account has the required permissions. If you receive `401`, verify the key value is correct. A `403` indicates the service account lacks the necessary role for the requested resource.
...
- [Authentication and Login](/docs/userguide/authentication) — Manage user roles and permissions
- [Single Sign-On (SSO)](/docs/manage/administrator-guide/sso/overview) — Configure SSO for your organization
- [SigNoz MCP Server](/docs/ai/signoz-mcp-server) — Use service account keys to connect AI tools to SigNoz via MCP
