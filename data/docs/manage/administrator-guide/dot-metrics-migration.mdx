...
To verify the migration is successful, you can check the logs of the `migration` job logs:
...
After the migration is complete, you should remove the migration jobs from your Docker Swarm setup to prevent them from running again.**
...
### Step 1: Migrate Historical Data (For High Retention Users)
Please ensure that you have the `clickhouse` installed on your system and ClickHouse server is running and accessible.
...
To enable the new dot metrics, you need to set the environment variable `DOT_METRICS_ENABLED` to `true` in your SigNoz configuration. 

Follow these steps to update the environment variable for the `signoz` service:
...
- Join our [Community Slack](https://signoz.io/slack) for the support
