...
This section explains migrating to dot-metrics in a Kubernetes environment using Helm charts. It covers steps for migrating historical data, alerts, and dashboards, enabling dot metrics, and cleaning up migration jobs.
...
If you are deploying with Helm charts:

1. **Update your `values.yaml`** to include the migration configuration.
2. **Execute the migration commands** using the Docker method, ensuring your ClickHouse connection details match your Helm deployment.
3. **Check the migration** by inspecting your dashboards and alerts once it's finished.
