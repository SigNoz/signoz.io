---
date: 2025-06-24
id: dot-metrics-migration
title: Dot Metrics Migration Guide
---
...
SigNoz is transitioning from running both the old and new exporters simultaneously to running only the new dot-metrics exporter.
...
- The migration scripts are idempotent, so you do not need to worry about running them multiple times.
- After completing the migration, remove any configuration changes made for the migration, according to your deployment type.
</Admonition>
...
- Ensure a seamless transition to the new metrics system
...
- Record the count of total metrics, samples, and fingerprints to verify after migration.
- Review the environment variables used for configuring database connections, performance settings, and migration behavior. Understanding these variables is crucial for a successful migration.
...
This section covers migrating to dot-metrics in a Kubernetes environment using Helm charts. It includes steps for migrating historical data, alerts, and dashboards, enabling dot metrics, and cleaning up migration jobs.
...
If you are using Helm charts for deployment:

1. **Update your `values.yaml`** to include the migration configuration.
2. **Run the migration commands** using the Docker approach, ensuring your ClickHouse connection parameters match your Helm deployment.
...
This job runs alongside your SigNoz pods and connects to ClickHouse to perform insert operations for migrating older data (for users with high retention periods).
...
To migrate alerts and dashboards, add an init container to your SigNoz deployment. This init container runs before the main SigNoz container and executes the necessary queries on the SQLite database.
...
To migrate alerts and dashboards, run the migration script against your SQLite database using the same Docker image. If running separately, stop the SigNoz container first, as the script modifies the SQLite database.
...
To migrate alerts and dashboards, create another service in your Docker Swarm setup using the migration image. Stop the existing SigNoz service before running this migration job, as it modifies the SQLite database. Bring it back up after migration is complete.
...
Ensure ClickHouse is installed and running on your system. Run the migration command to backfill historical data:
...
Stop the SigNoz service before running the migration script, as it modifies the SQLite database. Ensure you have the correct user access to the SQLite database.
