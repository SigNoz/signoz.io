...
<Admonition type="info">
SigNoz reads the path component from the URL (e.g. `/signoz`) and uses it as the base path for all API routes and frontend assets. Health endpoints (`/api/v1/health`, `/api/v2/healthz`, `/api/v2/readyz`, `/api/v2/livez`) remain accessible without the prefix for container healthchecks.
</Admonition>
