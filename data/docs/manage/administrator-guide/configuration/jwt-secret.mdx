...
<Admonition type="info" title="Applies to the JWT token provider only">
Since v0.143.0 SigNoz defaults to the `opaque` session token provider, which does not use a JWT secret. The secret below matters only when you set `tokenizer.provider: jwt`. See [Session Token Provider Configuration](https://signoz.io/docs/manage/administrator-guide/configuration/session-tokens/).
</Admonition>

When the JWT provider is active, SigNoz uses a JWT secret key to sign and verify all user session tokens. **It is critical to set this secret to ensure the security of your SigNoz instance.**
