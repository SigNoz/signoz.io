---
date: 2026-09-16
title: Restrict Dashboard Access with Custom Roles
...
- **The list shows everything.** `list` is not filtered by `read`, so the list still shows every dashboard in the workspace. A row the role cannot read stays clickable and reports the denial when it is opened. Use `read` to restrict a dashboard's contents, not its presence in the list.
...
- **Denied controls are disabled, not hidden.** Editing, locking, publishing, and deleting each state the transaction they need on hover, so a user can tell what to ask for.
