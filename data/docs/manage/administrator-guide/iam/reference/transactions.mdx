...
This page currently covers roles, service accounts, API keys, telemetry resources (logs, traces, metrics, meter metrics), and quick filters. Transactions for other observability resources such as dashboards, alerts, and pipelines will be documented as they become available for fine-grained access control.
</Admonition>
...
**Resource:** `metaresource` · **Kind:** `quick-filter` · **Selector:** quick filter ID (use `*` for all quick filters)

Controls the quick filters panel in the Logs, Traces, Exceptions, API Monitoring, and Meter explorers. Quick filters on the infrastructure pages are not gated by these transactions.

| Relation | Description | Managed Role Access |
|----------|-------------|---------------------|
| `list` | List the quick filter configurations in the workspace. | signoz-admin, signoz-editor, signoz-viewer |
| `read` | View the quick filters panel in the explorers. Without it, the panel shows a permission-denied callout. | signoz-admin, signoz-editor, signoz-viewer |
| `update` | Edit quick filters (add, remove, or reorder them) from the settings panel. Requires `read` as well. | signoz-admin |
