...
This page currently covers roles, service accounts, API keys, dashboards, and telemetry resources (logs, traces, metrics, meter metrics). Transactions for other observability resources such as alerts and pipelines will be documented as they become available for fine-grained access control.
</Admonition>
...
The list is not filtered by `read`. A principal holding `list` sees every dashboard in the workspace, and a dashboard they cannot `read` reports the denial when they open it. Scope `read` to restrict a dashboard's contents, not its presence in the list.
