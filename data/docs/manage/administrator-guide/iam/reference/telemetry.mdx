...
Telemetry resources (logs, traces, metrics, meter metrics) control who can query telemetry data in SigNoz. They support a single relation, `read`, scoped by a telemetry selector. All three managed roles (`signoz-admin`, `signoz-editor`, `signoz-viewer`) have unrestricted (`*`) read access on every telemetry resource; restrictions apply only through custom roles.
