...
| `metaresource` | `dashboard` | Dashboards and their panels across the workspace |
| `metaresource` | `cloud-integration` | Connected cloud provider accounts (AWS, Azure, GCP) |
| `metaresource` | `auth-domain` | Authenticated domains and their SSO configuration |
| `telemetryresource` | `logs` | Log data collected across the workspace |
| `telemetryresource` | `traces` | Distributed tracing data collected across the workspace |
| `telemetryresource` | `metrics` | Metric data collected across the workspace |
| `telemetryresource` | `meter-metrics` | Usage metering data for the workspace |

For the full transactions reference for each resource, see the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/).
...
| **Only selected** | Resource name or ID | The transaction applies only to the specified instances. Refer to the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/) for the selector format used by each resource. |
| **None** | No selector | The transaction is not granted for any instance of that resource. |

<Admonition type="info">
Telemetry resources (logs, traces, metrics, meter metrics) use a query-scoped selector format instead of instance IDs: transactions are scoped by query type and, optionally, by ingestion key. See [Telemetry Selectors](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/#telemetry-selectors) in the Transactions Reference.
</Admonition>
...
For the complete list of transactions each managed role includes, see the [Transactions Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/transactions/).
</Admonition>
...
- [Telemetry Access Reference](https://signoz.io/docs/manage/administrator-guide/iam/reference/telemetry/) — How telemetry transactions are evaluated and queries are authorized
...
- [Scope Telemetry Access by Ingestion Key](https://signoz.io/docs/manage/administrator-guide/iam/user-guides/scope-telemetry-access-by-ingestion-key/) — Restrict a role to specific telemetry data
- [Restrict Dashboard Access](https://signoz.io/docs/manage/administrator-guide/iam/user-guides/restrict-dashboard-access/): scope a role to specific dashboards
