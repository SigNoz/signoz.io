...
| `telemetryresource` | `logs` | Log data queried through explorers, dashboards, alerts, and the API |
| `telemetryresource` | `traces` | Trace data queried through explorers, dashboards, alerts, and the API |
| `telemetryresource` | `metrics` | Metric data queried through explorers, dashboards, alerts, and the API |
...
<Admonition type="info">
Telemetry resources (logs, traces, metrics, meter metrics) use a query-scoped selector format instead of instance IDs: grants are scoped by query type and, optionally, by ingestion key. See [Telemetry Selectors](https://signoz.io/docs/manage/administrator-guide/iam/transactions/#telemetry-selectors) in the Transactions Reference.
</Admonition>
