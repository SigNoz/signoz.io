...
description: Blip traces 40M spans/hr with Self-Hosted SigNoz to surface slow database calls, replace Jaeger and Elastic overhead, and cut incident resolution time by 14x.
...
Blip used Self-Hosted SigNoz to improve their issue resolution time by 14x. We sat down with Nate Bohman, Senior DevOps Engineer at Blip, to understand how the team uses its own deployment to improve observability. Some of the key points that the team liked about SigNoz:
...
Read on to find out how Blip runs Self-Hosted SigNoz as its observability backend.
