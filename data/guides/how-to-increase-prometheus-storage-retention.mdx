...
```bash
curl -X GET <http://localhost:9090/api/v1/status/config> | jq '.data.storage.tsdb.retention'
```
...
1. Open the Prometheus configuration file in a text editor.
2. Locate or add the `-storage.tsdb.retention.time` flag.
3. Set the desired retention period (e.g., `30d` for 30 days, `720h` for 30 days, or `1y` for 1 year).
4. Save the changes and restart Prometheus to apply the new settings.
