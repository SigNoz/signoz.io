      ...
      "step": [
        {
          ...
          "text": "Locate the [Prometheus configuration](https://signoz.io/guides/what-is-prometheus-target/) file (usually `prometheus.yml`). Look for the `-storage.tsdb.retention.time` flag in the startup command or configuration. Access the Prometheus UI and navigate to the `/config` endpoint to view current settings. Use the Prometheus API to query retention information."
        },
        {
          "@type": "HowToStep",
