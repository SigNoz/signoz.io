---
title: Prometheus Node Exporter - Install, Run & Visualize [Part 1]
...
In [Part 1 - Prometheus Node Exporter: Install, Run & Visualize](https://signoz.io//guides/prometheus-node-exporter/), you’ll learn:

- How to install and run Prometheus Node Exporter
- How to scrape and visualize host metrics with Prometheus

In [Part 2 - Prometheus Node Exporter: Running in Production](https://signoz.io/guides/prometheus-node-exporter-metrics/), we go beyond the basics and focus on production readiness, including:

- Which Node Exporter metrics actually matter in production
- How to tune collectors and expose custom host metrics
...
<KeyPointCallout title="Exporter for Windows OS." defaultCollapsed="true">
  For Windows-based systems, host metrics can be collected using the <a href="https://github.com/prometheus-community/windows_exporter" rel="noopener noreferrer nofollow" target="_blank">Windows Exporter</a>. It is an officially supported exporter from the Prometheus community and provides functionality
  similar to that of the Node Exporter, specifically designed for Windows environments.
</KeyPointCallout>
...
In [Part 2]((https://signoz.io/guides/prometheus-node-exporter-metrics/)), we go beyond basic setup and focus on running Node Exporter in production, tuning collectors to reduce noise, identifying the most important host metrics, exposing custom signals, and configuring practical alerting rules that map to real-world operational risks.
