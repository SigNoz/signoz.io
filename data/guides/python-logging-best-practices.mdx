...
- **Log Levels:** You can categorize messages by severity (e.g., `DEBUG`, `INFO`, `WARNING`, `ERROR`), allowing you to filter out noise.1
- **Configurability:** You can easily change the logging output: sending it to the console, a file, or a network service, without changing your application code.
...
While the standard `logging` library requires a custom `Filter` object, `structlog` makes this pattern much simpler using its built-in support for Python's `contextvars`. A `ContextVar` holds data that is local to a specific task or thread, making it perfect for storing request-scoped information.
