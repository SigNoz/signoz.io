...
- **[Structured Logging](https://signoz.io/guides/structlog/)**: Using structured formats like JSON for logs makes them easier to parse, query, and analyze. This approach enables more efficient troubleshooting, such as counting specific error occurrences or identifying patterns.
...
Click `New Dashboard` and then import JSON file to create an existing dashboard. For this demo we are using SigNoz [Hostmetrics JSON](https://github.com/SigNoz/dashboards/blob/main/hostmetrics/hostmetrics.json) Dashboard. For more information, refer to [**Hostmetrics** documentation](https://signoz.io/docs/userguide/hostmetrics/#setup).
    ...
    On the `$host_name` select your environment, on which you downloaded the [otel](https://signoz.io/blog/opentelemetry-vs-datadog/)-collector exporter. If exporter is running properly you can see your Remote Server metrics.
