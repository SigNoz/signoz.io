...
Prometheus offers several advantages that make it a compelling choice for modern monitoring needs:

1. Time-Series Database Designed for Monitoring
    - Prometheus stores metrics as time-series data, making it highly optimized for storing and querying monitoring data.
    - It provides efficient storage, high availability, and support for multi-dimensional data through labels.
2. Pull-Based Model
    - Prometheus uses a pull model to scrape metrics from targets, ensuring:
        - Flexibility in adding/removing monitored systems.
        - Avoiding the complexity of setting up agents on all targets.
        - Real-time monitoring with consistent intervals.
3. Robust Querying with PromQL
    - PromQL (Prometheus Query Language) allows users to perform:
        - Aggregations, filtering, and complex computations.
        - Generating alerts directly from the queries.
        - Building dashboards with rich visualizations.
4. Dynamic Service Discovery
    - Prometheus supports dynamic service discovery via integrations with platforms like Kubernetes, Consul, and more. This eliminates manual configurations and enables monitoring in dynamic, containerized environments.
5. Alerting with Alertmanager
    - Built-in support for alerting using rules defined in Prometheus.
    - Alerts can be:
        - Deduplicated, grouped, and sent to various channels (e.g., email, Slack, PagerDuty).
        - Tailored to specific thresholds and conditions, ensuring proactive monitoring.
6. Ecosystem and Integrations
    - Compatible with many exporters to monitor applications, databases, Linux hosts, and more.
    - Easily integrates with visualization tools like Grafana for advanced dashboards and SigNoz for end-to-end observability.
7. Open Source and Active Community
    - Prometheus is open-source, backed by the CNCF (Cloud Native Computing Foundation), ensuring:
        - There are no licensing costs.
        - A vibrant, active community for ongoing support and development.
        - Frequent updates and compatibility with modern environments.
8. Scalability and Flexibility
    - Scales horizontally with features like federation and remote storage integrations.
    - Suitable for both small setups (single server) and large-scale environments with multiple instances.
9. Suitable for Cloud-Native and Microservices
    - Designed for cloud-native architectures, with built-in support for container orchestration platforms like Kubernetes.
    - Provides granular insights into container and microservice performance.
10. High Reliability
    - Prometheus operates as a standalone tool without external dependencies, ensuring:
        - High availability even during failures.
        - Resilience in distributed and dynamic environments.

These features make Prometheus particularly well-suited for containerized environments and microservices architectures.
...
- Data Scraping Process and Intervals:
    ...
    - For Prometheus to pull metrics from a target, the target must expose a specific HTTP endpoint, typically at the `/metrics` path. The HTTP endpoints follow the format; `hostaddress/metrics`. This endpoint provides the current state of the target in a plain text format that Prometheus can understand.
        ...
        <Figure src="/img/guides/2025/02/prometheus-monitoring-101-how-does-prometheus-work-Untitled%201.webp" alt="Metrics displayed from an HTTP endpoint (localhost:8080/metrics)" caption="Metrics displayed from an HTTP endpoint (localhost:8080/metrics)" />
        
- Time Series Database (TSDB) for Efficient Storage:
...
2. Query Language (PromQL)
    ...
    <Figure src="/img/guides/2025/02/prometheus-monitoring-101-how-does-prometheus-work-Untitled%203.webp" alt="PromQL query" caption="PromQL query" />
...
Read more on [How Does Prometheus Work?](https://signoz.io/guides/how-does-prometheus-work/)
...
4. Verify Configuration in SigNoz Cloud
    ...
    <Figure src="/img/guides/2025/02/prometheus-monitoring-101-configure-prometheus-to-use-non-default-port-image%204.webp" alt="Verify Configuration in SigNoz Cloud" caption="Verify Configuration in SigNoz Cloud" />
