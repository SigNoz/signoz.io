---
title: Linux Log Files on Steriods with Logrotate
date: 2026-05-19
tags: [Linux, Logging]
authors: [dhruv_ahuja]
description: Learn how to configure logrotate in Linux for effective log management. Covers rotation policies, copytruncate vs create, cron integration, testing, and monitoring logs with SigNoz.
...
Effective [log management](https://signoz.io/log-management/) is essential on Linux systems. Logs are crucial for debugging, auditing, and troubleshooting production workloads, but they can also grow rapidly and consume valuable disk space if left unmanaged. Application outages, particularly in self-hosted setups, are often caused due to logs exhausting available disk space.

The `logrotate` utility solves this problem by automatically rotating, compressing, and removing old logs based on a defined schedule or when they reach a specific size threshold. 

This guide will walk you through the process of implementing and managing log rotation using `logrotate`, plus some best practices and advanced techniques for managing <Tooltip text="locked files" content="A log file that is being actively held open by a running application, such as a web server or a background worker." />, enabling you to keep your system healthy and your logs organized.<br />
...
<a href="https://github.com/logrotate/logrotate" target="_blank" rel="noopener noreferrer">Logrotate</a> is a system utility designed to simplify the administration of log files on a system that generates a lot of log data. It allows for the automatic rotation, compression, removal, and mailing of log files.

By configuring `logrotate`, you can prevent disk exhaustion by automatically pruning old logs before they fill up your storage. It allows you to rotate logs on a specific time schedule, such as daily or weekly, or strictly by size when they exceed a specified threshold. To further save disk space, the utility can compress archived logs using tools like gzip. It also helps you enforce a strict retention policy by keeping only a fixed number of historical files, and can even run post-rotation scripts to gracefully instruct applications to reopen their log files without any downtime.
...
On many modern Linux distributions, `logrotate` is already installed out of the box. If it is not, you can install it using your system's package manager.
...
```
*(On older releases, `yum install logrotate` may be the available command.)*
...
`logrotate` behavior is governed by configuration files. It usually reads a main configuration file that sets global defaults, and then includes application-specific files.
...
Let's look at a practical example for the system log:
...
The `rotate <count>` directive specifies the exact number of old log files to keep before deleting the oldest one. For instance, setting `rotate 7` alongside a `daily` schedule ensures you keep exactly one week of historical logs.
...
If an application cannot be signaled to reopen its log file, you can use `copytruncate`. It copies the current log file to an archive and then truncates the original file in place to zero bytes. 
...
Here, we keep 14 days of Nginx logs. We use `delaycompress` so the most recently rotated log is not compressed immediately, which is safer if Nginx briefly continues writing to the old file before reopening logs. The `postrotate` script sends a `USR1` signal to Nginx, which tells Nginx to reopen its log files.
...
While `logrotate` keeps your disk usage in check locally, managing logs on individual servers doesn't scale in a distributed microservices environment. You need centralized monitoring and alerting to gain true observability.
...
The <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/receiver/filelogreceiver/README.md#log-rotation" target="_blank" rel="noopener noreferrer">official documentation</a> for the File Log Receiver goes into more detail about its in-built support for log rotation, and configuring the receiver to handle log truncation.

## Conclusion

While `logrotate` is an older utility, it remains one of the most reliable and indispensable tools for Linux system administration. Setting up a small, well-tested configuration serves as an excellent first line of defense against disk space exhaustion. It keeps your log retention predictable, prevents surprise application crashes, and ensures that local troubleshooting remains easy by keeping active log files reasonably sized.

To get the most out of your log management strategy, keep application-specific rules neatly isolated in the `/etc/logrotate.d/` directory rather than cluttering the global configuration. When dealing with live application logs, always prefer the `create` directive paired with a `postrotate` signal over `copytruncate` whenever possible. 
 
This significantly reduces the risk of dropping active log lines during rotation. Furthermore, you should verify any new policies using the `logrotate -d` debug flag before deploying them to production environments.

Finally, while `logrotate` handles the local file lifecycle, pairing it with a centralized OpenTelemetry-native observability platform like SigNoz guarantees that your historical logs remain highly searchable across your entire infrastructure. This unified approach ensures you have complete visibility into your applications and that critical failures never go unnoticed.
