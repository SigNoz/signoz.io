...
Logs are crucial for debugging, auditing, and troubleshooting production workloads, but they can also grow rapidly and consume valuable disk space if left unmanaged. Application outages, particularly in self-hosted setups, are often caused due to logs exhausting available disk space.  
And so, it becomes vital that you maintain your log files frequently, add guardrails around the the log generation and storage behaviour for logs generated from user as well as system services.

This is where The Linux logrotate utility comes in handy. It helps solve such problems by providing capabilities to configure automatic log rotation, compression, and deletion based on configured schedules or file sizes.
...
When managing log rotation in Linux, `logrotate` is the go-to system utility designed to simplify the administration of log files on systems that generate a lot of log data. It allows for the automatic rotation, compression, removal, and mailing of log files.
...
While <a href="https://github.com/logrotate/logrotate" target="_blank" rel="noopener noreferrer">logrotate</a> is an older utility, it remains one of the most reliable and indispensable tools for Linux system administration. Setting up a small, well-tested configuration serves as an excellent first line of defense against disk space exhaustion.  
...
To get the most out of your log management strategy, keep application-specific rules neatly isolated in the `/etc/logrotate.d/` directory rather than cluttering the global configuration. When dealing with application logs, always avoid `copytruncate` where possible.
