---
title: How to Set Up and Manage Log Rotation Using logrotate in Linux
date: 2024-07-11
tags: [Linux, Logging]
authors: [simran_kumari]
description: Learn how to set up and Manage Log Rotation using Logrotate in Linux. Explore configuration, examples, commands and best practices to prevent log file overflow with SigNoz.
keywords: [logs, logrotate, linux logrotate, log rotation, what is log rotation, logrotate configuration, ubuntu log rotate, logrotate examples]
related_articles: ['/guides/how-can-i-limit-the-size-of-apache-access-log-and-limit-the-number-of-archived-logs-it-keeps', '/blog/best-open-source-log-management-tools', '/guides/systemctl-logs', '/guides/default-logging-file-for-spring-boot-application', '/blog/docker-log-rotation', '/guides/error-log']
---

Effective [log management](/log-management/) in Linux is crucial for maintaining system health, performance, and security. Logs are invaluable records of system events, application behaviour, and security incidents. However, these logs can quickly grow out of control without proper management, consuming valuable disk space and potentially causing system issues. This is where log rotation comes into play. Regularly rotating logs ensures they are archived systematically, preventing disk space exhaustion and enhancing overall system performance.

This article walks through Logrotate Linux setup, configuration, and real-world usage, along with how [SigNoz](https://signoz.io/) can help you monitor logrotate logs at scale.

## What is Logrotate?

`Logrotate` is a utility that automates the rotation, compression, and deletion of log files. For anyone managing a Linux server, it is one of the first tools worth learning. It keeps log files at manageable sizes and automatically archives old logs without manual effort.
...
For Ubuntu and Debian-based systems, `logrotate` is usually included in the default repositories, making installation simple with `apt`. If you need to set up Ubuntu log rotation from scratch, the process takes less than a minute.
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 1.webp"
  alt="Installing logrotate on Ubuntu"
  caption="Installing logrotate on Ubuntu"
/>

Once installed, Ubuntu log rotate settings are managed through the same `/etc/logrotate.conf` and `/etc/logrotate.d/` structure used by every other distribution.
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 2.webp"
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 3.webp"
...
Once `logrotate` is installed, you need to configure it to manage your log files. Understanding the locations, syntax, and common directives of the logrotate configuration file is essential for effective log rotation.
...
The global configuration file defines the default settings and behaviours for logrotate. It specifies the rotation frequency, compression options, and other global parameters that apply to all log files unless overridden by more specific configurations. This is the starting point for any Linux log rotation configuration.

<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 4.webp"
...
This directory contains additional configuration files that can override or extend the global settings. Each file in this directory typically corresponds to a specific application or log file, allowing for fine-tuned control over log rotation policies. For instance, you might have separate logrotate configuration files for web server logs, database logs, and system logs, each with its own rotation schedule and parameters.

<Figure
  src="/img/guides/2024/07/logrotate-linux-9a1791a9-da17-4ba9-83aa-1275585e8a4f.webp"
  alt="logrotate additional configuration files directory"
  caption="logrotate additional configuration files directory"
/>
...
The logrotate configuration files include various directives that control how and when log files are rotated, compressed, and managed. These directives allow you to customize the behaviour of logrotate to suit the needs of your system and applications. Below are some of the most commonly used directives:
...
The directives above form the building blocks of every Linux log rotation configuration you will write. By understanding these common directives and their usage, you can customize `logrotate` to manage your log files efficiently and effectively.
...
This example shows how to log rotate in Linux for a custom application. The logs are located in `/var/log/myapp/`, rotated weekly, and the last four logs are kept. The log files are compressed, and after rotation, the application is signalled to reopen them.
...
This example uses wildcards to manage multiple log files generated by the Apache web server. The log files in `/var/log/httpd/` are rotated daily, with the last five logs kept and compressed. The Apache server is gracefully restarted after log rotation. This is one of the more common logrotate examples you will encounter in production.
...
This example demonstrates how to customize the log rotation frequency for the [cron logs](/guides/crontab-logs/). The logs are rotated monthly, and the last twelve logs are kept. The logs are compressed, and empty logs are not rotated.
...
To enable log rotation for your application logs every 10 minutes, follow these steps. This demo shows how to log rotate in Linux on a custom schedule that goes beyond the built-in daily/weekly/monthly options.
...
Add the following logrotate configuration to rotate logs every 10 minutes based on their size:
...
- size 1M: This directive tells `logrotate` to rotate the log file when it reaches 1 megabytes in size.
...
The script above automates log rotation for the `myapp` logs, ensuring they are rotated according to the specified logrotate configuration. You can include this script in a cron job for regular execution.
...
The above cron job configuration instructs the system to run the `rotate-logs.sh` script as the root user every 10 minutes.
...
The above script continuously writes a sample log entry, including the date, to the application log file every second.
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 5.webp"
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 6.webp"
  alt="Watch the log directory to get the list of files generated by logrotate"
  caption="Watch the log directory to get the list of files generated by logrotate"
/>

This command updates the file listing every second, allowing you to see new log files being
created and old ones being compressed and archived.

## Monitoring Logrotate Linux Logs with SigNoz

Effective log management is crucial for maintaining system performance and preventing issues like disk space exhaustion. Manual monitoring via terminal commands can be challenging due to the lack of real-time alerts, scalability across multiple systems, and difficulty in promptly identifying patterns or anomalies.

SigNoz offers a robust solution by providing real-time monitoring capabilities. It allows you to track log rotation activities in real time and set up [alerts for specific log patterns](/docs/alerts-management/log-based-alerts/) or anomalies. With SigNoz, logs from various sources are centralized, simplifying management and ensuring proactive detection of issues like failed rotations or disk space concerns.

Compared to other tools, SigNoz stands out as an open-source platform, offering flexibility, comprehensive observability with metrics and tracing, and an intuitive interface that makes it easy to use. It is cost-effective and supported by an active community, making it a reliable choice for effective log management and system monitoring.

### Setting Up Log Monitoring in SigNoz

To monitor logrotate logs using SigNoz and ensure the logs are correctly forwarded, you can configure the [OpenTelemetry Collector](/blog/opentelemetry-collector-complete-guide/) to use the [syslog receiver](/docs/userguide/collecting_syslogs/). Here's how you can achieve this, along with setting up monitoring and alerts in SigNoz.

**Step 1: Configure logrotate to Send Rotated Logs to SigNoz**

Ensure the OpenTelemetry Collector is installed on your system. If not, install it using the following commands:
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 7.webp"
...
Ensure you have the latest version of the OpenTelemetry Collector. The commands above use version 0.116.0, which is the latest at the time of writing. To verify and download the most current version, visit the <a href="https://github.com/open-telemetry/opentelemetry-collector-releases" rel="noopener noreferrer nofollow" target="_blank">OpenTelemetry Collector Releases</a> page.
</KeyPointCallout>

Create the OpenTelemetry configuration file, `config.yaml`,~ in the folder otelcol-contrib, with the content below.
...
You can update the [ingestion key](/docs/ingestion/signoz-cloud/keys/) and [ingestion endpoint](/docs/ingestion/signoz-cloud/overview/#how-do-i-find-my-ingestion-url) with the values specified in the Otel-collector Setup step.
...
This logrotate configuration rotates logs in the `/var/log/myapp` directory when they reach 1MB in size, keeps 7 rotated logs, compresses them, and ensures the log rotation process doesn't break if a log file is missing or empty. After rotation, it signals the `myapp` process to reopen the log files.

**Step 3: Setting Up Log Monitoring in SigNoz**

To connect your application logs to SigNoz, you will need a [SigNoz Cloud](/teams/) account. This account provides access to the SigNoz platform, where you can configure and view your logs.

- Access your SigNoz cloud account by navigating to your SigNoz URL and logging in.
- Go to the "Logs" section. You will start receiving logs in the log explorer section from the [OpenTelemetry](/opentelemetry/) Collector.

<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 8.webp"
  alt="Exploring logs in the SigNoz Dashboard"
  caption="Exploring logs in the SigNoz Dashboard"
/>

- You can filter logs on various parameters, for example, if the body contains "Error"

<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 9.webp"
  alt="Filtering logs in SigNoz Logs Explorer"
  caption="Filtering logs in SigNoz Logs Explorer"
/>

What if you want to set up alerts if some error logs are received? You can make use of the [alert management](/alerts-management/) solution provided by SigNoz in that case.
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 10.webp"
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 11.webp"
...
<Figure
  src="/img/guides/2024/07/logrotate-linux-Untitled 12.webp"
...
1. Generate Logs
  ...
  Generate sample logs by running `logrotate` or manually creating log entries to simulate rotation and alert triggering.
...
- Integrating logrotate logs with monitoring tools like SigNoz helps you track log rotation activities and set up alerts before small issues become outages.

## FAQs

### What is logrotate in Linux?

`logrotate` in Linux is a utility that automates the rotation, compression, and deletion of log files. It helps prevent log files from growing too large, thereby optimizing disk space and system performance. If you are new to Linux logrotate, the default configuration at `/etc/logrotate.conf` is a good starting point.

### How do I run the Logrotate command?
...
Yes, logrotate is typically configured to run automatically through cron jobs. These jobs trigger log rotation according to predefined schedules defined in the logrotate configuration files. On most distributions, a daily cron entry in `/etc/cron.daily/logrotate` handles this.
...
`logrotate` is essential in Linux to manage log files efficiently. It prevents log files from filling up disk space, ensures logs are organized and archived, and helps maintain system performance by managing log size. Any Linux logrotate setup — even a basic one — is better than letting logs grow unchecked.

### What is the Purpose of Log Rotation?

The primary purpose of log rotation is to manage log files systematically. It involves rotating logs periodically, compressing older logs to save disk space, and deleting outdated logs. Understanding what log rotation is helps you appreciate why every production Linux server should have a logrotate configuration in place.
...
In the logrotate configuration, `rotate 7` means that logrotate keeps up to 7 rotated log files. This directive specifies how many historical log files to retain before deleting the oldest log file during rotation.
