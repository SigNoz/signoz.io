...
The first step is auto-instrumenting your application services so they send traces and metrics to a centralized backend. For host-level metrics (CPU, memory, disk), you will also need the OpenTelemetry Collector running on your hosts. Follow the [SigNoz VM installation guide](/docs/introduction/opentelemetry-collection-agents/vm/install/) or the [Docker installation guide](/docs/introduction/opentelemetry-collection-agents/docker/install/) to set that up.

For application instrumentation, auto-instrumentation requires no code changes. The following steps are from the [SigNoz Python instrumentation docs](/docs/introduction/instrumentation/opentelemetry-python/).
...
For Java applications, attach the OTel Java agent. See the [SigNoz Java instrumentation docs](/docs/introduction/instrumentation/java/opentelemetry-java/) for full details.
...
For detailed setup instructions specific to your language and framework, see the [SigNoz documentation](/docs/introduction/).
