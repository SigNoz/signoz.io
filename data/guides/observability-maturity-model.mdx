...
The first step is auto-instrumenting your application services so they send traces and metrics to a centralized backend. For host-level metrics (CPU, memory, disk), you will also need the OpenTelemetry Collector running on your hosts. Follow the [SigNoz VM installation guide](/docs/opentelemetry-collection-agents/vm/install/) or the [Docker installation guide](/docs/opentelemetry-collection-agents/docker/install/) to set that up.
...
For Java applications, attach the OTel Java agent. See the [SigNoz Java instrumentation docs](/docs/instrumentation/java/opentelemetry-java/) for full details.
