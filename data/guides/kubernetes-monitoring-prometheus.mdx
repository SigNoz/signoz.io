...
- <a
  ...
  .
- <a href="https://helm.sh/" rel="noopener noreferrer nofollow" target="_blank">
    Helm
  </a> installed.
...
<Figure src="/img/guides/2024/06/prometheus_UI.webp" alt="Prometheus UI" caption="Prometheus UI" />
<br></br>Prometheus has been configured to automatically scrape cluster metrics. To verify this,
click on Status > Targets to see the Targets to see the Targets that are being monitored.{' '}

You should see a list of endpoints that Prometheus is currently scraping for metrics.

<Figure
  src="/img/guides/2024/06/prometheus-targets.webp"
  alt="Prometheus Targets"
  caption="Prometheus Targets"
/>
<br></br>You can see all the target endpoints being monitored by Prometheus:

<Figure
  src="/img/guides/2024/06/target-endpoints.webp"
  alt="Target endpoints Prometheus scrapes metrics from"
  caption="Target endpoints Prometheus scrapes metrics from"
/>
<br></br>
...
<Figure src="/img/guides/2024/06/Ingestion-Key.webp" alt="Ingestion key" caption="Ingestion key" />
<br></br>Apply the config map:
...
<a href="https://opentelemetry.io/" rel="noopener noreferrer nofollow" target="_blank">
  OpenTelemetry
</a> is an open-source framework that provides APIs and SDKs for instrumenting applications to
generate telemetry data. The <a
  href="https://opentelemetry.io/docs/collector/"
  rel="noopener noreferrer nofollow"
  target="_blank"
>
  OpenTelemetry Collector
</a> is a specific component within the OpenTelemetry ecosystem that facilitates the collection,
processing, and exporting of telemetry data. It is designed to sit between the instrumentation
points in your application and your observability backend (like SigNoz). The Collector receives data
from various sources, process it (for example, adding tags or annotations), and then export it to
one or more destinations in a standardized format.
...
<Figure
  src="/img/guides/2024/06/Create_Dashboard.webp"
  alt="Creating a dashboard in SigNoz"
  caption="Creating a dashboard in SigNoz"
/>
<br></br>Click on Configure your new dashboard:

<Figure
  src="/img/guides/2024/06/configure_dashboard.webp"
  alt="Dashboard configuration"
  caption="Dashboard configuration"
/>
<br></br>Give the dashboard a name, description, and tag (optional), then save it.

<Figure
  src="/img/guides/2024/06/dashboard-naming.webp"
  alt="Dashboard naming"
  caption="Dashboard naming"
/>
<br></br>After the dashboard has been created, panels can be set up. Click on Add Panels > New
Panel, to start creating visualizations in the dashboard:

<Figure
  src="/img/guides/2024/06/dashboard-panel.webp"
  alt="Adding panels in the SigNoz dashboard"
  caption="Adding panels in the SigNoz dashboard"
/>
<br></br>For the visualization option, select “Time Series”:

<Figure
  src="/img/guides/2024/06/Time_Series.webp"
  alt="Visualization options"
  caption="Visualization options"
/>
<br></br>You will be redirected to the query builder where you can run different queries against the
metrics coming in using the time series visualization.

In the query builder, type in “kube” to see Kubernetes metrics being exported to SigNoz:

<Figure
  src="/img/guides/2024/06/query-builder.webp"
  alt="Kubernetes metrics exported to SigNoz"
  caption="Kubernetes metrics exported to SigNoz"
/>
<br></br>For your first metrics query, type `kube_pod_container_status_running` then click on the
stage and run query button:

<Figure
  src="/img/guides/2024/06/run-queries.webp"
  alt="Kubernetes metrics being queried in SigNoz dashboard"
  caption="Kubernetes metrics being queried in SigNoz dashboard"
/>
<br></br>The `kube_pod_container_status_running` metric is a Prometheus metric exposed by the
kube-state-metrics service. This metric indicates whether containers within pods in your Kubernetes
cluster is currently in a running state. It's a gauge metric, meaning it represents a value at a
...
<Figure
  src="/img/guides/2024/06/signoz-dashboard.webp"
  alt="Saved SigNoz dashboard"
  caption="Saved SigNoz dashboard"
/>
<br></br>You can proceed to run more queries against metrics of your choice.
