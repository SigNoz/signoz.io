...
<Figure
  src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image.webp"
  alt="Prometheus: Context Deadline Exceeded Error"
  caption="Prometheus: Context Deadline Exceeded Error"
/>
...
1. Check the Prometheus UI:
   - Navigate to the "Targets" page in the Prometheus web interface
     {' '}
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image%201.webp"
       alt="Navigate to the Targets section in the Prometheus Web Interface"
       caption="Navigate to the Targets section in the Prometheus Web Interface"
     />
   - Look for targets marked as "down" with error messages containing "context deadline exceeded"
     {' '}
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image%202.webp"
       alt="Look for ContextDeadline Error in targets that are down"
       caption="Look for ContextDeadline Error in targets that are down"
     />
...
3. Use Prometheus debug endpoints:
   ...
   - Look for goroutines stuck in scrape operations
     {' '}
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image%203.webp"
       alt="Accessing the /debug/pprof endpoint on Prometheus server"
       caption="Accessing the /debug/pprof endpoint on Prometheus server"
     />
     You will notice a lot of data, which may appear difficult to read.
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image%204.webp"
       alt=""
       caption=""
     />
     Let’s decode it one by one : - There are 31 goroutines active at the time of the profile
     capture.
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-276bcb1e-d88f-479d-83eb-028068902545.webp"
       alt=""
       caption=""
     />
     - Blocked Goroutines (3 instances)
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-546fc198-8399-4937-96bc-c106bf46cef6.webp"
       alt=""
       caption=""
     />
     - "3 @ ..." indicates that 3 different goroutines are executing the same code path at this
     specific point in time. - The goroutines are stuck waiting for network-related operations. -
     ...
     `*net/http.(connReader).Read*`: Indicates that this goroutine is part of an HTTP request
     handler and is reading data. - `*net/http.(conn).serve*`: This suggests that the goroutine is
     handling an HTTP request. - Discovery Manager Goroutines (2 instances)
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-20ac5d3d-d07c-4efc-aab3-ad29d37ac46b.webp"
       alt=""
       caption=""
     />
     - These goroutines are involved in Prometheus's discovery manager, which handles service
     discovery for targets. - The `discovery/legacymanager.Manager.sender` function is related to
     sending target discovery updates. From above, we can figure out that the goroutine is stuck
     waiting for network-related operations. Hinting at network-related issues while trying to read
     data.
     <aside>
       💡 To observe clear changes during a context deadline exceeded scenario, you'd typically see
       more goroutines stuck in network I/O or waiting for timeout conditions, especially around the
       HTTP client or scraping logic. The `goroutine profile` would show more instances of these
       goroutines waiting on `net/http` or `context` functions.
     </aside>
4. Leverage external tools:
   - Use network diagnostic tools like `ping` or `traceroute` to check connectivity. For example, sending GET requests to the server throws an error, hinting at something wrong on the target app’s end.
     {' '}
     <Figure
       src="/img/guides/2024/09/context-deadline-exceeded-prometheus-image%205.webp"
       alt="Checking target sever for Context Deadline Exceeded Errors"
       caption="Checking target sever for Context Deadline Exceeded Errors"
     />
