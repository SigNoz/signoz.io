...
title: Prometheus Node Exporter - Running in Production [Part 2] 
...
In [Part 1 - Prometheus Node Exporter: Install, Run & Visualize](https://signoz.io//guides/prometheus-node-exporter/), you set up Prometheus Node Exporter, configured Prometheus to scrape host-level metrics, and verified that your system was emitting and collecting data correctly. While this setup works well for learning and small environments, production systems demand a more deliberate approach.
...
Node Exporter ships with 70+ collectors, most of which are enabled by default. You can explore the complete list of collectors and the metrics they expose in the <a href="https://github.com/prometheus/node_exporter" rel="noopener noreferrer nofollow" target="_blank">official Node Exporter GitHub repository</a>. 
...
groups:
  - name: node_exporter_alerts
    rules:
      # CPU
      - alert: HighCPUUsage
        # Triggers when average non-idle CPU usage exceeds 80% across all cores.
      ...
      - alert: HighMemoryUsage
        # Triggers when available memory drops below 10% of total memory.
...
By being intentional about what you collect and alert on, Node Exporter becomes a reliable signal for host health rather than a source of metric sprawl and alert fatigue. With these foundations in place, you’re now equipped to scale host monitoring confidently and integrate it into a broader observability strategy using OpenTelemetry and SigNoz.
