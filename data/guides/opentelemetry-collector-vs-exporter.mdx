...
3. **Standard Protocol Exporters**
    ...
    For legacy or established systems, the Zipkin exporter is still widely used to stay compatible with existing setups. Note that Jaeger now supports OTLP as of <a href="https://opentelemetry.io/blog/2023/jaeger-exporter-collector-migration/" target="_blank" rel="noopener noreferrer nofollow">Collector v0.85.0</a>, so a dedicated Jaeger exporter is no longer required - send [OTLP data](https://signoz.io/blog/what-is-otlp/) to Jaeger directly instead. These exporters:
...
otlp_exporter = OTLPSpanExporter(
    endpoint="collector:4317",  # host:port — the gRPC exporter takes no URL scheme
...
Built on OpenTelemetry, SigNoz natively ingests OpenTelemetry data—[along with Prometheus metrics](https://signoz.io/blog/opentelemetry-vs-prometheus/)—and unifies metrics, traces, and logs in a single application. This gives you a complete, end-to-end view of system performance without stitching together separate tools.
