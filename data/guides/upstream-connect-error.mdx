...
keywords:
  [
    ...
    502 bad gateway nginx upstream,
    503 service unavailable kubernetes,
    connection refused while connecting to upstream,
    upstream connect error spring boot,
    java 11 networking,
    docker nginx upstream connection failed,
  ]
---
...
When health checks fail, the load balancer removes instances from rotation. If ALL instances fail
health checks (even if they're actually running), the load balancer has no upstream targets to
connect to, causing upstream connect errors
