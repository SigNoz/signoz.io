...
To extract label values, use the `label()` function:

```
label(metric_name, "label_name")

```

For instance, to get all unique values of the `method` label for HTTP requests:

```
label(http_requests_total, "method")
...
For more complex queries, use the `group()` function to group by label values:
