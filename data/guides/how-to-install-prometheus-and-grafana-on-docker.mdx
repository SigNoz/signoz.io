...
keywords:
  [Prometheus, Grafana, Docker, monitoring, installation, containerization, metrics, visualization]
---
...
1. **Create a Dedicated Docker Network**
   ...
   <Figure
     src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image.webp"
     alt="command: docker network ls"
     caption="command: docker network ls"
   />
...
2. **Create a [Prometheus Configuration](https://signoz.io/guides/what-is-prometheus-target/) File**
   ...
   📌 _This configuration tells Prometheus to scrape metrics from itself every 15 seconds._
...
3. **Create a Docker Compose File**
   ...
   📝 _Using `restart: unless-stopped` ensures Prometheus restarts automatically if it crashes or the system reboots._
...
5. **Verify Prometheus is Running**
   ...
   <Figure
     src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%201.webp"
     alt="command: docker ps"
     caption="command: docker ps"
   />
...
1. **Add a New Scrape Job to `prometheus.yml`**
   ...
   📌 _Ensure `app-host` is resolvable from within the Prometheus container—use a Docker service name or internal container name if they're on the same network._

   💡 _You can also customize the scrape interval per job:_
...
2. **Restart Prometheus to Apply the New Configuration**
   ...
   🔍 _Alternatively, use `docker exec` to test the config before restarting:_
...
3. **Verify Metric Collection in the Prometheus UI**
   ...
   - Use the **Graph** tab to query example metrics, such as:
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%202.webp"
       alt="Prometheus graph using PromQL Query"
       caption="Prometheus graph using PromQL Query"
     />
...
1. **Update Your `docker-compose.yml` to Include Grafana**
   ...
   📌 _Using a named volume ensures Grafana persists dashboards and data sources across container restarts._
...
2. **Launch Grafana**
   ...
   <Figure
     src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%203.webp"
     alt="Command: docker-compose up -d"
     caption="Command: docker-compose up -d"
   />
...
1. **Set up Prometheus as a Data Source in Grafana**
   - Navigate to Connections → Data Sources → Add data source in the left side panel.
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%204.webp"
   ...
   - Enter the **Prometheus server URL** in the configuration field.
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%205.webp"
...
2. **Create a New Dashboard and Panel**
   - Navigate to **Dashboards** → **New Dashboard** → Click on the **Add Visualization** button
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%206.webp"
   ...
   - Select **Prometheus** from the data source dropdown
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image%207.webp"
...
3. **Input Your PromQL Expression**
   - In the query editor, enter your metric division expression. For example, to calculate CPU usage for each core:
     ...
     {' '}
     <Figure
       src="/img/guides/2025/06/how-to-install-prometheus-and-grafana-on-docker-image_(6).webp"
       alt="PromQL expression for CPU usage"
       caption="PromQL expression for CPU usage"
     />
...
1. **Add cAdvisor to Your `docker-compose.yml`:**
   ...
   📝 _cAdvisor provides insights like memory, CPU, and network usage for each container._
