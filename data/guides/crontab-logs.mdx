...
<script type="application/ld+json">
  ...
  "image": "https://signoz.io/img/blog/2023/12/crontab-logs-cover.webp",
  "author": {
    "@type": "Person",
    "name": "Simran Kumari"
  },
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://signoz.io/blog/crontab-logs/"
  },
  "description": "To read and find crontab logs, check the default system log files like /var/log/syslog or /var/log/cron. Learn how to locate, read, and analyze these logs using command-line tools, with insights on integrating with SigNoz for comprehensive monitoring.",
  "keywords": "crontab logs, logs, cronjob",
...
1. View all cron job entries in syslog:
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-9c1cc39e-ffc7-4d9b-bcad-b03d71bdaa34.webp" alt="Syslog cronjob entries - Ubuntu" caption="Syslog cronjob entries - Ubuntu" /><br></br>2. View the latest cron log entries using `tail`:
    
    ```bash
    tail -f /var/log/syslog | grep CRON
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled.webp" alt="Latest cronjob log entries - Ubuntu" caption="Latest cronjob log entries - Ubuntu" /><br></br>3. Use `less` for detailed log examination:
    
    ```bash
    less /var/log/syslog | grep CRON
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%201.webp" alt="Detailed cronjob logs - Ubuntu" caption="Detailed cronjob logs - Ubuntu" /><br></br>On Red Hat/CentOS
...
1. View all cron job entries in the cron log:
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%202.webp" alt="Syslog cronjob entries - RedHat" caption="Syslog cronjob entries - RedHat" /><br></br>2. View the latest cron log entries using `tail`:
    
    ```bash
    tail -f /var/log/cron
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-image.webp" alt="Latest cronjob log entries - RedHat" caption="Latest cronjob log entries - RedHat" /><br></br>3. Use `less` for detailed log examination:
    
    ```bash
    less /var/log/cron
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-image2.webp" alt="Detailed cronjob log entries - RedHat" caption="Detailed cronjob log entries - RedHat" /><br></br>
...
1. Create a `script.sh` for the cron job using any editor.
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%203.webp" alt="Using `vi` editor for custom crontab log script" caption="Using `vi` editor for custom crontab log script" /><br></br>
      ```bash
      #!/bin/bash
...
1. Make the `script.sh` file executable.
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%204.webp" alt="Making custom crontab logs creation script executable" caption="Making custom crontab logs creation script executable" /><br></br>2. Open your Crontab file for editing:
    
    ```bash
    crontab -e
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%205.webp" alt="Opening Crontab file" caption="Opening Crontab file" /><br></br>3. Add a cron job with custom logging:
...
1. Check the last 10 logs of the log file:
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%206.webp" alt="Latest cronjob log entries" caption="Latest cronjob log entries" /><br></br>2. Add an exit status check in your script:
...
1. View the latest entries in the log file:
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%207.webp" alt="Latest cronjob log entries with timestamp" caption="Latest cronjob log entries with timestamp" /><br></br>Each log entry should include a timestamp indicating when the job was executed.
...
3. Check the output and error logs.
    ...
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%208.webp" alt="Latest entries of cronjob output logs" caption="Latest entries of cronjob output logs" /><br></br>
    
    ```bash
    tail -n 10 /var/log/custom_error.log
    ```
    
    <Figure src="/img/guides/2024/07/crontab-logs-Untitled%209.webp" alt="Latest entries of cronjob error logs" caption="Latest entries of cronjob error logs" /><br></br>By following these practices, you can create custom Crontab logs, check the success of your cron jobs, verify timestamps, and effectively manage output and error logs. This helps you maintain and troubleshoot your automated tasks efficiently.
...
<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2010.webp" alt="Get Started with SigNoz cloud" caption="Get Started with SigNoz cloud" /><br></br>1. Next, you need to select your use case, here its `Logs Management`. 

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2011.webp" alt="Selecting a use-case to work with using SigNoz" caption="Selecting a use-case to work with using SigNoz" /><br></br>1. Under Logs Management, you need to select the source of our data, for crontab logs choose `Syslogs`. 

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2012.webp" alt="Selecting Data source for monitoring using SigNoz" caption="Selecting Data source for monitoring using SigNoz" /><br></br>1. You also need to select the environment based on your system specifications and move to the next steps.

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2013.webp" alt="Selecting Environment from which logs will be sent to SigNoz" caption="Selecting Environment from which logs will be sent to SigNoz" /><br></br>Setup OpenTelemetry Binary as an agent

On clicking the `Continue to Next Step` button you will see the next set of steps to follow. 

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2014.webp" alt="Steps to set up Otel-Collector" caption="Steps to set up Otel-Collector" /><br></br>The same steps will be followed with some tweaks. 
...
<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2015.webp" alt="Downloading Otel-Collector" caption="Downloading Otel-Collector" /><br></br>1. Extract otel-collector tar.gz to the `otelcol-contrib` folder using the command specified below:

```bash
mkdir otelcol-contrib && tar xvzf otelcol-contrib_0.88.0_linux_amd64.tar.gz -C otelcol-contrib
```

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2016.webp" alt="Extracting Otel-collector tar file" caption="Extracting Otel-collector tar file" /><br></br>1. Create config.yaml in folder otelcol-contrib with the below content in it.

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2017.webp" alt="Creating config file for Otel-collector" caption="Creating config file for Otel-collector" /><br></br>
```yaml
receivers:
  otlp:
    protocols:
  ...
  syslog:
    ...
    operators:
      - type: move
        ...
        to: body
  filelog:
    include: [ /var/log/custom_log.log ] # Update with your actual log file path
    start_at: end
...
<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2018.webp" alt="Untitled" caption="Untitled" />
...
<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2019.webp" alt="Restarting rsyslog service" caption="Restarting rsyslog service" /><br></br>1. Check the status of your service. Make sure your service is in an active state. 

```bash
sudo systemctl status rsyslog.service
```

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2020.webp" alt="Status of rsyslog service" caption="Status of rsyslog service" /><br></br>Once things are up and running, you can click on `Done` button of the SigNoz steps and you will be redirected to `Logs Explorer` where you will see your logs getting updated.

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2021.webp" alt="SigNoz Logs Explorer" caption="SigNoz Logs Explorer" /><br></br>You can expand the logs and see the various attributes associated with them. You can also filter logs based on the attributes and their values.

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2022.webp" alt="Log Attributes Section in SigNoz Dashboard" caption="Log Attributes Section in SigNoz Dashboard" /><br></br>For example, say you redirect output from various files to SigNoz , and now you want to filter logs associated with a particular logfile, you can filter based on query `log.file.name=custom_log.log` or any other file name.

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2023.webp" alt="Filtering logs in SigNoz Log Explorer" caption="Filtering logs in SigNoz Log Explorer" /><br></br>From the below image, you can see that no data is returned as there are no logs associated with the file name `custom.log` .

<Figure src="/img/guides/2024/07/crontab-logs-Untitled%2024.webp" alt="No data was found based on the search query passed in the SigNoz Dashboard" caption="No data was found based on the search query passed in the SigNoz Dashboard" /><br></br>
