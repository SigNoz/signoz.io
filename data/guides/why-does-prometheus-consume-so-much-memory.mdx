...
authors:
  - simran_kumari
description: >-
  Discover why Prometheus consumes high memory and learn effective strategies to
  optimize its performance without compromising monitoring capabilities.
keywords:
...
related_articles:
  ...
  - /guides/how-to-increase-prometheus-storage-retention/
...
1. Key metrics to watch:
    ...
    - `prometheus_tsdb_head_samples_appended_total`: Total number of appended samples.
        ...
        <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 1.webp" alt="Metric prometheus_tsdb_head_samples_appended_total" caption="Metric prometheus_tsdb_head_samples_appended_total" />
        
    - `process_resident_memory_bytes`: Prometheus process memory usage.
        
        <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 2.webp" alt="Metric process_resident_memory_bytes" caption="Metric process_resident_memory_bytes" />
...
3. Query analysis:
    ...
    - Identify high-cardinality metrics: Run `topk(10, count by (__name__)({__name__=~".+"}))` to find the top 10 metrics with the most series and detect high-cardinality metrics.
        ...
        <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 3.webp" alt="Identify High-Cardinality Metrics" caption="Identify High-Cardinality Metrics" />
    ...
    - Monitor query load: Check `prometheus_engine_query_duration_seconds` to spot long-running or resource-intensive queries. Use recording rules for complex queries.
        ...
        <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 4.webp" alt="Monitor Query Load" caption="Monitor Query Load" />
    ...
    - Analyze [target metrics](https://signoz.io/guides/what-is-prometheus-target/): Use `prometheus_target_interval_length_seconds` to review scrape intervals and identify any targets with high scrape frequencies that may be overloading memory.
        ...
        <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 5.webp" alt="Identify targets with high scrape frequencies" caption="Identify targets with high scrape frequencies" />
...
- Integrated tracing: Combines metrics and distributed tracing, providing deeper insights into application performance.
    ...
    <Figure src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image 6.webp" alt="Integrating traces and logs with metrics for comprehensive observability" caption="Integrating traces and logs with metrics for comprehensive observability" />
