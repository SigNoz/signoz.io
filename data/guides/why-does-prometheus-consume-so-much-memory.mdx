...
3. External storage:
   - Offloading older or less frequently accessed data to an external storage system can free up memory and disk space. Read more about <a href="https://prometheus.io/docs/prometheus/latest/storage/#remote-storage-integrations" rel="noopener noreferrer nofollow" target="_blank">external storage integrations in Prometheus</a>.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-7db4965a-eeb3-4493-81c1-5b40bd21f2e8.webp"
       alt="Using external storage for storing grafana metrics"
       caption="Using external storage for storing grafana metrics"
     />
...
1. Key metrics to watch:
   ...
   - `prometheus_tsdb_head_series`: Number of time series in the head block.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image.webp"
       alt="Metric prometheus_tsdb_head_series"
       caption="Metric prometheus_tsdb_head_series"
     />
   - `prometheus_tsdb_head_samples_appended_total`: Total number of appended samples.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%201.webp"
       alt="Metric prometheus_tsdb_head_samples_appended_total"
       caption="Metric prometheus_tsdb_head_samples_appended_total"
     />
   - `process_resident_memory_bytes`: Prometheus process memory usage.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%202.webp"
       alt="Metric process_resident_memory_bytes"
       caption="Metric process_resident_memory_bytes"
     />
...
3. Query analysis:
   ...
   - Identify high-cardinality metrics: Run `topk(10, count by (__name__)({__name__=~".+"}))` to find the top 10 metrics with the most series and detect high-cardinality metrics.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%203.webp"
       alt="Identify High-Cardinality Metrics"
       caption="Identify High-Cardinality Metrics"
     />
   - Monitor query load: Check `prometheus_engine_query_duration_seconds` to spot long-running or resource-intensive queries. Use recording rules for complex queries.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%204.webp"
       alt="Monitor Query Load"
       caption="Monitor Query Load"
     />
   - Analyze [target metrics](https://signoz.io/guides/what-is-prometheus-target/): Use `prometheus_target_interval_length_seconds` to review scrape intervals and identify any targets with high scrape frequencies that may be overloading memory.
     {' '}
     <Figure
       src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%205.webp"
       alt="Identify targets with high scrape frequencies"
       caption="Identify targets with high scrape frequencies"
     />
...
1. <a href="https://thanos.io/" rel="noopener noreferrer nofollow" target="_blank">
     Thanos
   </a>
   : Thanos adds horizontal scaling, long-term storage, and downsampling capabilities to Prometheus,
   making it a good option for large-scale deployments.
2. <a href="https://victoriametrics.com/" rel="noopener noreferrer nofollow" target="_blank">
     VictoriaMetrics:
   </a> VictoriaMetrics is a high-performance time-series database that is compatible with
   Prometheus but optimized for large-scale environments and efficient resource usage.
3. <a href="https://grafana.com/oss/mimir/" rel="noopener noreferrer nofollow" target="_blank">
     Grafana Mimir
   </a>
   : Mimir is another scalable solution that works well for multi-tenant environments and offers
   horizontal scaling similar to Thanos.
...
- Integrated tracing: Combines metrics and distributed tracing, providing deeper insights into application performance.
  {' '}
  <Figure
    src="/img/guides/2024/11/why-does-prometheus-consume-so-much-memory-image%206.webp"
    alt="Integrating traces and logs with metrics for comprehensive observability"
    caption="Integrating traces and logs with metrics for comprehensive observability"
  />
