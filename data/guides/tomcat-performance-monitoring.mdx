...
- **Application Metrics**: Request rate, average latency, error percentage, and p99 latency
  ...
  <NextFigure src="/img/guides/2026/03/tomcat-performance-monitoring-apm-metrics.webp" alt="Tomcat application metrics in SigNoz, latency percentiles, request rate, Apdex score, and per-endpoint breakdown showing the three demo endpoints with their latency and error rate distributions." caption="Tomcat demo service RED metrics and per-endpoint breakdown." width={1200} height={675} priority />
...
- **Traces**: Click any trace to see the span waterfall. You should see spans for `GET /`, `GET /slow.jsp`, and `GET /error.jsp`.
  ...
  <NextFigure src="/img/guides/2026/03/tomcat-performance-monitoring-traces.webp" alt="SigNoz trace detail view showing GET /index.jsp span for tomcat-monitoring-demo with duration, service metadata, and span attributes." caption="Detailed span view for a Tomcat request with execution time and attributes." width={1200} height={675} />
...
<NextFigure src="/img/guides/2026/03/tomcat-performance-monitoring-traces-list.webp" alt="SigNoz trace explorer list view filtered for tomcat-monitoring-demo service showing multiple GET /slow.jsp spans with response times and HTTP status 200." caption="Filtered trace list showing GET /slow.jsp requests with duration and status." width={1200} height={675} />

Finally, move to the `Logs` tab to inspect correlated application logs.

<NextFigure src="/img/guides/2026/03/tomcat-performance-monitoring-logs.webp" alt="SigNoz logs explorer displaying repeated Servlet.service exceptions for error.jsp from tomcat-monitoring-demo service." caption="Tomcat servlet exception logs correlated in SigNoz." width={1200} height={675} />
...
<NextFigure src="/img/guides/2026/03/tomcat-performance-monitoring-dashboard.webp" alt="Screenshot of an Apache Tomcat dashboard displaying JVM metrics including heap limit (2.06 GB), heap usage percentage (2.87%), heap used trend, old generation memory trend, no-heap memory usage, GC rate, and GC pause P95 over the last hour." caption="Apache Tomcat JVM metrics dashboard showing heap usage, GC activity, and memory trends in real time." width={1200} height={675} />
