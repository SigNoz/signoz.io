...
keywords:
  [Prometheus, PromQL, unique label values, count function, monitoring, metrics, Grafana, SigNoz]
---
...
<Figure
  src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image.webp"
  alt="Prometheus Labels"
  caption="Prometheus Labels"
/>
...
1. Add the Prometheus server as the data source to your Grafana server.
   ...
   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%201.webp"
     alt="Adding Prometheus server as a data source in Grafana"
     caption="Adding Prometheus server as a data source in Grafana"
   />

2. Create a new dashboard by adding a visualization.

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%202.webp"
     alt="Creating Grafana Dashboard"
     caption="Creating Grafana Dashboard"
   />

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%203.webp"
     alt="Add visualization to Grafana Dashboard"
     caption="Add visualization to Grafana Dashboard"
   />

3. Select the data source i.e. the Prometheus server.

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%204.webp"
     alt="Selecting Prometheus server as data source for Grafana Dashboard"
     caption="Selecting Prometheus server as data source for Grafana Dashboard"
   />

4. Use the Prometheus query editor to enter your unique label count query (e.g., `count(up) without(instance)`)

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%205.webp"
     alt="Running PromQL queries in Grafana to see unique label counts."
     caption="Running PromQL queries in Grafana to see unique label counts."
   />

5. Choose an appropriate visualization type (e.g., gauge, bar chart, or table)

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%206.webp"
     alt="Choosing visualization type in the Grafana dashboard"
     caption="Choosing visualization type in the Grafana dashboard"
   />

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%207.webp"
     alt="Switching to pie charts and running PromQL queries"
     caption="Switching to pie charts and running PromQL queries"
   />

6. Set the panel title to "Active Instances"

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%208.webp"
     alt="Giving a title to Dashboard Panel in Grafana"
     caption="Giving a title to Dashboard Panel in Grafana"
   />

7. Adjust thresholds and other display options as needed and apply the changes.

   <Figure
     src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%209.webp"
     alt="Grafana panel showing unique instance count."
     caption="Grafana panel showing unique instance count."
   />
...
- Efficient High-Cardinality Management: SigNoz is optimized to handle metrics with numerous unique label combinations, improving performance and reducing resource strain compared to standard PromQL setups.
  ...
  <Figure
    ...
    alt="Efficient High-Cardinality Data Management using SigNoz"
    caption="Efficient High-Cardinality Data Management using SigNoz"
  />
- Streamlined Querying: SigNoz offers a more user-friendly interface for constructing complex queries, making it easier to perform multi-dimensional analysis without the complexity often encountered in PromQL.
  {' '}
  <Figure
    src="/img/docs/product-features/query-builder/group_by.gif"
    alt="Streamline Querying using SigNoz"
    caption="Streamline Querying using SigNoz"
  />
- Integrated Visualization: With built-in advanced visualization tools, SigNoz enables dynamic and insightful dashboards, making it simpler to analyze and understand metrics, especially when dealing with high-cardinality data.
  {' '}
  <Figure
    src="/img/guides/2024/09/prometheus-query-to-count-unique-label-values-image%2010.webp"
    alt="Integrated Visualization in SigNoz"
    caption="Integrated Visualization in SigNoz"
  />
