...
<Figure
  src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Screenshot_2024-11-09_at_11.52.45.webp"
  alt="Checking range vectors for `node_cpu_seconds_total` over 5 min"
  caption="Checking range vectors for `node_cpu_seconds_total` over 5 min"
/>
...
1. Align with Metric Frequency:
   The time range should generally match the frequency of data collection for your metric. - High-frequency metrics (e.g., per-second data) benefit from shorter time ranges (like 30s to 5m) for recent data aggregation. - Low-frequency metrics (e.g., daily data) are better suited to longer time ranges, such as several hours or days.
...
<Figure
  src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Screenshot_2024-11-09_at_19.23.50.webp"
  alt="Calculating a basic Range Vector Query over 10min"
  caption="Calculating a basic Range Vector Query over 10min"
/>
...
1. `avg_over_time`: Calculates the average value over the specified range duration.
   ...
   <Figure
     src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Screenshot_2024-11-09_at_20.28.14.webp"
     alt="Using `avg_over_time` function for 10min"
     caption="Using `avg_over_time` function for 10min"
   />
...
2. `min_over_time` and `max_over_time`: These functions return the minimum and maximum values within the specified time range.
   ...
   <Figure
     src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Screenshot_2024-11-09_at_20.29.15.webp"
     alt="Using `max_over_time` function to identify highest memory availability"
     caption="Using `max_over_time` function to identify highest memory availability"
   />
...
3. `sum_over_time`: Calculates the total sum of values within the range.
   ...
   <Figure
     src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Screenshot_2024-11-09_at_20.30.25.webp"
     alt="Using `sum_over_time` to add all prometheus HTTP requests over the last hour"
     caption="Using `sum_over_time` to add all prometheus HTTP requests over the last hour"
   />
...
4. `count_over_time`: Counts the number of data points available in the time range.
   ...
   <Figure
     src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-038bd15a-b3b3-45b8-88be-5d1b2757798e.webp"
     alt="Using `count_over_time`to check data availability"
     caption="Using `count_over_time`to check data availability"
   />
...
2.  Using Conditional Operators:
    Prometheus also supports conditional operators (`and`, `or`, `unless`) that help you filter and compare range vectors: - `and`: Returns values that match in both range vectors. - `or`: Returns values that match in either range vector. - `unless`: Excludes values present in the second vector from the first vector.
        For example:
...
- `SIGNOZ_INGESTION_KEY`: Authenticates your application with the SigNoz collector.
  ...
  {' '}
  <Figure
    src="/img/guides/2024/11/generating-range-vectors-from-return-values-in-prometheus-queries-Untitled.webp"
