...
meta_title: "Fix Duplicate Log Messages in Python Logging"
...
related_articles:
  ...
  - '/guides/how-should-i-log-while-using-multiprocessing-in-python'
  - '/blog/python-syslog'
---
...
- Child loggers are explicitly created using dot notation, forming a hierarchy such as `A.B.C` represents, `A` as a child logger of the root logger, `A.B` as child logger of parent logger `A` and similarly, `A.B.C` as a child of parent logger `A.B`.
    ...
    Child loggers inherit configuration from their parent loggers, but they can also have their own handlers, formatters, and levels. This allows for fine-grained control over logging output at different parts of an application.<br />
    Do note that this propagation behavior is rooted in how Python's [default logging formatter](https://signoz.io/guides/what-is-pythons-default-logging-formatter/) and logger hierarchy work.
...
We can then setup a sample flask app that sends the logs to SigNoz using [OpenTelemetry](https://signoz.io/opentelemetry/)
