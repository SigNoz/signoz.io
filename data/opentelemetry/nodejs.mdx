...
- Node.js **18.19+** or **20.6+**, per the <a href="https://github.com/open-telemetry/opentelemetry-js/blob/main/doc/upgrade-to-2.x.md#-nodejs-supported-versions" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry JS 2.x supported versions</a>. Node 18 reached <a href="https://nodejs.org/en/about/eol" target="_blank" rel="noopener noreferrer nofollow">end of life on March 27, 2025</a>, so use Node 20 or later.
...
<KeyPointCallout title="Sending Telemetry to Self-Hosted SigNoz" defaultCollapsed="true">
...
Because this project is CommonJS, every setup in this guide preloads instrumentation with `--require`. An ESM project (`"type": "module"`) needs a different mechanism entirely: see [Using ESM?](#using-esm) below.

If your `package.json` sets `"type": "module"`, you need `--import` to load the SDK and a loader hook so the instrumentation can patch your dependencies. Node 18.19.0+ is required:
...
You now have a Node.js service emitting traces, runtime metrics, and correlated logs through OpenTelemetry. Because all of it is [standard OTLP](https://signoz.io/blog/what-is-otlp/), the same instrumentation points at any compliant backend. It is only the endpoint that changes.
