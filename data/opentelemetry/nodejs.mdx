...
<KeyPointCallout>
 ...
 You don't configure these metrics anywhere. SigNoz derives them from the spans your application is already exporting — each span carries a start time, a duration, and a status, which is everything needed to compute request rate, error percentage, and latency percentiles per service and per endpoint.
 
 That means auto-instrumentation alone gets you a working service dashboard, and you only need the [custom metrics](https://signoz.io/opentelemetry/custom-metrics-nodejs/) covered later for numbers that aren't derivable from spans, such as queue depth or business counters.
...
Because the auto-instrumentation propagates trace context across HTTP calls, requests that hop from one service to the next arrive in SigNoz as a single connected trace.
