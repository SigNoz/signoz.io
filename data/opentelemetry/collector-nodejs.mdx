...
meta_title: "Set Up the OTel Collector - Node.js"
published_date: 2024-06-05
updated_date: 2026-07-14
tags: [opentelemetry-tutorials]
authors: [yuvraj]
description: "Set up the OpenTelemetry Collector to manage and export telemetry from your Node.js application to SigNoz."
keywords: 
  - opentelemetry
...
The [OpenTelemetry](https://signoz.io/opentelemetry/) Collector is a crucial component for managing and exporting telemetry data. It acts as a central point to receive, process, and export traces, metrics, and logs from various services, enhancing the observability of your microservices architecture.
...
- **Connection Issue:** [OpenTelemetry traces](https://signoz.io/blog/opentelemetry-spans/) are not being exported correctly, and errors like **`Parse Error: Expected HTTP/`** appear in the logs.
    ...
    **Solution**:
    ...
    Verify that the OpenTelemetry environment variables are set correctly. Ensure that the **`OTEL_EXPORTER_OTLP_ENDPOINT`** is set to the correct endpoint. Change the [OTLP (OpenTelemetry Protocol)](https://signoz.io/blog/what-is-otlp/) endpoint port from **`4317`** (gRPC) to **`4318`** (HTTP) if you're facing connection issues. This change resolved the issue in our setup:
...
- **Unauthorized Error with Signoz Ingest:** When trying to send traces to Signoz, you encounter a **`401 Unauthorized`** error.
    ...
    **Solution**:
    ...
    ```bash
    curl -v -H "signoz-ingestion-key: your-ingestion-key" https://ingest.in.signoz.cloud:443
    ```
