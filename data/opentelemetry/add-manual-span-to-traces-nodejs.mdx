...
meta_title: "Node.js OpenTelemetry - Custom Spans & Manual Tracing"
date: 2026-08-17
...
Before we begin, ensure you have a Node.js application already instrumented with OpenTelemetry — see the [OpenTelemetry Node.js guide](https://signoz.io/opentelemetry/nodejs/) if you haven't set that up yet. You'll need the following [OpenTelemetry](https://signoz.io/opentelemetry/) libraries in your order service:
...
A single span tells you how long an operation took. Nesting spans tells you *where inside it* the time went. In the validation flow above, one slow order could mean a slow product lookup or a slow stock update, and a flat span can't distinguish the two.

`startActiveSpan` sets the new span as the active span for the duration of its callback, so any span you start inside that callback automatically becomes its child. You don't need to pass parents around manually:

```jsx
async function validateOrder(order) {
  return tracer.startActiveSpan('validate-order', async (parentSpan) => {
    try {
      for (const item of order.products) {
        // Child span — parented to 'validate-order' automatically
        await tracer.startActiveSpan('check-product-stock', async (childSpan) => {
          childSpan.setAttribute('product.id', item.productId)
          childSpan.setAttribute('product.quantity', item.quantity)

          try {
            const res = await fetch(`http://product:3003/products/${item.productId}`)
...
In the SigNoz flamegraph this renders as one `validate-order` bar with a `check-product-stock` bar per product nested beneath it, so a single slow lookup is immediately visible.

Two things worth getting right:

- **Always end spans in a `finally` block.** A span whose `end()` is skipped by an early return or a thrown error never gets exported, and the trace arrives incomplete.
- **Keep span names low-cardinality.** Name the operation (`check-product-stock`), not the instance (`check-product-stock-abc123`). Identifying values belong in attributes, where they stay queryable without fragmenting your trace data.
...
<Admonition type="info">
  Next, learn how to define and visualize your own metrics in [Setting up Custom Metrics -
  OpenTelemetry Node.js](https://signoz.io/opentelemetry/custom-metrics-nodejs/).
</Admonition>
