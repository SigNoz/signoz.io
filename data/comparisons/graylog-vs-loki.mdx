...
- **Lucene-based Search Syntax**: Imagine being able to search not just for "Shakespeare" but for "books mentioning Hamlet in the first 10 pages."
  - _Note: Lucene is an open-source library for indexing and searching text. In Graylog, Lucene-based search allows efficient indexing and quick retrieval of log data using complex queries, similar to how a library index helps find books._
- **Versatile Input Types**: Accepts books (logs) from diverse sources, whether old manuscripts (syslog) or modern PDFs (GELF).
  - _Note: Syslog acts like a messenger that delivers log data from different sources to a central location for storage and analysis._
...
- Loki stores log data as compressed chunks in object storage like S3 or GCS. Instead of creating full-text indexes, Loki uses minimal indexes based on labels (like `app="webapp"`).
  - _Note: Chunks are fixed-size, compressed blocks of log data, typically representing a specific time range of logs from a single stream (defined by unique label combinations). They are immutable, compressed using algorithms, and stored in scalable object storage systems._
