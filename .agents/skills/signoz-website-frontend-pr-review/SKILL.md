...
- **Listicle / icon-card items must live in `constants/componentItems.ts`**, not inside component files. Flag any `{ name, href, clickName }` arrays defined inline in a component.
