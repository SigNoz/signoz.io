...
## Collecting Nodejs logs when application is deployed on Docker or Kubernetes
  When yourn application is deployed in Docker or a Kubernetes cluster the logs from the console are automatically collected and store in the node. The signoz collector will automatically collect these and  it will be visible on the UI. 
  You can add a console transport very easily as stated [here](https://github.com/winstonjs/winston/blob/master/docs/transports.md#console-transport) 
  ```
...
## Collecting Nodejs logs when application is deployed on a Host
  ...
  You can add a file transport very easily as stated [here](https://github.com/winstonjs/winston/blob/master/docs/transports.md#file-transport)
  ```
