...
1. **Use an unique correlation ID per request**
...
3. **Add contextual data in your logs**
...
6. **Provide informative application logs**
...
7. **Centralized logging solution**
...
If the application logs contained request context identifiers (such as trace ids, span ids, traceflags or user-defined baggage based on [https://www.w3.org/TR/trace-context/#traceparent-header](https://www.w3.org/TR/trace-context/#traceparent-header) ) it would result in a much richer correlation between logs and traces, as well as correlation between logs emitted by different components of a distributed system. This would make logs significantly more valuable in distributed systems.
...
1. We do an initial set up of zap logger.

<figure data-zoomable align='center'>
    <img src="/img/blog/2023/01/microservices-logging-setupLog.webp" alt="Microservices Logging Setup Log."/>
    <figcaption><i>Microservices Logging Setup Log.</i></figcaption>
</figure>


2. Secondly, we wrap the zap logger and a context for use to add trace context information in the log messages.

<figure data-zoomable align='center'>
    <img src="/img/blog/2023/01/microservices-logging-logger-with-ctx.webp" alt="Microservices Logging Logger with ctx."/>
    <figcaption><i>Microservices Logging Logger with ctx.</i></figcaption>
</figure>

3. The APIs which is using the log method, Passing context configures the logger to add trace_id, span_id and trace_flags field to structured log messages. This option is only useful with backends that don't support OTLP and instead parse log messages to extract structured information.
...
4. With the integration of wrapper with the zap library, this is how our logs look like(with traceid, spanid and traceflags). This follows the [log data model](https://opentelemetry.io/docs/reference/specification/logs/data-model/) from OpenTelemetry. For comparison, let’s look at logs with and without the integration of the wrapper.
