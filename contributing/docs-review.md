...
21. In fences of more than 40 lines, `minimap` goes with `defaultCollapsed`, on reference-only content: a full manifest, a complete source file, a log, a stack trace. The core instructional snippet for the step carries neither flag. Flag `minimap` without `defaultCollapsed` as P2. Flag reference-only content that misses the pair as P3. Flag a low `collapse={N}` that hides instructional content as P2. See [When to use defaultCollapsed / minimap](docs-codeblock.md#when-to-use-defaultcollapsed--minimap).
