...
function GetStartedColumn({ option, index }: { option: GetStartedOption; index: number }) {
  return (
    <article
      className={`flex min-h-[220px] flex-col justify-between border-signoz_slate-400/35 py-8 md:px-12 ${
        index === 0 ? 'md:border-x' : ''
      }`}
    >
      <div className="max-w-[330px]">
        <h3 className="m-0 text-sm leading-5 text-signoz_vanilla-400">{option.title}</h3>
        <p className="m-0 mt-3 text-xl font-medium leading-8 tracking-[-0.2px] text-signoz_vanilla-100">
          {option.description}
        </p>
      </div>
      ...
      <div>
        ...
        <p className="m-0 mt-2 text-[24px] font-medium leading-none tracking-[-0.45px] text-signoz_vanilla-100">
          {option.price}
        </p>
      </div>
    </article>
...
export default function HomepageGetStartedExperiment() {
  return (
    <section
    ...
    >
      <div className="mx-auto max-w-[1200px] border-y border-signoz_slate-400/35">
        <div className="grid gap-0 md:grid-cols-[0.82fr_1fr_1fr]">
          <div className="flex min-h-[220px] flex-col justify-between border-b border-signoz_slate-400/35 py-8 md:border-b-0 md:pr-8">
            <div>
              <h2 className="m-0 max-w-[310px] text-[38px] font-medium leading-[1.08] tracking-[-0.95px] text-signoz_vanilla-100 md:text-[44px] md:tracking-[-1.15px]">
