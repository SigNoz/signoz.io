...
const Authors: AuthorInterface = {
  ...
  bhupesh: {
    ...
    image_url:
      "https://signoz.io/img/authors/bhupesh_varshney_profile_pic.webp",
  },
  ezz: {
...
export default function AuthorInfo({ author }: { author: string }) {
  ...
  return (
    <div
      className="author-info flex items-center gap-4 my-4"
      onClick={handelAuthorClick}
    >
      <Image
      ...
      <div className="author-name-team">
        <h4 className="mb-2 mt-0 capitalize">{Authors[author]?.name}</h4>
        <div className="mb-2 capitalize text-xs">{Authors[author]?.title}</div>
      </div>
    </div>
  );
}
