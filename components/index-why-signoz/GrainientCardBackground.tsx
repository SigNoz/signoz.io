...
import { hexToRgbFloat32 } from '@/utils/hexToRgb'
...
export default function GrainientCardBackground({ className }: GrainientCardBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const uniforms = {
      iResolution: new Float32Array([1, 1]),
      iTime: 0,
      uColor1: hexToRgbFloat32(SIGNOZ_COLORS.robin500),
      uColor2: hexToRgbFloat32(SIGNOZ_COLORS.cherry500),
      uColor3: hexToRgbFloat32(SIGNOZ_COLORS.ink500),
      uColor4: hexToRgbFloat32(SIGNOZ_COLORS.slate500),
      uColor5: hexToRgbFloat32(SIGNOZ_COLORS.aqua600),
    }
    ...
    const render = () => {
      ...
      if (uniformLocations.iResolution) {
        gl.uniform2fv(uniformLocations.iResolution, uniforms.iResolution)
      }
      if (uniformLocations.iTime) gl.uniform1f(uniformLocations.iTime, uniforms.iTime)
      if (uniformLocations.uColor1) gl.uniform3fv(uniformLocations.uColor1, uniforms.uColor1)
      if (uniformLocations.uColor2) gl.uniform3fv(uniformLocations.uColor2, uniforms.uColor2)
      if (uniformLocations.uColor3) gl.uniform3fv(uniformLocations.uColor3, uniforms.uColor3)
      if (uniformLocations.uColor4) gl.uniform3fv(uniformLocations.uColor4, uniforms.uColor4)
      if (uniformLocations.uColor5) gl.uniform3fv(uniformLocations.uColor5, uniforms.uColor5)
      gl.drawArrays(gl.TRIANGLES, 0, 3)
    }

    const resize = () => {
      const rect = container.getBoundingClientRect()
      resizeCanvas(gl, Math.max(1, rect.width), Math.max(1, rect.height), pixelRatio)
      uniforms.iResolution[0] = gl.drawingBufferWidth
      uniforms.iResolution[1] = gl.drawingBufferHeight
      render()
    }
    ...
    const loop = (time: number) => {
      uniforms.iTime = (time - startTime) * 0.001
      render()
  ...
  return (
    <div
      ...
      style={{
        background:
          'radial-gradient(circle at 18% 18%, rgba(78, 116, 248, 0.6), transparent 32%), radial-gradient(circle at 82% 18%, rgba(7, 175, 230, 0.36), transparent 34%), linear-gradient(135deg, #0B0C0E 0%, #161922 42%, #E5484D 64%, #4E74F8 100%)',
      }}
    />
  )
}
