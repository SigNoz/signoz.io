...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, variant, children, ...props }, ref) => {
    return (
      <div className={cn('bg-card', cardVariants({ variant }), className)} ref={ref} {...props}>
        <div className="border-l2-border/50 relative h-full rounded-md border-2 border-dashed">
          {children}
        </div>
      </div>
    )
