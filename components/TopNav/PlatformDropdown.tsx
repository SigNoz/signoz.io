...
import { platformDropdownItemsSorted } from './constants'
import { useNavDropdown } from './NavDropdownContext'

export default function PlatformDropdown() {
  const { isOpen, open, close, triggerRef } = useNavDropdown('platform')
...
export function PlatformDropdownContent({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-y-4 p-6">
      <div className="grid grid-cols-1 gap-y-4">
        {platformDropdownItemsSorted.map((item) => (
          <TrackingLink
            href={item.url}
            className="group flex h-auto min-w-0 items-start gap-2"
            key={item.key}
            clickType="Nav Click"
            clickName={`${item.name} Platform Link`}
            clickText={item.name}
