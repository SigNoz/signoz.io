...
import type { OpenAPISpec, OpenAPIDocument } from '@/utils/openapiSpec'
...
/** One entry per operation per tag, so multi-tagged operations list under each. */
 ...
 */
export function buildApiReferenceMarkdown(spec: OpenAPISpec): string {
  ...
  return [
    ...
    `- Per-release markdown: ${siteMetadata.siteUrl}/api-reference/<release>.md (a release tag, e.g. \`${version}\`, or \`latest\`)`,
