    ...
    <Card className={`w-full ${className}`}>
      <div className={`relative h-full overflow-auto p-6`}>
        <div className="mb-4 flex items-center justify-between">
          ...
          {isCompleted && (
            <div className="flex items-center gap-2 text-sm text-signoz_forest-500">
        ...
        <div className="space-y-4">
          {/* Version Info */}
          <div className="flex flex-col rounded-lg bg-signoz_slate-400 p-3">
            ...
            <ul className="space-y-1">
              {releaseInfo?.instructions?.map((instruction, index) => (
                <li key={index} className="flex items-start gap-2 text-sm text-gray-300">
                  <span className="mt-1 text-signoz_robin-500">•</span>
                  {instruction ?? ''}
                </li>
              ))}
            </ul>
            ...
            <div>
              ...
              <div className="space-y-2">
                {releaseInfo?.warnings?.map((warning, index) => (
                  <div
                    key={index}
                    className="rounded-lg border border-signoz_cherry-400/20 bg-signoz_cherry-400/10 p-3"
                  >
                    <h5 className="mb-1 font-medium text-signoz_cherry-400">
                      {warning?.title ?? ''}
                    </h5>
          ...
          {/* Completion Button */}
          <div className="border-t-2 border-dashed border-signoz_slate-200/50 pt-4">
            <Button
              isButton={true}
