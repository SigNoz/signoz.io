...
const Header: React.FC = () => {
  ...
  return (
    <FeaturePageHeader
      title={
        <>
          Log Management Software for <br />
          <span className="text-gradient">High-Performance Log Analytics</span>
        </>
      }
      description={
        <>
          Collect logs from every service into one fast store, search them in seconds, and connect
          any line to the metric or trace behind it.
        </>
      }
...
const TerminalMock: React.FC = () => {
  ...
  return (
    <div className="z-1 relative w-full overflow-hidden rounded-xl border border-signoz_slate-400 bg-signoz_ink-500">
      {/* title bar */}
      ...
      <div className="mx-auto max-w-3xl space-y-3">
        {FAQ_ITEMS.map((item, i) => {
          ...
          return (
            <div
              key={i}
              className={`overflow-hidden rounded-xl border bg-signoz_ink-400 transition-colors ${
                isOpen ? 'border-signoz_slate-100' : 'border-signoz_slate-400'
              }`}
            >
              <button
                onClick={() => setOpenIndex(isOpen ? null : i)}
                className="flex w-full cursor-pointer items-center justify-between gap-4 border-none bg-transparent px-5 py-4 text-left text-base font-semibold text-signoz_vanilla-100"
              >
                {item.question}
                <motion.span
                  animate={{ rotate: isOpen ? 45 : 0 }}
                  transition={{ duration: 0.2 }}
                  className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg border text-lg text-signoz_cherry-500 ${isOpen ? 'border-signoz_cherry-500/30 bg-signoz_cherry-500/10' : 'border-signoz_slate-400'}`}
                >
                  +
                </motion.span>
              </button>
              <AnimatePresence>
                {isOpen && (
                  ...
                  >
                    <p className="m-0 px-5 pb-5 text-sm leading-relaxed text-signoz_vanilla-400">
                      {item.answer}
                    </p>
                  </motion.div>
                )}
