...
export function GameResults({ isWon, score = 0, timeTaken, targetWord, guesses }: GameResultsProps) {
  ...
  function generateShareText () {
    ...
    if (guesses.length === 0) {
      return `Hey there, I just played today's DevOps Wordle by SigNoz and got a score of ${score}. Can you beat me 😉? Try it out at https://wordle.signoz.io/todayswordle.`;
    } else {
      const emojiMatrix = generateEmojiMatrix();
      return `Hey there, I just played today's DevOps Wordle by SigNoz and got a score of ${score} in ${formattedTime} minutes. Can you beat me 😉? Here's my game: \n\n${emojiMatrix}\n\nTry it out at https://wordle.signoz.io/todayswordle.`;
    }
  }
  ...
  return (
    <div className={`flex flex-col items-center justify-center gap-6 p-8 bg-black/30 backdrop-blur-md rounded-lg border-2 ${isWon ? 'border-[#233457] neon-box-border' : 'border-[#cc3939] neon-box-red'} relative mt-12 max-w-[600px] w-full mx-auto mb-10 ${lexend.className}`}>
      ...
      <div className="x-cta text-center" > 
        <p className="text-gray-400 text-sm ">
          <a href="https://x.com/signoz_io" target="_blank" className="text-[#4558c4] hover:text-[#5569d7] inline-block hover:underline">~ A new wordle is released every midnight on X ~</a>
        </p>
      </div>
