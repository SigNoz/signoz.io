...
const ExperimentVariant: React.FC<{
  formData: FormState
  errors: ErrorsProps
  isSubmitting: boolean
  submitFailed: boolean
  ...
  return (
    <form className="w-100 mt-[24px]">
      ...
      <button
        ...
        className={`mb-[16px] flex w-full items-center justify-center rounded-full ${variantId === EXPERIMENTS.TEAMS_PAGE.variants.VARIANT ? 'bg-signoz_cherry-500' : 'bg-signoz_robin-500'} py-2 pl-4 pr-3 font-medium ${isSubmitting ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}
      >
        {isSubmitting ? (
